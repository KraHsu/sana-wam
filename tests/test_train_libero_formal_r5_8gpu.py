...
CONFIG = Path("configs/experiments/libero_formal_r5_8gpu_epoch1.yaml")
RUNNER = Path("scripts/train_libero_formal_r5_8gpu.py")
TRAINER = Path("src/sana_wam/train/trainer.py")


def test_r5_config_is_one_full_epoch_at_preserved_global_batch() -> None:
...
def test_r5_is_strict_warm_start_with_uniform_full_noise_support() -> None:
    cfg = OmegaConf.load(CONFIG)
    ...
    assert cfg.training.warmup_steps == 66


def test_r5_checkpoint_contract_matches_default_deployment() -> None:
...
def test_r5_runner_has_single_supervisor_and_zero_restart_torchrun() -> None:
...
def test_r5_runner_uses_two_stage_terminal_commit_and_safe_ancestry() -> None:
    source = RUNNER.read_text(encoding="utf-8")
    ast.parse(source)
    success_body = source[
...
def test_r5_worker_collectively_commits_summary_and_emits_monitor_logs() -> None:
    source = RUNNER.read_text(encoding="utf-8")
    ast.parse(source)
