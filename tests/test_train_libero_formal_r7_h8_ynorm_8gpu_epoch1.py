...
CONFIG = Path("configs/experiments/libero_formal_r7_h8_ynorm_8gpu_epoch1.yaml")
RUNNER = Path("scripts/train_libero_formal_r7_h8_ynorm_8gpu_epoch1.py")
TRAINER = Path("src/sana_wam/train/trainer.py")


def test_r7_h8_ynorm_epoch1_config_is_one_full_epoch_at_preserved_global_batch() -> None:
...
def test_r7_h8_ynorm_epoch1_is_strict_r6_warm_start_with_restored_text_rmsnorm() -> None:
    cfg = OmegaConf.load(CONFIG)
    ...
    assert list(cfg.training.init_checkpoint_allow_missing_patterns) == [
    ...
    assert cfg.training.formal_predecessor_r6_result_sha256 == (
        "1b448d4cc92f284844ba44c68dc536d1a22fee0a75bf76541fed8131ec756b90"
    )
    assert cfg.training.scientific_predecessor_r6_formal40_result_sha256 == (
        "642d262526122c660a71b50d9c535e591ab7866687ea134a5e0a7d868a5d74ca"
    )


def test_r7_h8_ynorm_epoch1_checkpoint_contract_matches_default_deployment() -> None:
...
def test_r7_h8_ynorm_epoch1_runner_has_single_supervisor_and_zero_restart_torchrun() -> None:
    ...
    assert "SANA_WAM_R7_H8_YNORM_SUPERVISED_WORKER" in source
    assert "FAILED_CLOSED_NON_RESUMABLE" in source
    assert "automatic_rerun_permitted" in source
    assert "r6_h8_formal40" in source
    assert 'aggregate.get("episodes") != 40' in source
    assert 'aggregate.get("successes") != 2' in source
    assert "scientific_r6_h8_formal40" in source
    assert "_verify_materialized_ynorm_warm_start" in source
    assert source.count("weight.numel() != 2_240") == 2
    assert "weight.numel() != 2_304" not in source
    assert '"checkpoint_missing_only": [YNORM_WEIGHT_KEY]' in source
    assert '"pretrained_text_rmsnorm_restored": True' in source
...
def test_r7_h8_ynorm_epoch1_runner_binds_pretrained_ynorm_before_root_creation() -> None:
    source = RUNNER.read_text(encoding="utf-8")
    ast.parse(source)
    assert (
    ...
    source_offset = supervisor.index(
        "pretrained_ynorm_source = _verify_pretrained_ynorm_source()"
    )
    root_offset = supervisor.index("_prepare_root_parent()")
    assert source_offset < root_offset
    assert source.count("checkpoint_artifact = _verify_file(") == 1
    assert source.count('"pretrained_ynorm_source": pretrained_ynorm_source') == 2
    assert "SANA_BASE_YNORM_BF16_SHA256" in source[
        source.index("def _verify_materialized_ynorm_warm_start") : source.index(
            "def _verify_saved_ynorm_checkpoint"
        )
    ]


def test_r7_h8_ynorm_tensor_digest_rejects_single_element_drift() -> None:
...
def test_r7_h8_ynorm_epoch1_runner_uses_two_stage_terminal_commit_and_safe_ancestry() -> None:
    source = RUNNER.read_text(encoding="utf-8")
    ast.parse(source)
    success_body = source[
...
def test_r7_h8_ynorm_epoch1_worker_collectively_commits_summary_and_emits_monitor_logs() -> None:
    source = RUNNER.read_text(encoding="utf-8")
    ast.parse(source)
    assert "logging.basicConfig(" in source
    assert "R7 preflight complete; entering formal training" in source
