...
from sana_wam.model.video_backbone.wan.shared.core.gradient.gradient_checkpoint import gradient_checkpoint_forward
...
class ActionDiTState:
    ...
    t_mod: torch.Tensor  # (B, t_mod_params, dim) per-sample, or (B, T_action, t_mod_params, dim) per-token (AR)
    t_embed: torch.Tensor  # (B, dim) or (B, T_action, dim)
    ...
    # AR (block-causal) extras. ``frame_ids`` gives each action token its chunk
    # index with modality parity (``2c+1``) for framed RoPE + the AR attention
    # descriptor; ``None`` keeps the non-AR per-sample / 1D-RoPE behaviour.
    frame_ids: Optional[torch.Tensor] = None  # (T_action,) long
...
class ActionDiT(ActionBackbone):
    ...
    def _get_rope_freqs_at(self, positions: torch.Tensor) -> torch.Tensor:
        """Gather precomputed 1D RoPE frequencies at explicit per-token positions.

        Used by the AR (framed) path so a token's noisy/clean copies share a
        rotary phase and action tokens carry a frame-aligned temporal position.
        """
        positions = positions.to(dtype=torch.long, device=self.freqs.device)
        if int(positions.max().item()) >= self.freqs.shape[0]:
            raise ValueError(
                f"rope position {int(positions.max().item())} exceeds precomputed RoPE cache "
                f"length {self.freqs.shape[0]}; increase ``max_action_len``."
            )
        if int(positions.min().item()) < 0:
            raise ValueError("rope_positions must be non-negative.")
        return self.freqs[positions]
    ...
    def prepare_state(
        ...
        *,
        ...
        token_proprio_emb: Optional[torch.Tensor] = None,
        ...
        """Initialise the action-side joint-loop state.

        Per-token timestep (AR): when ``token_timesteps`` ``(B, T_action)`` is
        given, AdaLN modulation is built **per action token** (``t_mod`` becomes
        ``(B, T_action, t_mod_params, dim)``) so noisy and clean copies of the
        duplicated AR sequence — and different chunks within the noisy copy —
        ...
        if token_timesteps is not None:
            T = x.shape[1]
            if token_timesteps.shape != (B, T):
                raise ValueError(
                    f"token_timesteps must be (B={B}, T_action={T}); got {tuple(token_timesteps.shape)}."
                )
            t = self.time_embedding(token_timesteps.reshape(-1)).reshape(B, T, -1)  # (B, T, dim)
            if token_proprio_emb is not None:
                # F4: additive per-token proprio delta (zero-init encoder ⇒ no-op at start).
                t = t + token_proprio_emb.to(t.dtype)
            t_mod = self.time_projection(t)  # (B, T, t_mod_params, dim)
            timestep = token_timesteps
        else:
            timestep = self._prepare_timestep(timestep, B)
            t = self.time_embedding(timestep)
            t_mod = self.time_projection(t)
        # Framed RoPE (AR): index the precomputed 1D frequencies by an explicit
        # per-token position instead of ``arange(T)``. The duplicated AR sequence
        # requires a token's noisy and clean copies to share the *same* rotary
        # phase, and action tokens to carry a temporal (frame-aligned) position;
        # the caller passes positions that satisfy both. ``None`` ⇒ original
        # ``self.freqs[:T]`` behaviour, byte-identical.
        if rope_positions is not None:
            T = x.shape[1]
            if rope_positions.shape != (T,):
                raise ValueError(f"rope_positions must be (T_action={T},); got {tuple(rope_positions.shape)}.")
            action_freqs = self._get_rope_freqs_at(rope_positions).to(device=x.device)
        else:
        ...
        payload = ActionDiTState(
            ...
            frame_ids=(frame_ids.to(device=x.device) if frame_ids is not None else None),
        )
        return ActionState(
    ...
    def pre_attn_at_layer_for_compile(
        ...
        t_mod = payload.t_mod
        modulation = block.modulation.to(dtype=t_mod.dtype, device=t_mod.device)
        if t_mod.dim() == 4:
            # Per-token AR modulation: t_mod (B, T, 6, dim); broadcast the
            # (1, 6, dim) block params over the token axis, then split into six
            # per-token (B, T, dim) tensors that modulate norm(x) elementwise.
            chunks = (modulation.unsqueeze(1) + t_mod).chunk(6, dim=2)
            shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = (c.squeeze(2) for c in chunks)
        else:
            chunks = (modulation + t_mod).chunk(6, dim=1)
            shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = chunks
