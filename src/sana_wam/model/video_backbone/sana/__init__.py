...
# Walk up from this file until we find a dir containing ``third_party/Sana``
# (robust to the src/ layout instead of a fixed parents[N] index).
_SANA_ROOT = None
for _anc in _Path(__file__).resolve().parents:
    _cand = _anc / "third_party" / "Sana"
    if _cand.is_dir():
        _SANA_ROOT = _cand
        break
if _SANA_ROOT is not None and str(_SANA_ROOT) not in _sys.path:
...
from sana_wam.model.video_backbone.sana.adapter import SanaVideoBackbone  # noqa: E402
from sana_wam.model.video_backbone.sana.blocks_split import SanaMSVideoSplit  # noqa: E402
