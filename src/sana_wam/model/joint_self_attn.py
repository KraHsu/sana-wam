...
from sana_wam.model.ar.mot_compile import CompiledMoTLoop
from sana_wam.model.ar.mot_driver import MoTJointDriver
from sana_wam.model.compile_options import compile_mode, section_enabled, self_attn_compile_cfg
...
class DualSystemSelfAttnArchitecture(BaseWAMArchitecture):
    ...
    def __init__(self, cfg=None):
        ...
        # SANA-only knob: rebalances the linear-attention denominator for
        # action-query rows so the ~100:1 video:action token imbalance doesn't
        # swamp action self-attention (see SanaMoTJointDriver). Only forwarded
        # to SanaMoTJointDriver; the softmax MoTJointDriver never sees it.
        if str(cfg.get("attn_kernel", "softmax")) == "linear_relu":
            asw = cfg.get("action_self_attn_weight", None)
            if asw is not None:
                self._mot_driver_kwargs["action_self_attn_weight"] = float(asw)
            asm = cfg.get("action_self_attn_mode", None)
            if asm is not None:
                self._mot_driver_kwargs["action_self_attn_mode"] = str(asm)
            avm = cfg.get("action_video_attn_mode", None)
            if avm is not None:
                self._mot_driver_kwargs["action_video_attn_mode"] = str(avm)
        if self.video_backbone is not None:
    ...
    def build_mot_driver(self) -> MoTJointDriver:
        ...
        if kernel == "linear_relu":
            from sana_wam.model.ar.sana_mot_driver import (
                SanaMoTJointDriver,
            )
