"""AV-1B single-call bridge to Sana's autograd Triton ChunkCausalGDN.

This module is deliberately limited to the frozen AV-1B non-formal screen.
...
``end-of-bin reducer -> 20/64/20 conditioner -> per-block zero-init 20/64``.
...
AV1B_HW: Final = (5, 1, 1)
AV1B_INITIALIZER_REVISION: Final = "cach-av1b-vendor-gdn-bridge-v1"
AV1B_SEED0: Final = 202608033
AV1B_SHARED_NAMED_SEED: Final = 2026080331
AV1B_CANDIDATE_ONLY_SEED: Final = 2026080332
...
class AV1BVendorGDNBridgeSpec:
    """Exact reduced topology frozen by the AV-1B run card."""
...
def _require_cuda_zero(device: torch.device | str) -> torch.device:
    ...
    if resolved.type != "cuda" or resolved.index != 0:
        raise ValueError("AV-1B is frozen to logical device cuda:0")
    return resolved
...
def _validate_source_task(source: AV1SyntheticTask) -> None:
    if source.recipe_digest != AV1B_TASK_RECIPE_SHA256:
        raise RuntimeError("the source task recipe digest differs from AV-1B")
...
def _validate_vendor_task(task: AV1BVendorTask) -> None:
    ...
    if any(
        ...
        raise TypeError("all AV-1B floating task tensors must be cuda:0 FP32")
    ...
    if any(
        ...
        raise TypeError("all AV-1B mask tensors must be cuda:0 bool")
    exact_shapes = {
    ...
    if len(task.chunks) != 2:
        raise ValueError("AV-1B requires exactly two layout chunks")
        ...
        *,
        ...
        if self.action_output_projection is not None:
            if action_condition is None:
                raise RuntimeError("candidate block requires action condition")
            post_seam = post_vendor + self.action_output_projection(
                action_condition
            )
...
class AV1BVendorGDNBridgeArm(nn.Module):
    """One AV-1B arm; only CACH-A owns or evaluates an action seam."""
    ...
    def _candidate_actions_for_mode(
        ...
        if mode == "no_action":
            if self.no_action_slot is None:
                raise RuntimeError("candidate no-action slot is absent")
            return self.no_action_slot.view(1, 1, -1).expand(
                self.spec.batch_size,
                self.spec.valid_action_count,
                self.spec.action_dim,
            )
        raise ValueError("mode must be correct, shuffle, or no_action")
    ...
    def _candidate_action_condition(
        ...
        for task_chunk in task.chunks:
            ...
            valid_parts.append(reduced.latent_valid_mask[:, :valid_count])
        ...
        if not bool(valid_mask.all()):
            raise RuntimeError("all five single-call frames must be valid")
        conditioned = self.action_conditioner(raw)
        conditioned = torch.where(
            valid_mask.unsqueeze(-1),
            conditioned,
            torch.zeros_like(conditioned),
        )
        return raw, conditioned, valid_mask
    ...
    def forward(
        ...
        *,
        ...
        if last_post_vendor is None:
            raise AssertionError("AV-1B depth must be nonzero")
...
def build_av1b_vendor_gdn_pair(
    ...
    *,
    ...
    expected_candidate_extra_parameters = candidate_only | {"no_action_slot"}
    if (
    ...
    if frozenset(candidate_buffers) - common_buffer_state:
        raise RuntimeError("unlisted candidate-only buffer state is forbidden")
    for name in sorted(
    ...
    for block in candidate.blocks:
        ...
        if bool(projection.bias.detach().count_nonzero()):
            raise RuntimeError("candidate action projection bias must be zero")
...
def pure_torch_last_block_tail(
    ...
    """Evaluate only the last action seam, FFN, and video-output tail."""

    post_seam = post_vendor_hidden + F.linear(
        action_condition,
...
def _last_tail_fixed_inputs(
...
) -> tuple[AV1BVendorGDNBlock, Tensor, Tensor]:
    ...
    if output.last_block_action_condition is None:
        raise ValueError("candidate output lacks its action condition")
    block = arm.blocks[-1]
    ...
    return (
        ...
        output.last_block_action_condition.detach(),
    )
...
def last_block_local_seam_parameter_jvp(
    ...
    block, post_vendor, action_condition = _last_tail_fixed_inputs(arm, output)
    projection = block.action_output_projection
...
def last_block_local_action_condition_jvp(
    ...
    *,
    ...
    """JVP over the detached last-block action-condition input only."""

    block, post_vendor, action_condition = _last_tail_fixed_inputs(arm, output)
    projection = block.action_output_projection
    assert projection is not None
    if valid_mask is None:
        valid_mask = output.action_condition_valid_mask
    if valid_mask is None:
        raise ValueError("candidate output lacks an action-condition mask")
    valid_mask = valid_mask.detach()
    direction = _alternating_unit_direction(action_condition, valid_mask)

    def tail(dynamic_action_condition: Tensor) -> Tensor:
        return pure_torch_last_block_tail(
            post_vendor,
    ...
    rms = tangent_output.float().square().mean().sqrt()
    return AV1BLocalJVPDiagnostic(
        scope="LAST_BLOCK_POST_VENDOR_ACTION_CONDITION_PURE_TORCH_TAIL",
        primal=primal,
        tangent_output=tangent_output,
        input_direction=direction,
        rms=rms,
...
def _screen_require_config(config: Mapping[str, object]) -> None:
    ...
    if config.get("schema") != "cach.av1b.vendor_gdn_bridge.config.v1":
        raise ValueError("AV-1B config schema differs")
    ...
    for name, expected in expected_topology.items():
        if topology.get(name) != expected:
            raise ValueError(f"config topology.{name} differs from AV-1B")
    ...
    for name, expected in expected_optimizer.items():
        if optimizer.get(name) != expected:
            raise ValueError(f"config optimizer.{name} differs from AV-1B")
...
def _screen_train_arm(
    ...
    *,
    ...
    if len(loss_trace) != 201:
        raise RuntimeError("loss trace must contain steps 0 through 200")
...
def _screen_jit_inventory(
    ...
    if not cache_text:
        raise RuntimeError("TRITON_CACHE_DIR is required for AV-1B")
...
def run_av1b_screen(
    ...
    *,
    ...
    """Run the complete in-memory AV-1B two-arm non-formal screen.
    ...
    if os.environ.get("FUSED_GDN_PRECISION") != "0":
        raise RuntimeError("AV-1B requires FUSED_GDN_PRECISION=0")
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("AV-1B requires exactly one visible CUDA device")
    ...
    if not final_action_jvp.finite:
        raise FloatingPointError("step200 local action-condition JVP is non-finite")
    ...
    metrics: dict[str, object] = {
        ...
        "primary_reference_step200": reference_primary,
        "primary_candidate_correct_step200": candidate_primary,
        "primary_candidate_shuffle_step200": shuffle_primary,
        "primary_candidate_no_action_step200": no_action_primary,
        ...
        "last_block_local_action_condition_jvp_rms_step200": float(
    ...
    finite_metric_values = all(
        ...
        for value in (
            candidate_train.seam_grad_rms_step0,
    ...
    card_validity_checks: Mapping[str, bool] = {
        **identity_checks,
        ...
        "exact_optimizer_steps": (
            reference_train.optimizer_steps == 200
            and candidate_train.optimizer_steps == 200
        ),
    ...
    diagnostics: Mapping[str, object] = {
        ...
        "no_action_frozen_exact_zero": no_action_slot_valid,
        "optimizer_steps_completed_by_arm": {
            "REF-GDN-CORRECTED": reference_train.optimizer_steps,
            "CACH-A": candidate_train.optimizer_steps,
        },
        ...
        "post_vendor_observability": {
            ...
            "last_block_step200_post_vendor_hidden_sha256": tensor_digest(
                candidate_train.final_output.last_block_post_vendor_hidden.detach()
            ),
        },
        ...
        "local_jvp": {
            ...
            "step200_action_condition_scope": final_action_jvp.scope,
            "step200_action_condition_primal_digest": (
    ...
    return {
        "schema": "cach.av1b.vendor_gdn_bridge.screen.v1",
        "validity": {
            "implementation_valid": exact_vendor_identity,
            "data_valid": (
