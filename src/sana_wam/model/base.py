...
class BaseWAMArchitecture(ABC, nn.Module):
    ...
    def prepare_inputs(self, batch: list[dict]) -> dict:
        ...
        # Per-sample clean-prefix length in ACTION steps (growing-history): action
        # steps [0, k) are the observed past (already executed) and are excluded from
        # the action loss in compute_loss, so the model is supervised only on future
        # actions. Absent ⇒ 0 (legacy: supervise the whole trajectory).
