...
def _check_temporal_divisibility(num_video_frames: int, temporal_compression: int, causal_temporal: bool) -> None:
    ...
    ``sana_wam.train.utils.temporal_contract.apply_temporal_contract_bridge``
...
class RoboTwinDataset(BaseActionDataset):
        ...
        if self.normalize_mode is not None:
            ...
            if action_stats_path is not None:
                ...
                if os.path.exists(stats_path):
                    ...
                    from sana_wam.dataloader.robotwin_stats_computation import (
    ...
    def _build_sample(self, ep_idx: int, start: int, logical_len: Optional[int] = None) -> dict:
        ...
        *valid* (the rest read as padding via the masks), so a fixed max-T
        ...
        # F4 per-chunk proprio: full normalized state sequence (state at each raw
        # step; raw_actions[t] = state at step t). The architecture extracts the
        # per-chunk states it needs (it knows num_chunks/action_tokens_per_chunk).
        proprio_seq_tensor = torch.from_numpy(raw_actions[: self.num_frames].astype(np.float32))
