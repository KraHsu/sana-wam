...
class AsyncInferenceExecutor:  # pragma: no cover - AR mandates async=none
    """Stub. The block-AR engine is sync-only (async breaks the KV cache), so this
    is never constructed. Present only so ``from ...optimizations import
    AsyncInferenceExecutor`` resolves in the (disabled) async branch."""

    def __init__(self, *a, **k):
        raise NotImplementedError(
            "Async inference is not supported for the block-AR engine "
            "(optimization.async_inference.mode must be 'none')."
        )
