...
class BaseInferenceEngine(ABC):
    ...
    def reset(self) -> None:
        """Clear per-episode engine state at an episode boundary.

        No-op by default — stateless engines (e.g. :class:`JointInferenceEngine`,
        :class:`MockInferenceEngine`) need nothing here. Stateful engines such as
        the block-AR :class:`ARInferenceEngine`, which carry a KV cache across
        ``generate`` calls, override this. Propagated from ``WAMPolicy.reset``.
        """
        return None
