"""Action-space conversions between sana-wam EEF output and downstream robot APIs.

The sana-wam policy server returns 20-dim EEF actions when trained with
``action_mode=eef``::
...
These helpers are pure numpy and have no dependency on the ``sana_wam`` package,
...
def eef20d_to_ee16d(action: np.ndarray) -> np.ndarray:
    ...
    sana-wam EEF (20D): ``[l_xyz(3), l_rot6d(6), l_grip(1), r_xyz(3), r_rot6d(6), r_grip(1)]``
...
def robotwin_endpose_to_eef20d(
    ...
    """Assemble 20D sana-wam EEF proprio from RoboTwin endpose fields.
