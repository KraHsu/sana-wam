#!/usr/bin/env python3
"""One-shot immutable runner for the non-formal CACH-A4-R1 screen.

R1 is a launcher-only environment repair.  Its standard-library-only pre-root
path verifies the frozen A3 scientific predecessor, the frozen failed A4
attempt, the unchanged original A4 source six, the dynamically bound R1 source
six, an explicit fresh root/nonce, and one idle physical GPU index/UUID before
importing the R1 bridge.  That bridge delegates the scientific screen to the
frozen A4 bridge after requiring the eager-helper environment guards.  Every
created root terminates with immutable evidence, including failure
placeholders; no token/claim, checkpoint, real-data, AV2, formal, or automatic
rerun path is implemented.
"""
...
EXPECTED_DECISION = Path(
    "docs/cach_sana_wam/architecture_validation/cach_a4_r1/"
    "CACH_A4_R1_LAUNCHER_ENV_FIX_DECISION.md"
)
EXPECTED_CARD = Path(
    "docs/cach_sana_wam/architecture_validation/cach_a4_r1/CACH_A4_R1_RUN_CARD.json"
)
EXPECTED_BRIDGE = Path(
    "src/sana_wam/model/cach_av1b_a4_state_conditioned_causal_odd_stream_r1.py"
)
...
EXPECTED_TEST = Path(
    "tests/test_cach_av1b_a4_state_conditioned_causal_odd_stream_r1.py"
)
...
AUTHORITY_SCHEMA = "cach.cach_a4_r1.runtime_execution_authority.v1"
AUTHORITY_OPERATION = "EXECUTE_CACH_A4_R1_ONCE"
USER_AUTHORITY_STATEMENT = "继续 A4"
USER_AUTHORITY_SHA256 = (
    "c3631ea159561fb7d13154c592e16e019384f111b798b3380c7bcf4a4e94350f"
)
EXPECTED_NONCE = "9e604d41d671ac4e705816857457e5a4"
EXPECTED_ROOT = EXPECTED_NAMESPACE_LEAF / (
    "cach-a4-state-stream-r1-9e604d41d671ac4e705816857457e5a4"
)
...
EXPECTED_EXACT_COMMAND = (
    ...
    "cach_a4_state_stream_r1/06f5d09127f8/"
    "cach-a4-state-stream-r1-9e604d41d671ac4e705816857457e5a4 "
    "--nonce 9e604d41d671ac4e705816857457e5a4 --gpu-index 0 "
...
CARD_SCHEMA = "cach.architecture_validation.cach_a4_r1_launcher_env_fix_run_card.v1"
...
class RootLifecycle:
    ...
    def freeze(self, terminal_state: str, result_sha256: str) -> None:
        ...
        receipt = {
            "schema": "cach.cach_a4_r1.freeze_receipt.v1",
            "root": str(self.root),
            "nonce": self.nonce,
...
def _validate_precreate(
    ...
    if (
        USER_AUTHORITY_STATEMENT != "继续 A4"
        or USER_AUTHORITY_SHA256
        != "c3631ea159561fb7d13154c592e16e019384f111b798b3380c7bcf4a4e94350f"
        or _sha_bytes(USER_AUTHORITY_STATEMENT.encode("utf-8")) != USER_AUTHORITY_SHA256
    ):
        _fail("AUTH_BLOCKED: exact fresh CACH-A4-R1 authority drifted")
    ...
    if Path(args.card) != EXPECTED_CARD or Path(args.config) != EXPECTED_CONFIG:
        _fail("card/config path differs from the A4-R1 contract")
    ...
    expected_root = EXPECTED_NAMESPACE_LEAF / f"cach-a4-state-stream-r1-{args.nonce}"
    ...
    if EXPECTED_NAMESPACE.exists() or EXPECTED_NAMESPACE_LEAF.exists():
        _fail("A4-R1 namespace already exists; this card is single execution only")
    ...
    if (
        ...
        _fail("R1 config launcher environment drifted")
    base_config = _strict_json_file(
        REPO_ROOT
        / "configs/experiments/cach_av1b_a4_state_conditioned_causal_odd_stream.yaml"
    )
    mechanically_reduced_config = json.loads(json.dumps(config))
    del mechanically_reduced_config["runtime"][
        "launcher_environment_before_torch_or_vendor_import"
    ]
    if mechanically_reduced_config != base_config:
        _fail("R1 config differs from frozen A4 beyond the launcher environment")
    if (
        card.get("state")
        != "ONE_IDLE_SINGLE_GPU_FRESH_ROOT_SYNTHETIC_EXECUTION_AUTHORIZED"
    ):
        _fail("card is not in the one-shot authorized A4-R1 state")
    ...
    if (
        ...
        or fresh_authority.get("exact_utf8_length_bytes") != 9
        or fresh_authority.get("satisfied") is not True
        or fresh_authority.get("scope") != "THIS_CACH_A4_R1_ONE_SHOT_BINDING_ONLY"
    ...
    if authority["source_sha256"] != source_hashes:
        _fail("materialized R1 six-file SHA binding mismatch")
...
def _timeout(_signum: int, _frame: object) -> NoReturn:
    raise TimeoutError("A4-R1 wall-time budget exceeded")


def _interrupted(signum: int, _frame: object) -> NoReturn:
    raise RuntimeError(f"A4-R1 interrupted by signal {signum}")
...
def _validate_screen(
    ...
    if (
        ...
        raise RuntimeError("A4-R1 launcher repair or vendor JIT evidence drifted")
...
def _failure_placeholder(
    ...
    return {
        "schema": "cach.cach_a4_r1.failure_placeholder.v1",
        "artifact": name,
        "available": False,
...
def _publish_failure(
    ...
    failure = {
        "schema": "cach.cach_a4_r1.failure_result.v1",
...
def main() -> int:
    ...
    run_context: dict[str, Any] = {
        "schema": "cach.cach_a4_r1.run_context.v1",
        "root": args.root,
        "nonce": args.nonce,
        ...
        "launcher_only_repair": {
            "gdn_disable_compile": "1",
            "torchdynamo_disable": "1",
            "scientific_screen_delegated_unchanged": True,
        },
    ...
    try:
        ...
        from sana_wam.model.cach_av1b_a4_state_conditioned_causal_odd_stream_r1 import (
            run_cach_a4_screen,
        )
        ...
        artifact_values: dict[str, object] = {
            ...
            "RAW_EVIDENCE.json": {
                "schema": "cach.cach_a4_r1.raw_evidence.v1",
        ...
        result = {
            "schema": "cach.cach_a4_r1.direct_final_result.v1",
            "root": args.root,
            "nonce": args.nonce,
