...
R4_ROOT = Path(
    ...
    "libero-ar-formal2000-r4-single-gpu-2000-"
    "9c7e8f1a64b24dc38ef0a9d6c27b5314"
)
R4_RESULT_SHA256 = "87448a3310075aac461ae279540501a75eda594fe47a5ec17296215e37ce1740"
R4_CHECKPOINT_SHA256 = (
    "324070da04f9d2d8543b550c11b40f5d94e9c57195166c963d97c13832ec400f"
)
DIAGNOSTIC_RESULT = Path(
    "/DATA/share/sana_wam_libero_diagnostics/aligned_action20/"
    "libero-spatial-t3-i0-4f2d1a7c9e8b43f6a105c2d7e9b83064/RESULT.json"
)
...
def _verify_predecessors() -> dict[str, Any]:
    result = _verify_file(R4_ROOT / "RESULT.json", R4_RESULT_SHA256)
    checkpoint = _verify_file(
        R4_ROOT / "checkpoint_step_2000.safetensors", R4_CHECKPOINT_SHA256
    )
    payload = json.loads((R4_ROOT / "RESULT.json").read_text(encoding="utf-8"))
    if (
        payload.get("valid_run") is not True
        or payload.get("execution_result") != "PASS"
        or payload.get("checkpoint", {}).get("sha256") != R4_CHECKPOINT_SHA256
    ):
    ...
    if (
        ...
        or diagnostic_payload.get("verdict")
...
def _prepare_root_parent() -> None:
    ...
    namespace_paths = (
        ...
        Path("/DATA/share/sana_wam_libero_training/formal_epoch1_r5"),
    )
    ...
    try:
        with _supervisor_signal_guard():
            ...
            attempt_sha256 = _exclusive_json(
                RUN_ROOT / "ATTEMPT.json",
                {
                    ...
                    "training": {
                        ...
                        "warm_start_checkpoint_sha256": R4_CHECKPOINT_SHA256,
            ...
            result = {
                ...
                "valid_run": True,
                "verdict": "LIBERO_AR_R5_8GPU_EPOCH1_TRAINING_COMPLETE",
                "execution_result": "PASS",
                ...
                "training": {
                    **summary,
                    ...
                    "warm_start_checkpoint_sha256": R4_CHECKPOINT_SHA256,
                    "optimizer_and_lr_state": "fresh",
                    "r4_low_noise_optimizer_steps": 2000,
