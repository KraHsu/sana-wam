#!/usr/bin/env python
"""Fail-closed 8-GPU R8 task-balanced continuous two-round successor."""
...
CONFIG_RELATIVE_PATH = (
    "configs/experiments/libero_formal_r8_taskbalanced_8gpu_continuous2.yaml"
)
BASELINE_CONFIG_RELATIVE_PATH = (
    "configs/experiments/libero_formal_r8_taskbalanced_8gpu_successor1.yaml"
)
RUN_NONCE = "dc83dfc0e8ffdeb1bcd4422713b52e56"
RUN_ROOT = Path(
    "/DATA/share/sana_wam_libero_training/"
    "formal_taskbalanced_r8_h8_ynorm_fp32attn_continuous2/"
    "libero-ar-r8-taskbalanced-continuous2-causal1-warmstart-8gpu-8680-" + RUN_NONCE
)
...
EXPECTED_TASKS = 40
EXPECTED_BALANCED_ROUNDS = 2
EXPECTED_ROUND_DRAWS_PER_TASK = 868
EXPECTED_TOTAL_DRAWS_PER_TASK = 1_736
EXPECTED_ROUND_WINDOW_DRAWS = 34_720
EXPECTED_TOTAL_WINDOW_DRAWS = 69_440
EXPECTED_ROUND_DRAWS_PER_RANK = 4_340
EXPECTED_TOTAL_DRAWS_PER_RANK = 8_680
EXPECTED_FINAL_STEP = 8_680
...
WORKER_MARKER = "SANA_WAM_R8_TASKBALANCED_CONTINUOUS2_SUPERVISED_WORKER"
...
def _load_and_verify_config():
    ...
    exact = {
        ...
        "training.balanced_rounds_in_this_run": EXPECTED_BALANCED_ROUNDS,
    ...
    if candidate != baseline:
        raise RuntimeError(
            "continuous-two-round config differs from the one-round task-balanced "
            "config outside output root, step/schedule horizon, and round count"
        )
...
def _verify_source(args: argparse.Namespace) -> dict[str, Any]:
    ...
    config = _verify_file(ROOT / CONFIG_RELATIVE_PATH, args.expected_config_sha256)
    return {
        "repo_commit": commit,
...
def _validate_sampler_summary(summary: dict[str, Any]) -> None:
    expected = {
        ...
        "draws_per_task": EXPECTED_ROUND_DRAWS_PER_TASK,
        "global_draw_count": EXPECTED_ROUND_WINDOW_DRAWS,
        "draws_per_rank": EXPECTED_ROUND_DRAWS_PER_RANK,
        "num_replicas": 8,
...
def _continuous_two_round_contract() -> dict[str, Any]:
    if (
        EXPECTED_FINAL_STEP != EXPECTED_BALANCED_ROUNDS * EXPECTED_ROUND_DRAWS_PER_RANK
        or EXPECTED_TOTAL_DRAWS_PER_RANK != EXPECTED_FINAL_STEP
        or EXPECTED_TOTAL_WINDOW_DRAWS != EXPECTED_FINAL_STEP * 8
        or EXPECTED_TOTAL_DRAWS_PER_TASK
        != EXPECTED_BALANCED_ROUNDS * EXPECTED_ROUND_DRAWS_PER_TASK
    ):
        raise RuntimeError("continuous two-round arithmetic contract differs")
    return {
        "schema_version": "sana-wam-libero-continuous-two-round-contract-v1",
        "balanced_rounds": EXPECTED_BALANCED_ROUNDS,
        "sampler_epoch_indices": [0, 1],
        "draws_per_task_per_round": EXPECTED_ROUND_DRAWS_PER_TASK,
        "draws_per_task_total": EXPECTED_TOTAL_DRAWS_PER_TASK,
        "global_draws_per_round": EXPECTED_ROUND_WINDOW_DRAWS,
        "global_draws_total": EXPECTED_TOTAL_WINDOW_DRAWS,
        "draws_per_rank_per_round": EXPECTED_ROUND_DRAWS_PER_RANK,
        "draws_per_rank_total": EXPECTED_TOTAL_DRAWS_PER_RANK,
        "optimizer": "single_persistent_fp32_master_adamw",
        "optimizer_instance_count": 1,
        "fp32_master_set_count": 1,
        "optimizer_state_reset_between_rounds": False,
        "lr_schedule": "single_cosine_with_one_warmup",
        "lr_schedule_steps": EXPECTED_FINAL_STEP,
        "warmup_steps": 50,
        "lr_restart_between_rounds": False,
...
def _worker(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if summary.get("window_draws") != EXPECTED_TOTAL_WINDOW_DRAWS:
        ...
        if rank == 0:
            _exclusive_json(
                ...
                {
                    "schema_version": "sana-wam-libero-taskbalanced-continuous2-worker-v1",
                    "training": summary,
                },
            )
        dist.barrier()
...
def _supervise(args: argparse.Namespace) -> None:
    ...
    attempt = {
        "schema_version": "sana-wam-libero-taskbalanced-continuous2-attempt-v1",
        "run": {"root": str(RUN_ROOT), "nonce": RUN_NONCE},
        "source": source,
        "predecessor": predecessor,
        "continuous_training_contract": _continuous_two_round_contract(),
        "sampler_contract": {
            "contract": "task_balanced_40_v1",
    ...
    try:
        ...
        result = {
            "schema_version": "sana-wam-libero-r8-taskbalanced-continuous2-result-v1",
            "valid_run": True,
            "execution_result": "PASS",
            "verdict": "LIBERO_AR_R8_TASKBALANCED_CONTINUOUS2_H8_8GPU_TRAINING_COMPLETE",
            "run": {"root": str(RUN_ROOT), "nonce": RUN_NONCE},
            ...
            "training": {
                **summary,
                "balanced_rounds_in_this_run": EXPECTED_BALANCED_ROUNDS,
                "full_dataset_epoch_claimed": False,
            },
            "checkpoint": {
        ...
        if not os.path.lexists(RUN_ROOT / "RESULT.json"):
            _exclusive_json(
                ...
                {
                    "schema_version": "sana-wam-libero-taskbalanced-continuous2-failure-v1",
                    "valid_run": False,
