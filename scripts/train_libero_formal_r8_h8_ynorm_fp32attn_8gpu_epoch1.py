#!/usr/bin/env python
"""Fail-closed 8-GPU supervisor/worker for the R8 FP32-AR successor."""
...
CONFIG_RELATIVE_PATH = "configs/experiments/libero_formal_r8_h8_ynorm_fp32attn_8gpu_epoch1.yaml"
RUN_NONCE = "01b7c080232706dddcdbe20bc0fbcdf5"
RUN_ROOT = Path(
...
R7_FP32_AR_SOURCE_SHA256 = {
    "src/sana_wam/model/ar/sana_ar_linear_attn.py": (
        "3ed9e26bc13a2ac8df706caf1f76ad92655e72a865400ffbe21d7f5392270b28"
    ),
    "src/sana_wam/model/ar/sana_ar_inference.py": (
        "d5bbbe56f6f0d3fe0a9f902245a8b7864171f6b6ec5b7b2710d005a08e512f9d"
    ),
    "src/sana_wam/model/ar/sana_ar_mot_driver.py": (
        "16465d414e4e216df236a970fcacf355d216ed1474f23aa9e99995d6cf5ef34f"
    ),
}
...
def _verify_source(
    ...
    if Path.cwd().resolve() != ROOT.resolve():
        raise RuntimeError("R8 must run from the repository root")
    ...
    for relative_path in FP32_AR_SOURCE_PATHS:
        ...
        if artifact["sha256"] == R7_FP32_AR_SOURCE_SHA256[relative_path]:
            raise RuntimeError(
                f"R8 FP32-AR source did not change from R7: {relative_path}"
            )
        fp32_ar_sources[relative_path] = {
            **artifact,
            "r7_sha256": R7_FP32_AR_SOURCE_SHA256[relative_path],
        }
    return {
        "repo_commit": commit,
...
def _verify_config():
    ...
    exact = {
        ...
        "training.init_checkpoint": str(PREDECESSOR_R7_ROOT / "checkpoint_step_4337.safetensors"),
        "training.init_checkpoint_sha256": PREDECESSOR_R7_CHECKPOINT_SHA256,
        "training.formal_predecessor_r7_result_sha256": PREDECESSOR_R7_RESULT_SHA256,
        "training.formal_predecessor_r7_checkpoint_sha256": PREDECESSOR_R7_CHECKPOINT_SHA256,
        "training.scientific_predecessor_r7_formal40_result_sha256": FORMAL40_RESULT_SHA256,
    ...
    for path, expected in exact.items():
        ...
        if observed != expected:
            raise RuntimeError(f"R8 config requires {path}={expected!r}, got {observed!r}")
    if OmegaConf.select(
        cfg, "training.init_checkpoint_allow_missing_patterns", default=None
    ) is not None:
        raise RuntimeError("R8 requires an exact R7 checkpoint schema with no allowlist")
    if OmegaConf.select(cfg, "model.video_backbone.fp32_attention", default=False):
        raise RuntimeError(
            "R8 FP32-AR treatment is source-level; the legacy config knob must stay off"
        )
    candidate = OmegaConf.to_container(cfg, resolve=True)
    baseline = OmegaConf.to_container(
        OmegaConf.load(
            ROOT / "configs/experiments/libero_formal_r7_h8_ynorm_8gpu_epoch1.yaml"
        ),
        resolve=True,
    )
    ...
    if candidate != baseline:
        raise RuntimeError(
            "R8 config differs from R7 outside root and frozen predecessor lineage"
        )
    ...
    if contract.get("action_steps") != 20 or contract.get("history_len") != 17:
        raise RuntimeError("R8 deployment parity contract differs")
...
def _verify_predecessors() -> dict[str, Any]:
    result = _verify_file(
        PREDECESSOR_R7_ROOT / "RESULT.json", PREDECESSOR_R7_RESULT_SHA256
    )
    checkpoint = _verify_file(
        PREDECESSOR_R7_ROOT / "checkpoint_step_4337.safetensors",
        PREDECESSOR_R7_CHECKPOINT_SHA256,
    )
    payload = json.loads(
        (PREDECESSOR_R7_ROOT / "RESULT.json").read_text(encoding="utf-8")
    )
    if (
        payload.get("valid_run") is not True
        or payload.get("execution_result") != "PASS"
        or payload.get("verdict")
        != "LIBERO_AR_R7_H8_YNORM_8GPU_EPOCH1_TRAINING_COMPLETE"
        or payload.get("checkpoint", {}).get("sha256")
        != PREDECESSOR_R7_CHECKPOINT_SHA256
    ):
        raise RuntimeError("R7/H8/y_norm predecessor RESULT semantics differ")
    ...
    if (
        ...
        raise RuntimeError(
            "R7/H8/y_norm formal40 predecessor semantics differ from 4/40"
        )
    return {
        "r7_h8_ynorm_epoch1_result": result,
        "r7_h8_ynorm_epoch1_checkpoint": checkpoint,
        "scientific_r7_h8_ynorm_formal40": formal40,
    }


def _verify_materialized_r7_warm_start(trainer: Any) -> dict[str, Any]:
    """Prove R8 loaded the complete R7 checkpoint with an exact schema."""
    import torch
    from safetensors import safe_open

    checkpoint_path = PREDECESSOR_R7_ROOT / "checkpoint_step_4337.safetensors"
    ...
    if missing or unexpected:
        raise RuntimeError(
            "R8 strict R7 warm-start schema differs: "
    ...
    if weight is None or weight.numel() != 2_240:
        raise RuntimeError("R8 inherited text RMSNorm parameter is absent or malformed")
    if weight.requires_grad:
        raise RuntimeError("R8 inherited text RMSNorm must remain frozen")
    detached = weight.detach()
    if detached.is_meta or not bool(torch.isfinite(detached).all().item()):
        raise RuntimeError("R8 inherited text RMSNorm is non-materialized/non-finite")
    parameter_sha256 = _verify_tensor_sha256(
        detached,
        YNORM_BF16_SHA256,
        label="materialized R8 inherited BF16 attention_y_norm.weight",
    )
    return {
        "strict_load": True,
        ...
        "r7_checkpoint_loaded_exactly": True,
        "source_checkpoint_sha256": PREDECESSOR_R7_CHECKPOINT_SHA256,
...
def _verify_materialized_fp32_ar_operator(trainer: Any) -> dict[str, Any]:
    """Bind the materialized trainer to the R8 FP32 train/cache operator."""
    import torch
    ...
    if not isinstance(driver, SanaARMoTJointDriver):
        raise RuntimeError("R8 trainer did not materialize SanaARMoTJointDriver")
    if observed_eps != AR_LINEAR_ATTN_EPS or driver.eps != AR_LINEAR_ATTN_EPS:
        raise RuntimeError(
            "R8 train/cache epsilon differs: "
    ...
    if dtype_map != expected_dtype_map:
        raise RuntimeError(f"R8 FP32-AR compute dtype map differs: {dtype_map}")
    return {
        "enabled": True,
...
def _verify_saved_ynorm_checkpoint(path: Path, expected_tensor_sha256: str) -> dict[str, Any]:
    ...
    with safe_open(str(path), framework="pt", device="cpu") as checkpoint:
        if YNORM_WEIGHT_KEY not in checkpoint.keys():
            raise RuntimeError("R8 checkpoint omitted the restored text RMSNorm")
        weight = checkpoint.get_tensor(YNORM_WEIGHT_KEY)
    if weight.numel() != 2_240 or not bool(torch.isfinite(weight).all().item()):
        raise RuntimeError("R8 checkpoint text RMSNorm is malformed or non-finite")
    raw = weight.contiguous().view(torch.uint8).numpy().tobytes()
    observed = hashlib.sha256(raw).hexdigest()
    if observed != expected_tensor_sha256:
        raise RuntimeError("R8 checkpoint changed the frozen pretrained text RMSNorm")
    return {
        "name": YNORM_WEIGHT_KEY,
        "present": True,
        "numel": weight.numel(),
...
def _prepare_root_parent() -> None:
    ...
    namespace_paths = (
        ...
        Path("/DATA/share/sana_wam_libero_training/formal_epoch1_r8_h8_ynorm_fp32attn"),
    )
    ...
    if RUN_ROOT.parent != namespace_paths[-1]:
        raise RuntimeError("R8 root parent differs from its declared namespace")
    if os.path.lexists(RUN_ROOT):
        raise FileExistsError(f"R8 root already exists: {RUN_ROOT}")
...
def _freeze_root_contents() -> None:
    ...
    if stat.S_IMODE(os.lstat(RUN_ROOT).st_mode) != 0o700:
        raise RuntimeError("R8 root must remain mode 0700 before terminal publication")
...
def _verify_frozen_contents(*, expected_root_mode: int) -> None:
    ...
    if (
        ...
        raise RuntimeError(
            f"R8 root mode differs from {expected_root_mode:04o} during verification"
        )
    for directory, subdirs, files in os.walk(RUN_ROOT, topdown=False):
        base = Path(directory)
        for name in files:
            ...
            if (
                ...
                raise RuntimeError(f"R8 file is not frozen to 0444: {child}")
        for name in subdirs:
            child = base / name
            metadata = os.lstat(child)
            if (
                ...
                raise RuntimeError(f"R8 directory is not frozen to 0555: {child}")
...
def _seal_root(expected_terminal: str) -> None:
    ...
    if not stat.S_ISDIR(metadata.st_mode) or stat.S_IMODE(metadata.st_mode) != 0o555:
        raise RuntimeError("R8 terminal root did not freeze to mode 0555")
    ...
    if (
        ...
        raise RuntimeError(f"R8 terminal artifact is not frozen: {terminal}")
...
def _publish_success_result(payload: Any) -> str:
    if os.path.lexists(RUN_ROOT / "RESULT.json") or os.path.lexists(
        ...
        raise FileExistsError("R8 terminal artifact already exists")
...
def _discard_uncommitted_result() -> None:
    ...
    if not stat.S_ISREG(metadata.st_mode) or stat.S_ISLNK(metadata.st_mode):
        raise RuntimeError("uncommitted R8 RESULT is unsafe")
...
def _supervisor_signal_guard():
    ...
    def _raise_interrupted(signum, _frame):
        raise InterruptedError(f"R8 supervisor received signal {signum}")
...
def _worker(args: argparse.Namespace) -> None:
    if os.environ.get("SANA_WAM_R8_H8_YNORM_FP32ATTN_SUPERVISED_WORKER") != "1":
        raise RuntimeError("R8 worker may only be launched by its supervisor")
    world_size = int(os.environ.get("WORLD_SIZE", "0"))
    rank = int(os.environ.get("RANK", "-1"))
    local_rank = int(os.environ.get("LOCAL_RANK", "-1"))
    if world_size != 8 or rank not in range(8) or local_rank not in range(8):
        raise RuntimeError("R8 worker rank environment differs")
    logging.basicConfig(
        level=logging.INFO,
        format=(
    ...
    worker_logger = logging.getLogger("sana_wam.r8_ynorm_fp32attn.worker")
    ...
    try:
        worker_logger.info("NCCL process group initialized; beginning R8 preflight")
        ...
        def _fail_if_any_rank(stage: str, local_error: str | None) -> None:
            ...
            if failures:
                raise RuntimeError(
                    f"R8 {stage} failed on one or more ranks: "
        ...
        if cfg is None:
            raise RuntimeError("R8 config missing after successful collective preflight")
        ...
        try:
            if len(trainer.dataset) != EXPECTED_DATASET_WINDOWS:
                raise RuntimeError("R8 dataset window count differs")
            trainables = [
                parameter
            ...
            if (
                ...
                raise RuntimeError("R8 trainable parameter partition differs")
            warm_start_integrity = _verify_materialized_r7_warm_start(trainer)
        ...
        worker_logger.info("R8 preflight complete; entering formal training")
        ...
        try:
            ...
            if warm_start_integrity is None or fp32_ar_operator is None:
                raise RuntimeError("R8 warm-start/operator proof is absent")
            post_training_warm_start = _verify_materialized_r7_warm_start(trainer)
            ...
            if (
                ...
                raise RuntimeError("R8 frozen inherited text RMSNorm changed during training")
            if post_training_operator != fp32_ar_operator:
                raise RuntimeError("R8 FP32-AR operator contract changed during training")
            summary["warm_start_integrity"] = {
                **warm_start_integrity,
                "post_training_parameter_sha256": post_training_warm_start[
                    "parameter"
                ]["sha256"],
        ...
        if summary is None:
            raise RuntimeError("R8 summary missing after successful collective preflight")
        ...
        if rank == 0:
            try:
                ...
                if len(set(canonical)) != 1:
                    raise RuntimeError("R8 rank summaries differ")
                worker_result_sha256 = _exclusive_json(
                    RUN_ROOT / "WORKER_RESULT.json",
                    {
                        "schema_version": "sana-wam-libero-r8-h8-ynorm-fp32attn-worker-result-v1",
        ...
        dist.barrier()
        worker_logger.info("R8 worker completed with rank-consistent terminal summary")
    finally:
    ...
    try:
        with _supervisor_signal_guard():
            ...
            attempt_sha256 = _exclusive_json(
                ...
                {
                    "schema_version": "sana-wam-libero-r8-h8-ynorm-fp32attn-attempt-v1",
                    ...
                    "training": {
                        ...
                        "warm_start_checkpoint_sha256": PREDECESSOR_R7_CHECKPOINT_SHA256,
                        "init_checkpoint_allow_missing_patterns": [],
                        "strict_r7_checkpoint_schema": True,
                    ...
                    "treatment": {
                        "name": "fp32_ar_attention_operator",
                        "source_level_unconditional": True,
                        "legacy_config_knob_enabled": False,
                        "epsilon": AR_LINEAR_ATTN_EPS,
            ...
            environment.update(
                {
                    ...
                    "PYTHONUNBUFFERED": "1",
                    "SANA_WAM_R8_H8_YNORM_FP32ATTN_SUPERVISED_WORKER": "1",
            ...
            if (
                warm_start_integrity.get("r7_checkpoint_loaded_exactly") is not True
                ...
                raise RuntimeError("R8 worker strict R7 warm-start proof differs")
            ...
            if (
                ...
                raise RuntimeError("R8 worker FP32-AR operator proof differs")
            saved_ynorm = _verify_saved_ynorm_checkpoint(
                checkpoint_path,
                warm_start_integrity.get("parameter", {}).get("sha256", ""),
            )
            result = {
                "schema_version": "sana-wam-libero-r8-h8-ynorm-fp32attn-result-v1",
                "valid_run": True,
                "verdict": "LIBERO_AR_R8_H8_YNORM_FP32ATTN_8GPU_EPOCH1_TRAINING_COMPLETE",
                "execution_result": "PASS",
                ...
                "training": {
                    **summary,
                    ...
                    "predecessor_r7_h8_ynorm_optimizer_steps": EXPECTED_FINAL_STEP,
                    "r8_h8_ynorm_fp32attn_uniform_optimizer_steps": EXPECTED_FINAL_STEP,
                ...
                "scientific_scope": {
                    "establishes": (
                        "one R7-warm-start H8 epoch under the shared FP32 AR "
                        "training/cache attention operator"
                    ),
                    "does_not_establish": (
                        "closed-loop policy improvement or value-distribution equivalence"
                    ),
                    "single_axis_vs_r7": "fp32_ar_attention_operator_source",
        ...
        if root_created and not success_committed:
            ...
            if os.path.lexists(RUN_ROOT / "FAILED.json"):
                raise RuntimeError("R8 FAILED artifact already exists") from error
            _publish_failure(
                {
                    "schema_version": "sana-wam-libero-r8-h8-ynorm-fp32attn-failure-v1",
                    "valid_run": False,
