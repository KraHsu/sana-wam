#!/usr/bin/env python3
"""Build or verify the byte-scoped CACH Stage-2B L3 source checkpoint.

The verifier authenticates one additive, zero-collision source archive over the
caller-pinned L2B checkpoint.  It never imports the L3 implementation, replays
...
DEFAULT_MANIFEST = (
    ROOT / "docs/cach_sana_wam/stage2b_l3/SOURCE_MANIFEST.json"
)
...
L2B_MANIFEST_PIN = {
    "path": "docs/cach_sana_wam/stage2b_l2b/SOURCE_MANIFEST.json",
    "sha256": "5e577cc857de232529ea8737321643ba07a9366d2fba0c519c322da722b20487",
}
L2B_VERIFIER_PIN = {
    "path": "scripts/verify_cach_stage2b_l2b.py",
    "sha256": "6fb42de438057a1fb6fe9900872d1ba3158a009aaa00126123d3dd66a15f8ce3",
}
L2B_BUNDLE_PIN = {
    "mode": "0444",
    "path": (
        "/DATA/share/sana_cach_source_bundles/"
        "cach_stage2b_l2b_f2380eabfe06daeb7087dc139cd67441e2164b27f91bc71ff6295b7fc8e43533_"
        "20260731.tar"
    ),
    "sha256": "f2380eabfe06daeb7087dc139cd67441e2164b27f91bc71ff6295b7fc8e43533",
    "size_bytes": 194_560,
}
L2B_OUTER_OVERLAY_PIN = {
    "order": [
        "stage0",
        "stage1",
        "stage2",
...
EXPECTED_BASE_LINEAGE = {
    ...
    "stage2b_l2b_source_bundle": L2B_BUNDLE_PIN,
    "stage2b_l2b_source_manifest": L2B_MANIFEST_PIN,
    "stage2b_l2b_verifier": L2B_VERIFIER_PIN,
}
...
SOURCE_FILES = (
    "docs/cach_sana_wam/stage2b_l3/README.md",
    "docs/cach_sana_wam/stage2b_l3/STAGE2B_L3_LIGHTWEIGHT_TEST_REPORT.json",
    "docs/cach_sana_wam/stage2b_l3/STAGE2B_L3_SOURCE_FILES.txt",
    (
        "docs/cach_sana_wam/stage2b_l3/"
        "STAGE2B_L3_DETERMINISTIC_RECOVERY_IMPLEMENTATION_PLAN_20260731.md"
    ),
    "scripts/verify_cach_stage2b_l3.py",
    (
        "src/sana_wam/model/video_backbone/sana/"
        "hybrid_cache_stage2b_recovery.py"
    ),
...
FROZEN_L3_CORE_PINS = {
    (
        "docs/cach_sana_wam/stage2b_l3/"
        "STAGE2B_L3_DETERMINISTIC_RECOVERY_IMPLEMENTATION_PLAN_20260731.md"
    ): "bfbeab7df26dcc85ce2d367fc23c46d756c9e4421d53967babf3609fb5e31153",
    (
        "src/sana_wam/model/video_backbone/sana/"
        "hybrid_cache_stage2b_recovery.py"
    ): "00d6614a433b3acd7d4fec555eb84e2db292bec6abc30becae873c756d484512",
    "tests/test_cach_stage2b_recovery.py": (
        "1629f857456035cfbeb1908ac379bc236fd64ab928fd9b411be5e3b319559230"
    ),
}
...
EXPECTED_SCOPE = {
    "execution_scope": "cpu_synthetic_single_process_stage2b_l3_deterministic_recovery",
    "file_count": 7,
    "kind": "stage2b_l3_zero_collision_delta_over_verified_stage2b_l2b",
...
MANIFEST_KEYS = {
    ...
    "inherited_source_pins",
    "overlay",
    "retained_stage2b_l3_evidence",
    "schema",
    "scope",
    "stage2b_l3_source_bundle",
    "status",
}
...
EXPECTED_REPORT_ARTIFACT_STATE = {
    ...
    "stage2b_l3_source_bundle_created": False,
}
EXPECTED_REPORT_AUTHORITY = {
    ...
    "l3_production_recovery_authorized": False,
...
EXPECTED_REPORT_PROHIBITED_ACTIONS = {
    ...
    "production_recovery_started": False,
    "training_started": False,
}
EXPECTED_REPORT_LIMITATIONS = {
    "callable_scope": "cpu_synthetic_only_exact_reopened_ledger",
    "cuda_device_mapping": "not_admitted",
    "deterministic_recovery_implemented": True,
    "fd_bound_hostile_mutation_resistance": "not_admitted",
    "multi_process_writer_fencing": "not_admitted",
    "old_writer_death_proof": "not_admitted",
    "production_recovery": "not_admitted",
    "real_data_power_loss_semantics": "not_admitted",
...
EXPECTED_REPORT_PREDECESSOR_PINS = {
    "l2b_source_bundle_mode": L2B_BUNDLE_PIN["mode"],
    "l2b_source_bundle_path": L2B_BUNDLE_PIN["path"],
    "l2b_source_bundle_sha256": L2B_BUNDLE_PIN["sha256"],
    "l2b_source_bundle_size": L2B_BUNDLE_PIN["size_bytes"],
    "l2b_source_manifest_sha256": L2B_MANIFEST_PIN["sha256"],
    "l2b_verifier_sha256": L2B_VERIFIER_PIN["sha256"],
...
EXPECTED_REPORT_AUDITS = {
    ...
    "aggregate_open_p1": 0,
    "reviews": {
        "code": {"open_p0": 0, "open_p1": 0},
        "plan": {"open_p0": 0, "open_p1": 0},
        "test": {"open_p0": 0, "open_p1": 0},
    },
    "review_count": 3,
...
EXPECTED_TESTED_SOURCE_PINS = {
    path: digest
    for path, digest in FROZEN_L3_CORE_PINS.items()
    if path
    != (
        "docs/cach_sana_wam/stage2b_l3/"
        "STAGE2B_L3_DETERMINISTIC_RECOVERY_IMPLEMENTATION_PLAN_20260731.md"
...
EXPECTED_L2B_VERIFIER_OUTPUT = {
    ...
    "l3_authorized": False,
    "manifest_sha256": L2B_MANIFEST_PIN["sha256"],
...
class Stage2BL3VerificationError(RuntimeError):
    """Raised when any L3 source-checkpoint invariant differs."""


def _fail(message: str) -> None:
    raise Stage2BL3VerificationError(message)
...
def _strict_json(payload: bytes, label: str) -> Any:
    ...
    try:
        ...
        raise Stage2BL3VerificationError(f"{label} is invalid JSON") from exc
...
def _repo_path(raw: object, label: str) -> tuple[str, Path]:
    ...
    try:
        ...
        raise Stage2BL3VerificationError(f"{label} cannot be resolved") from exc
...
def _verify_core_pins(
    observed: object,
    source_payloads: dict[str, bytes],
) -> None:
    expected = _pin_entries(FROZEN_L3_CORE_PINS)
    if not _exact_equal(observed, expected):
        _fail("frozen L3 core-pin declaration differs")
    for entry in expected:
        payload = source_payloads.get(entry["path"])
        if payload is None:
            _fail(f"frozen L3 core member is absent: {entry['path']}")
        if _sha256(payload) != entry["sha256"]:
            _fail(f"frozen L3 core member differs: {entry['path']}")
...
def _run_l2b_verifier() -> dict[str, Any]:
    """Run the pinned predecessor verifier only as a byte-checking subprocess."""

    verifier_bytes = _verify_fixed_file_pin(L2B_VERIFIER_PIN, "L2B verifier")
    verifier_path = os.fspath(ROOT / L2B_VERIFIER_PIN["path"])
    manifest_path = os.fspath(ROOT / L2B_MANIFEST_PIN["path"])
    wrapper = (
        "import sys\n"
        "source = sys.stdin.buffer.read()\n"
        "script_path, manifest_path, manifest_sha256 = sys.argv[1:]\n"
        "sys.argv = [script_path, '--manifest', manifest_path, "
    ...
    if completed.returncode != 0 or completed.stderr.strip():
        _fail("fixed L2B predecessor verifier failed or emitted stderr")
    result = _strict_json(completed.stdout.strip(), "L2B verifier output")
    if not _exact_equal(result, EXPECTED_L2B_VERIFIER_OUTPUT):
        _fail("fixed L2B verifier output differs or grants authority")
    return result


def _load_l2b_manifest() -> dict[str, Any]:
    payload = _verify_fixed_file_pin(L2B_MANIFEST_PIN, "L2B outer manifest")
    value = _strict_json(payload, "L2B outer manifest")
    if (
        not isinstance(value, dict)
        or value.get("schema") != "cach.stage2b_l2b.source_manifest.v1"
        or not _exact_equal(value.get("stage2b_l2b_source_bundle"), L2B_BUNDLE_PIN)
    ):
        _fail("caller-pinned L2B outer manifest identity differs")
    return value


def _verify_l2b_predecessor() -> dict[str, Any]:
    manifest = _load_l2b_manifest()
    _verify_fixed_bundle(L2B_BUNDLE_PIN, "L2B source bundle")
    _run_l2b_verifier()
    return manifest
...
def _resolved_inventory_sha256(files: dict[str, str]) -> str:
    inventory = {
        "files": [
...
def _resolved_predecessor_inventory(
    ...
    for path, label in (
        ...
        if path == STAGE2_MANIFEST_PIN["path"]:
            raw = _verify_fixed_file_pin(STAGE2_MANIFEST_PIN, label)
        elif path == STAGE2B_MANIFEST_PIN["path"]:
            raw = _verify_fixed_file_pin(STAGE2B_MANIFEST_PIN, label)
        else:
            raw = _read_repo_file(path, label)
    ...
    l2a_raw = _verify_fixed_file_pin(L2A_MANIFEST_PIN, "L2A outer manifest")
    l2a_manifest = _strict_json(l2a_raw, "L2A outer manifest")
    if (
        not isinstance(l2a_manifest, dict)
        or l2a_manifest.get("schema") != "cach.stage2b_l2a.source_manifest.v1"
        or not _exact_equal(
            l2a_manifest.get("stage2b_l2a_source_bundle"), L2A_BUNDLE_PIN
        )
    ):
        _fail("fixed L2A manifest identity differs during inventory rebuild")
    ...
    if len(resolved) != 112:
        _fail("resolved post-L2A inventory does not contain exactly 112 paths")
    post_l2a_digest = _resolved_inventory_sha256(resolved)
    if (
        post_l2a_digest
        != L2B_OUTER_OVERLAY_PIN["predecessor_resolved_inventory_sha256"]
        or not _exact_equal(l2b_manifest.get("overlay"), L2B_OUTER_OVERLAY_PIN)
    ):
        _fail("post-L2A inventory or fixed L2B outer overlay differs")

    l2b_files = _manifest_file_map(l2b_manifest, "L2B outer manifest")
    l2b_bundle_files = _tar_regular_file_map(
        _verify_fixed_bundle(L2B_BUNDLE_PIN, "L2B source bundle"),
        "L2B source bundle",
    )
    if (
        len(l2b_files) != 7
        or resolved.keys() & l2b_files.keys()
        or not _exact_equal(l2b_bundle_files, l2b_files)
    ):
        _fail("L2B delta is not an exact seven-file zero-collision overlay")
    resolved.update(l2b_files)
    if len(resolved) != 119:
        _fail("resolved L2B predecessor inventory does not contain 119 paths")
    resolved_digest = _resolved_inventory_sha256(resolved)
    if resolved_digest != L2B_RESOLVED_INVENTORY_SHA256:
        _fail("resolved 119-path L2B inventory digest differs")
    return resolved, resolved_digest
...
def _verify_report(payload: bytes) -> None:
    report = _strict_json(payload, "L3 lightweight report")
    if not isinstance(report, dict) or set(report) != REPORT_KEYS:
        _fail("L3 lightweight report top-level schema differs")
    if (
        report["schema"] != "cach.stage2b_l3.live_lightweight_report.v1"
        or report["status"]
        != (
            "bounded_l3_lightweight_green_three_audits_no_p0_p1_"
            "not_production_recovery_filesystem_runtime_admitted_non_scientific"
        )
        ...
        or report["scope"]
        != (
            "deterministic_recovery_synthetic_cpu_single_process_"
            "exact_reopened_stage2b_ledger"
        )
        or report["retained_stage2b_l3_evidence"] is not None
    ):
        _fail("L3 lightweight report identity differs")
    for observed, expected, label in (
        ...
        if not _exact_equal(observed, expected):
            _fail(f"L3 lightweight report {label} differs")

    results = report["results"]
    if not isinstance(results, dict) or set(results) != {
        "focused_l2a_l2b_regression",
        "focused_l3_cpu_synthetic_recovery",
        "full_cach_cpu_regression",
        "py_compile",
        "ruff",
    }:
        _fail("L3 lightweight report result inventory differs")
    focused_predecessor = results["focused_l2a_l2b_regression"]
    predecessor_keys = {
        "command",
        "deselected",
        "duration_seconds",
    ...
    if (
        not isinstance(focused_predecessor, dict)
        or set(focused_predecessor) != predecessor_keys
    ):
        _fail("focused L2A+L2B result schema differs")
    _require_duration(
        focused_predecessor["duration_seconds"], "focused L2A+L2B duration"
    )
    predecessor_without_duration = dict(focused_predecessor)
    predecessor_without_duration.pop("duration_seconds")
    if not _exact_equal(
        predecessor_without_duration,
        {
            "command": (
        ...
        _fail("focused L2A+L2B result differs")

    focused = results["focused_l3_cpu_synthetic_recovery"]
    focused_keys = {
    ...
    if not isinstance(focused, dict) or set(focused) != focused_keys:
        _fail("focused L3 result schema differs")
    ...
    if not _exact_equal(
        ...
        _fail("focused L3 result differs")
    ...
    if not _exact_equal(results["py_compile"], expected_compile):
        _fail("L3 py_compile result differs")
    expected_ruff = {
        "command": (
            ".venv/bin/ruff check "
            "src/sana_wam/model/video_backbone/sana/"
    ...
    if not _exact_equal(results["ruff"], expected_ruff):
        _fail("L3 Ruff result differs")


def _source_bytes() -> tuple[dict[str, bytes], list[dict[str, object]]]:
    if len(SOURCE_FILES) != 7 or len(set(SOURCE_FILES)) != 7:
        _fail("internal L3 source inventory is not seven unique paths")
    payloads: dict[str, bytes] = {}
    entries: list[dict[str, object]] = []
    for path in SOURCE_FILES:
        payload = _read_repo_file(path, "L3 source member")
    ...
    if payloads[source_list_path] != expected_list:
        _fail("L3 source-list bytes differ from the exact ordered inventory")
    _verify_report(
        payloads[
            "docs/cach_sana_wam/stage2b_l3/"
            "STAGE2B_L3_LIGHTWEIGHT_TEST_REPORT.json"
        ]
    )
...
def _verify_tar_structure(payload: bytes, payloads: dict[str, bytes]) -> None:
    with tarfile.open(fileobj=io.BytesIO(payload), mode="r:") as archive:
        ...
        if [member.name for member in members] != list(SOURCE_FILES):
            _fail("L3 tar member order or inventory differs")
        for member in members:
            if (
                ...
                _fail(f"L3 tar metadata differs: {member.name}")
            extracted = archive.extractfile(member)
            if extracted is None or extracted.read() != payloads[member.name]:
                _fail(f"L3 tar member bytes differ: {member.name}")
            if member.size != len(payloads[member.name]):
                _fail(f"L3 tar member size differs: {member.name}")
...
def _publish_bundle(payload: bytes, directory: Path) -> dict[str, object]:
    ...
    filename = f"cach_stage2b_l3_{digest}_{CHECKPOINT_DATE}.tar"
    flags = os.O_RDONLY | os.O_DIRECTORY | os.O_NOFOLLOW
    directory_fd = os.open(directory, flags | getattr(os, "O_CLOEXEC", 0))
    temporary = f".tmp-cach-stage2b-l3-{secrets.token_hex(16)}"
    ...
    observed, metadata = _stable_read_regular(target, "L3 source bundle")
    if (
        observed != payload
        ...
        _fail("published L3 source bundle bytes or metadata differ")
    return {
        "mode": "0444",
...
def build_bundle(directory: Path) -> dict[str, object]:
    ...
    l2b_manifest = _verify_l2b_predecessor()
    ...
    _verify_core_pins(_pin_entries(FROZEN_L3_CORE_PINS), payloads)
    predecessor, inventory_digest = _resolved_predecessor_inventory(l2b_manifest)
    collisions = sorted(predecessor.keys() & set(SOURCE_FILES))
    if collisions or len(predecessor) != 119:
        _fail(f"L3 source inventory collides with predecessor: {collisions}")
    ...
    return {
        ...
        "l3_authorized": False,
        "l3_production_recovery_authorized": False,
        "l3_runtime_authorized": False,
        "l3_source_checkpoint_verified": False,
        "model_executed": False,
        ...
        "retained_immutable_execution_evidence": False,
        "runtime_closure_verified": False,
        "schema": "cach.stage2b_l3.bundle_build.v1",
        "scientific_eligible": False,
        "stage3_authorized": False,
        "training_authorized": False,
        "transitive_runtime_closure": False,
...
def _verify_manifest_files(
    ...
    if not _exact_equal(observed, entries):
        _fail("L3 source manifest file inventory differs")
    for entry in entries:
        ...
        if (
            ...
            _fail(f"L3 source member hash/size differs: {path}")
...
def _verify_bundle(pin: object, expected: bytes, payloads: dict[str, bytes]) -> None:
    if not isinstance(pin, dict) or set(pin) != {
        ...
        _fail("L3 source bundle pin schema differs")
    digest = _sha256(expected)
    expected_path = (
        CANONICAL_BUNDLE_DIRECTORY
        / f"cach_stage2b_l3_{digest}_{CHECKPOINT_DATE}.tar"
    )
    expected_pin = {
        "mode": "0444",
    ...
    if not _exact_equal(pin, expected_pin):
        _fail("L3 source bundle pin differs")
    observed, metadata = _stable_read_regular(expected_path, "L3 source bundle")
    if (
        observed != expected
        or _sha256(observed) != digest
        ...
        _fail("L3 source bundle bytes, hash, size, or mode differ")
...
def verify(manifest_path: Path, expected_manifest_sha256: str) -> dict[str, object]:
    ...
    if Path(os.path.abspath(os.fspath(manifest_path))) != DEFAULT_MANIFEST:
        _fail("L3 manifest must use the canonical default repository path")
    manifest_raw, _ = _stable_read_regular(manifest_path, "L3 outer manifest")
    manifest_sha256 = _sha256(manifest_raw)
    if manifest_sha256 != expected_manifest_sha256:
        _fail("L3 outer manifest differs from the caller trust anchor")
    manifest = _strict_json(manifest_raw, "L3 outer manifest")
    if not isinstance(manifest, dict) or set(manifest) != MANIFEST_KEYS:
        _fail("L3 outer manifest top-level schema differs")
    if (
        manifest["schema"] != "cach.stage2b_l3.source_manifest.v1"
        or manifest["status"]
        != "l3_source_checkpoint_verified_scope_not_admitted_non_scientific"
        ...
        _fail("L3 outer manifest identity differs")
    if not _exact_equal(manifest["base_lineage"], EXPECTED_BASE_LINEAGE):
        _fail("L3 predecessor lineage differs")
    if not _exact_equal(manifest["scope"], EXPECTED_SCOPE):
        _fail("L3 source scope differs")
    if not _exact_equal(
        manifest["git_recovery_bundles"], EXPECTED_GIT_RECOVERY_BUNDLES
    ):
        _fail("L3 git recovery-bundle declaration differs")
    if manifest["retained_stage2b_l3_evidence"] is not None:
        _fail("L3 source checkpoint must not claim retained execution evidence")
    _verify_pin_set(
    ...
    _verify_core_pins(manifest["frozen_l3_core_pins"], payloads)
    ...
    expected_overlay = {
        ...
        "predecessor_resolved_file_count": 119,
        "predecessor_resolved_inventory_sha256": inventory_digest,
        "resolved_file_count": 126,
        "stage2b_l3_predecessor_collisions": [],
        "stage2b_l3_requires_zero_predecessor_collisions": True,
    }
    if collisions or not _exact_equal(manifest["overlay"], expected_overlay):
        _fail("L3 zero-collision overlay declaration differs")

    _verify_manifest_files(manifest["files"], payloads, entries)
    canonical_tar = _canonical_tar(payloads)
    _verify_bundle(
        manifest["stage2b_l3_source_bundle"], canonical_tar, payloads
    )
    return {
        "additive_artifact_valid": True,
        "caller_manifest_pin_verified": True,
        ...
        "retained_immutable_execution_evidence": False,
        "runtime_closure_verified": False,
        "schema": "cach.stage2b_l3.additive_artifact_verification.v1",
        "scientific_eligible": False,
        "source_file_count": 7,
        "stage3_authorized": False,
        "training_authorized": False,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify or build the byte-scoped Stage-2B L3 source checkpoint."
    )
    ...
    try:
        if arguments.emit_canonical_bundle:
            if (
                ...
                _fail("bundle build mode does not accept manifest trust-anchor inputs")
            bundle_directory = (
                CANONICAL_BUNDLE_DIRECTORY
                if arguments.bundle_directory is None
                else arguments.bundle_directory
            )
            result = build_bundle(bundle_directory)
        else:
            if arguments.bundle_directory is not None:
                _fail("verification mode does not accept --bundle-directory")
            if (
                arguments.manifest is None
                or arguments.expected_manifest_sha256 is None
            ):
                _fail(
                    "verification requires explicit --manifest and "
                    "--expected-manifest-sha256"
                )
            result = verify(arguments.manifest, arguments.expected_manifest_sha256)
    except (
        OSError,
        Stage2BL3VerificationError,
        ...
        failure = {
            ...
            "formal_filesystem_admission": False,
            "formal_ledger_root_authorized": False,
            "l2b_predecessor_verified": False,
            "l3_production_recovery_authorized": False,
            "l3_runtime_authorized": False,
            "l3_source_checkpoint_verified": False,
            "l3_authorized": False,
            ...
            "retained_immutable_execution_evidence": False,
            "runtime_closure_verified": False,
            "schema": "cach.stage2b_l3.additive_artifact_verification_failure.v1",
            "scientific_eligible": False,
            "stage3_authorized": False,
            "training_authorized": False,
            "transitive_runtime_closure": False,
