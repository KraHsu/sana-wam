#!/usr/bin/env python
"""Fail-closed 8-GPU supervisor/worker for R10-QKV-ADAPT."""
...
CONFIG_RELATIVE_PATH = (
    "configs/experiments/libero_formal_r10_qkv_adapt_8gpu_epoch1.yaml"
)
RUN_NONCE = "68ba8d044af41769b1913744d3c1079a"
RUN_ROOT = Path(
    "/DATA/share/sana_wam_libero_training/formal_epoch1_r10_qkv_adapt/"
    "libero-ar-r10-qkv-adapt-h8-ynorm-fp32attn-causal1-warmstart-uniform-8gpu-4337-"
    + RUN_NONCE
)
...
FORMAL40_RESULT = Path(
    "/DATA/share/sana_wam_libero_evaluation/r8_h8_ynorm_fp32attn_formal40/"
    "libero-r8-h8-ynorm-fp32attn-formal40-59f9b781a9ae40b3944916198a965545/"
    "FORMAL_RESULT.json"
)
FORMAL40_RESULT_SHA256 = (
    "29eb848d9a25207a109b95b0e576806753fc1a5a7befe8340de2bed395852e22"
)
K1_AB40_RESULT = Path(
    "/DATA/share/sana_wam_libero_diagnostics/r8_runtime_k1_ab40/"
    "libero-r8-runtime-k1-ab40-76b15ab5261180fb6157d4c27276a787/AB_RESULT.json"
)
K1_AB40_RESULT_SHA256 = (
    "9d9251880fae1456496b504e6e3ab0df528ca048fe8ab90b4449a458d6c9dd7f"
)
YNORM_WEIGHT_KEY = "video_backbone.dit.attention_y_norm.weight"
YNORM_BF16_SHA256 = "484248da5568c9366002b3404585666a7bfb1e81112094ed8036e2bdd0d9098b"
...
TRAINABLE_CONTRACT = "r10_qkv_adapt_v1"
...
EXPECTED_QKV_NAMES = tuple(
    f"video_backbone.dit.blocks.{index}.attn.qkv.weight" for index in range(20)
)
EXPECTED_QKV_PARAMETERS = 301_056_000
EXPECTED_QKV_TENSORS = 20
EXPECTED_TRAINABLE_PARAMETERS = 940_709_063
EXPECTED_TRAINABLE_TENSORS = 580
...
def _verify_source(
    ...
    if Path.cwd().resolve() != ROOT.resolve():
        raise RuntimeError("R10 must run from the repository root")
...
def _verify_config():
    ...
    exact = {
        ...
        "training.scientific_predecessor_r8_formal40_result_sha256": FORMAL40_RESULT_SHA256,
        "training.diagnostic_predecessor_r8_k1_ab40_result_sha256": K1_AB40_RESULT_SHA256,
        ...
        "training.cumulative_epochs_after_run": 2,
        "training.epochs_in_this_run": 1,
        "training.qkv_adaptation_epochs": 1,
    ...
    for path, expected in exact.items():
        ...
        if observed != expected:
            raise RuntimeError(
                f"R10 config requires {path}={expected!r}, got {observed!r}"
            )
    for forbidden in (
        ...
        if OmegaConf.select(cfg, forbidden, default="__ABSENT__") != "__ABSENT__":
            raise RuntimeError(f"R10 config forbids {forbidden}")
    if (
        OmegaConf.select(
        ...
        raise RuntimeError(
            "R10 requires an exact R8 checkpoint schema with no allowlist"
        )
    if OmegaConf.select(cfg, "model.video_backbone.fp32_attention", default=False):
        raise RuntimeError(
            "R10 FP32-AR treatment is source-level; the legacy config knob must stay off"
        )
    ...
    for payload in (candidate, baseline):
        ...
        for key in (
            "output_dir",
            "formal_predecessor_r6_result_sha256",
            "formal_predecessor_r6_checkpoint_sha256",
            "scientific_predecessor_r6_formal40_result_sha256",
            "formal_predecessor_r7_result_sha256",
            "formal_predecessor_r7_checkpoint_sha256",
            "scientific_predecessor_r7_formal40_result_sha256",
            "formal_predecessor_r8_result_sha256",
            "formal_predecessor_r8_checkpoint_sha256",
            "scientific_predecessor_r8_formal40_result_sha256",
            "diagnostic_predecessor_r8_k1_ab40_result_sha256",
            "init_checkpoint",
            ...
            "qkv_adaptation_epochs",
        ):
            training.pop(key, None)
    if candidate != baseline:
        raise RuntimeError(
            "R10 config differs from R8 outside lineage, cumulative epoch accounting, "
            "and the joint action/visual-QKV coadaptation contract"
        )
    ...
    if contract.get("action_steps") != 20 or contract.get("history_len") != 17:
        raise RuntimeError("R10 deployment parity contract differs")
...
def _verify_predecessors() -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("R8/H8/y_norm/FP32-AR predecessor RESULT semantics differ")
    ...
    if (
        ...
        or aggregate.get("successes") != 3
        or formal40_payload.get("training", {}).get("checkpoint_sha256")
        != PREDECESSOR_R8_CHECKPOINT_SHA256
        or formal40_payload.get("training", {}).get("result_sha256")
        != PREDECESSOR_R8_RESULT_SHA256
    ):
        raise RuntimeError(
            "R8/H8/y_norm/FP32-AR formal40 predecessor semantics differ from 3/40"
        )
    k1_ab40 = _verify_file(K1_AB40_RESULT, K1_AB40_RESULT_SHA256)
    k1_payload = json.loads(K1_AB40_RESULT.read_text(encoding="utf-8"))
    k1_aggregate = k1_payload.get("aggregate", {})
    k1_comparison = k1_payload.get("comparison", {})
    if (
        k1_payload.get("valid_run") is not True
        or k1_payload.get("execution_result") != "PASS"
        or k1_payload.get("verdict") != "LIBERO_R8_RUNTIME_K1_AB40_DESCRIPTIVE_COMPLETE"
        or k1_payload.get("formal_evaluation") is not False
        or k1_payload.get("parameter_updates") is not False
        or k1_aggregate.get("episodes") != 40
        or k1_aggregate.get("successes") != 1
        or k1_comparison.get("baseline_r8_same40_result_sha256")
        != FORMAL40_RESULT_SHA256
        or k1_comparison.get("baseline_r8_same40_successes") != 3
        or k1_comparison.get("treatment_execute_horizon") != 1
    ):
        raise RuntimeError("R8 runtime K1 same40 terminal semantics differ from 1/40")
    return {
        "r8_h8_ynorm_fp32attn_epoch1_result": result,
        "r8_h8_ynorm_fp32attn_epoch1_checkpoint": checkpoint,
        "scientific_r8_h8_ynorm_fp32attn_formal40": formal40,
        "diagnostic_r8_runtime_k1_ab40": k1_ab40,
    }


def _verify_materialized_r8_warm_start(trainer: Any) -> dict[str, Any]:
    """Prove R10 loaded the complete R8 checkpoint with an exact schema."""
    import torch
    ...
    if missing or unexpected:
        raise RuntimeError(
            "R10 strict R8 warm-start schema differs: "
            f"missing={missing}, unexpected={unexpected}"
        )

    parameters = dict(trainer.architecture.named_parameters())
    weight = parameters.get(YNORM_WEIGHT_KEY)
    if weight is None or weight.numel() != 2_240:
        raise RuntimeError(
            "R10 inherited text RMSNorm parameter is absent or malformed"
        )
    if weight.requires_grad:
        raise RuntimeError("R10 inherited text RMSNorm must remain frozen")
    detached = weight.detach()
    if detached.is_meta or not bool(torch.isfinite(detached).all().item()):
        raise RuntimeError("R10 inherited text RMSNorm is non-materialized/non-finite")
    parameter_sha256 = _verify_tensor_sha256(
        detached,
        YNORM_BF16_SHA256,
        label="materialized R10 inherited BF16 attention_y_norm.weight",
    )
    return {
        "strict_load": True,
        ...
        "inherited_ynorm_sha256": YNORM_BF16_SHA256,
        "parameter": {
            "name": YNORM_WEIGHT_KEY,
...
def _verify_materialized_fp32_ar_operator(trainer: Any) -> dict[str, Any]:
    """Bind the materialized trainer to the R10 FP32 train/cache operator."""
    import torch
    ...
    if not isinstance(driver, SanaARMoTJointDriver):
        raise RuntimeError("R10 trainer did not materialize SanaARMoTJointDriver")
    if observed_eps != AR_LINEAR_ATTN_EPS or driver.eps != AR_LINEAR_ATTN_EPS:
        raise RuntimeError(
            "R10 train/cache epsilon differs: "
    ...
    if dtype_map != expected_dtype_map:
        raise RuntimeError(f"R10 FP32-AR compute dtype map differs: {dtype_map}")
    return {
        "enabled": True,
...
def _composite_parameter_digest(
    ...
    for name, parameter in named_parameters:
        ...
        if detached.is_meta:
            raise RuntimeError(f"R10 QKV parameter is not materialized: {name}")
        metadata = {
            "name": name,
...
def _verify_materialized_trainable_partition(trainer: Any) -> dict[str, Any]:
    """Prove that R10 opens exactly the R8 roots plus all 20 video QKV weights."""
    ...
    if non_eval_video_modules:
        raise RuntimeError(
            "R10 video backbone must remain entirely in eval mode: "
    ...
    if (
        ...
        raise RuntimeError("R10 trainable parameter partition differs")
    ...
    for name in trainable_names:
        ...
        if len(matches) != 1:
            raise RuntimeError(
                f"R10 trainable parameter must match exactly one pattern: {name}"
            )
        match_counts[matches[0]] += 1
    if any(count == 0 for count in match_counts.values()):
        raise RuntimeError(f"R10 trainable pattern matched no tensor: {match_counts}")

    qkv_name_set = set(EXPECTED_QKV_NAMES)
    observed_qkv_names = {
        name for name in trainable_names if name.startswith("video_backbone.")
    }
    if observed_qkv_names != qkv_name_set:
        raise RuntimeError(
            "R10 trainable video names differ from the exact 20-layer QKV manifest"
        )
    qkv_parameters = [(name, named_parameters[name]) for name in EXPECTED_QKV_NAMES]
    if (
        len(qkv_parameters) != EXPECTED_QKV_TENSORS
        or sum(parameter.numel() for _, parameter in qkv_parameters)
        != EXPECTED_QKV_PARAMETERS
        or not all(parameter.requires_grad for _, parameter in qkv_parameters)
    ):
        raise RuntimeError("R10 materialized QKV trainability/count differs")

    frozen_video_trainables = [
        name
        for name, parameter in named_parameters.items()
        if name.startswith("video_backbone.")
        and name not in qkv_name_set
        and parameter.requires_grad
    ]
    if frozen_video_trainables:
        raise RuntimeError(
            f"R10 opened non-QKV video parameters: {frozen_video_trainables[:20]}"
        )
    ...
    return {
        ...
        "qkv": {
            "names": list(EXPECTED_QKV_NAMES),
            "tensor_count": len(qkv_parameters),
            "parameter_count": sum(
                parameter.numel() for _, parameter in qkv_parameters
            ),
            "all_require_grad": True,
            "composite_sha256": _composite_parameter_digest(qkv_parameters),
        },
...
def _verify_saved_ynorm_checkpoint(
    path: Path, expected_tensor_sha256: str
) -> dict[str, Any]:
    """Bind the deployable checkpoint to the same restored frozen RMSNorm."""
    import torch
    from safetensors import safe_open

    with safe_open(str(path), framework="pt", device="cpu") as checkpoint:
        if YNORM_WEIGHT_KEY not in checkpoint.keys():
            raise RuntimeError("R10 checkpoint omitted the restored text RMSNorm")
        weight = checkpoint.get_tensor(YNORM_WEIGHT_KEY)
    if weight.numel() != 2_240 or not bool(torch.isfinite(weight).all().item()):
        raise RuntimeError("R10 checkpoint text RMSNorm is malformed or non-finite")
    raw = weight.contiguous().view(torch.uint8).numpy().tobytes()
    observed = hashlib.sha256(raw).hexdigest()
    if observed != expected_tensor_sha256:
        raise RuntimeError("R10 checkpoint changed the frozen pretrained text RMSNorm")
    return {
        "name": YNORM_WEIGHT_KEY,
        "present": True,
        "numel": weight.numel(),
...
def _prepare_root_parent() -> None:
    ...
    namespace_paths = (
        ...
        Path("/DATA/share/sana_wam_libero_training/formal_epoch1_r10_qkv_adapt"),
    )
    ...
    if RUN_ROOT.parent != namespace_paths[-1]:
        raise RuntimeError("R10 root parent differs from its declared namespace")
    if os.path.lexists(RUN_ROOT):
        raise FileExistsError(f"R10 root already exists: {RUN_ROOT}")
...
def _freeze_root_contents() -> None:
    ...
    if stat.S_IMODE(os.lstat(RUN_ROOT).st_mode) != 0o700:
        raise RuntimeError("R10 root must remain mode 0700 before terminal publication")
...
def _verify_frozen_contents(*, expected_root_mode: int) -> None:
    ...
    if (
        ...
        raise RuntimeError(
            f"R10 root mode differs from {expected_root_mode:04o} during verification"
        )
    for directory, subdirs, files in os.walk(RUN_ROOT, topdown=False):
        base = Path(directory)
        for name in files:
            ...
            if (
                ...
                raise RuntimeError(f"R10 file is not frozen to 0444: {child}")
        for name in subdirs:
            child = base / name
            metadata = os.lstat(child)
            if (
                ...
                raise RuntimeError(f"R10 directory is not frozen to 0555: {child}")
...
def _seal_root(expected_terminal: str) -> None:
    ...
    if not stat.S_ISDIR(metadata.st_mode) or stat.S_IMODE(metadata.st_mode) != 0o555:
        raise RuntimeError("R10 terminal root did not freeze to mode 0555")
    ...
    if (
        ...
        raise RuntimeError(f"R10 terminal artifact is not frozen: {terminal}")
...
def _publish_success_result(payload: Any) -> str:
    if os.path.lexists(RUN_ROOT / "RESULT.json") or os.path.lexists(
        ...
        raise FileExistsError("R10 terminal artifact already exists")
...
def _discard_uncommitted_result() -> None:
    ...
    if not stat.S_ISREG(metadata.st_mode) or stat.S_ISLNK(metadata.st_mode):
        raise RuntimeError("uncommitted R10 RESULT is unsafe")
...
def _supervisor_signal_guard():
    ...
    def _raise_interrupted(signum, _frame):
        raise InterruptedError(f"R10 supervisor received signal {signum}")
...
def _worker(args: argparse.Namespace) -> None:
    if os.environ.get("SANA_WAM_R10_QKV_ADAPT_SUPERVISED_WORKER") != "1":
        raise RuntimeError("R10 worker may only be launched by its supervisor")
    world_size = int(os.environ.get("WORLD_SIZE", "0"))
    rank = int(os.environ.get("RANK", "-1"))
    local_rank = int(os.environ.get("LOCAL_RANK", "-1"))
    if world_size != 8 or rank not in range(8) or local_rank not in range(8):
        raise RuntimeError("R10 worker rank environment differs")
    logging.basicConfig(
        level=logging.INFO,
        format=(
    ...
    worker_logger = logging.getLogger("sana_wam.r10_qkv_adapt.worker")
    ...
    try:
        worker_logger.info("NCCL process group initialized; beginning R10 preflight")
        ...
        def _fail_if_any_rank(stage: str, local_error: str | None) -> None:
            ...
            if failures:
                raise RuntimeError(
                    f"R10 {stage} failed on one or more ranks: "
        ...
        if cfg is None:
            raise RuntimeError(
                "R10 config missing after successful collective preflight"
            )
        ...
        try:
            if len(trainer.dataset) != EXPECTED_DATASET_WINDOWS:
                raise RuntimeError("R10 dataset window count differs")
        ...
        worker_logger.info("R10 preflight complete; entering formal training")
        ...
        try:
            ...
            if (
                EXPECTED_WINDOW_DRAWS - EXPECTED_DATASET_WINDOWS
                != EXPECTED_SAMPLER_PADDING_REPEATS
            ):
            ...
            if (
                ...
                raise RuntimeError("R10 warm-start/operator/trainable proof is absent")
            post_training_warm_start = _verify_materialized_r8_warm_start(trainer)
            ...
            if (
                post_training_warm_start["parameter"]["sha256"]
                != warm_start_integrity["parameter"]["sha256"]
            ):
                raise RuntimeError(
                    "R10 frozen inherited text RMSNorm changed during training"
                )
            if post_training_operator != fp32_ar_operator:
                raise RuntimeError(
                    "R10 FP32-AR operator contract changed during training"
                )
            if (
                ...
                raise RuntimeError("R10 trainable manifest changed during training")
            if (
                post_training_partition["qkv"]["composite_sha256"]
                == trainable_partition["qkv"]["composite_sha256"]
            ):
                raise RuntimeError("R10 20-layer QKV composite did not update")
            summary["warm_start_integrity"] = {
                **warm_start_integrity,
                "post_training_parameter_sha256": post_training_warm_start["parameter"][
                    "sha256"
                ],
                "frozen_value_preserved_through_training": True,
            }
            summary["fp32_ar_operator"] = fp32_ar_operator
            summary["trainable_partition"] = {
                **post_training_partition,
                "initial_qkv_composite_sha256": trainable_partition["qkv"][
                    "composite_sha256"
                ],
                "qkv_composite_changed": True,
            }
        except Exception as exc:
            local_error = f"{type(exc).__name__}: {exc}"
        ...
        if summary is None:
            raise RuntimeError(
                "R10 summary missing after successful collective preflight"
            )
        ...
        if rank == 0:
            try:
                ...
                if len(set(canonical)) != 1:
                    raise RuntimeError("R10 rank summaries differ")
                worker_result_sha256 = _exclusive_json(
                    RUN_ROOT / "WORKER_RESULT.json",
                    {
                        "schema_version": "sana-wam-libero-r10-qkv-adapt-worker-result-v1",
        ...
        dist.barrier()
        worker_logger.info("R10 worker completed with rank-consistent terminal summary")
    finally:
    ...
    try:
        with _supervisor_signal_guard():
            ...
            attempt_sha256 = _exclusive_json(
                ...
                {
                    "schema_version": "sana-wam-libero-r10-qkv-adapt-attempt-v1",
                    ...
                    "treatment": {
                        "name": "r10_qkv_adapt",
                        "candidate_axis": "joint_action_visual_qkv_coadaptation",
                        "causal_qkv_attribution": False,
                        "matched_r8_continuation_control": False,
                        "contract": TRAINABLE_CONTRACT,
                        "patterns": list(TRAINABLE_PARAMETER_PATTERNS),
                        "video_backbone_eval": True,
                        "qkv_tensor_count": EXPECTED_QKV_TENSORS,
                        "qkv_parameter_count": EXPECTED_QKV_PARAMETERS,
                        "qkv_names": list(EXPECTED_QKV_NAMES),
            ...
            environment.update(
                {
                    ...
                    "PYTHONUNBUFFERED": "1",
                    "SANA_WAM_R10_QKV_ADAPT_SUPERVISED_WORKER": "1",
            ...
            if (
                ...
                != PREDECESSOR_R8_CHECKPOINT_SHA256
                or warm_start_integrity.get("parameter", {}).get("sha256")
                != YNORM_BF16_SHA256
            ):
                raise RuntimeError("R10 worker strict R8 warm-start proof differs")
            ...
            if (
                ...
                raise RuntimeError("R10 worker FP32-AR operator proof differs")
            trainable_partition = summary.get("trainable_partition", {})
            qkv_partition = trainable_partition.get("qkv", {})
            if (
                ...
                or qkv_partition.get("names") != list(EXPECTED_QKV_NAMES)
                or qkv_partition.get("tensor_count") != EXPECTED_QKV_TENSORS
                or qkv_partition.get("parameter_count") != EXPECTED_QKV_PARAMETERS
                or qkv_partition.get("all_require_grad") is not True
                or trainable_partition.get("qkv_composite_changed") is not True
                or trainable_partition.get("initial_qkv_composite_sha256")
                == qkv_partition.get("composite_sha256")
            ):
                raise RuntimeError("R10 worker QKV-ADAPT materialized proof differs")
            saved_ynorm = _verify_saved_ynorm_checkpoint(
                checkpoint_path,
                warm_start_integrity.get("parameter", {}).get("sha256", ""),
            )
            result = {
                "schema_version": "sana-wam-libero-r10-qkv-adapt-result-v1",
                "valid_run": True,
                "verdict": "LIBERO_AR_R10_QKV_ADAPT_H8_8GPU_EPOCH1_TRAINING_COMPLETE",
                "execution_result": "PASS",
                ...
                "training": {
                    **summary,
                    ...
                    "warm_start_checkpoint_sha256": PREDECESSOR_R8_CHECKPOINT_SHA256,
                    "optimizer_and_lr_state": "fresh",
                    "cumulative_epochs_after_run": 2,
                    "epochs_in_this_run": 1,
                    "qkv_adaptation_epochs": 1,
                    "predecessor_r8_h8_ynorm_fp32attn_optimizer_steps": EXPECTED_FINAL_STEP,
                    "r10_qkv_adapt_h8_optimizer_steps": EXPECTED_FINAL_STEP,
                ...
                "ynorm_contract": {
                    ...
                    "source_checkpoint_sha256": PREDECESSOR_R8_CHECKPOINT_SHA256,
                    "runtime_expected_sha256": YNORM_BF16_SHA256,
                    "saved_checkpoint_parameter": saved_ynorm,
                },
                "fp32_ar_operator": {
                    **fp32_ar_operator,
                    "source_files": source["fp32_ar_operator"]["source_files"],
                },
                "qkv_adapt": trainable_partition,
                "checkpoint": {
                    ...
                    "pretrained_text_rmsnorm": saved_ynorm,
                },
                "worker_result_sha256": worker_result_sha256,
                ...
                "train_deploy_parity": {
                    ...
                    "same_qkv_modules_in_train_and_deploy": True,
                    "checkpoint_contains_adapted_qkv": True,
                },
                "scientific_scope": {
                    "establishes": (
                        "one additional R8-warm-start H8 epoch jointly updating "
                        "the inherited action/proprio roots and exactly 20 video "
                        "DiT self-attention QKV weights"
                    ),
                    "does_not_establish": (
                        "closed-loop policy improvement, value-distribution equivalence, "
                        "or isolated causal attribution to QKV adaptation"
                    ),
                    "candidate_axis": "joint_action_visual_qkv_coadaptation",
                    "causal_qkv_attribution": False,
                    "matched_r8_continuation_control": False,
                    "r8_runtime_k1_h1_direction_rejected": True,
        ...
        if root_created and not success_committed:
            ...
            if os.path.lexists(RUN_ROOT / "FAILED.json"):
                raise RuntimeError("R10 FAILED artifact already exists") from error
            _publish_failure(
                {
                    "schema_version": "sana-wam-libero-r10-qkv-adapt-failure-v1",
                    "valid_run": False,
