#!/usr/bin/env python
"""Fail-closed 8-GPU supervisor/worker for the R7/H8 LIBERO y_norm correction."""
...
CONFIG_RELATIVE_PATH = "configs/experiments/libero_formal_r7_h8_ynorm_8gpu_epoch1.yaml"
RUN_NONCE = "2a2da1f198add6fc718b93d1f2697dc7"
RUN_ROOT = Path(
    "/DATA/share/sana_wam_libero_training/formal_epoch1_r7_h8_ynorm/"
    "libero-ar-r7-h8-ynorm-causal1-warmstart-uniform-8gpu-4337-"
    + RUN_NONCE
)
PREDECESSOR_R6_ROOT = Path(
    "/DATA/share/sana_wam_libero_training/formal_epoch1_r6_h8/"
    "libero-ar-r6-h8-causal1-warmstart-uniform-8gpu-4337-"
    "d21821529d8eba3e5027f85d624715df"
)
PREDECESSOR_R6_RESULT_SHA256 = "1b448d4cc92f284844ba44c68dc536d1a22fee0a75bf76541fed8131ec756b90"
PREDECESSOR_R6_CHECKPOINT_SHA256 = (
    "65c88c642429351f21e42f9c47aaa211b67cdcf27cab42a63350f100e028be47"
)
FORMAL40_RESULT = Path(
    "/DATA/share/sana_wam_libero_evaluation/r6_h8_formal40/"
    "libero-r6-h8-formal40-69f9bc9911bdab3b88025cec85696db7/FORMAL_RESULT.json"
)
FORMAL40_RESULT_SHA256 = (
    "642d262526122c660a71b50d9c535e591ab7866687ea134a5e0a7d868a5d74ca"
)
YNORM_WEIGHT_KEY = "video_backbone.dit.attention_y_norm.weight"
YNORM_ALLOW_MISSING_PATTERNS = (YNORM_WEIGHT_KEY,)
SANA_BASE_CHECKPOINT = Path(
    "/DATA/share/SANA-Video_2B_480p/checkpoints/SANA_Video_2B_480p.pth"
)
SANA_BASE_CHECKPOINT_SHA256 = (
    "052c4022488949153fe6400631725bcb8c739362a6d34edeab26236d8a23f1e7"
)
SANA_BASE_YNORM_KEY = "attention_y_norm.weight"
SANA_BASE_YNORM_FP32_SHA256 = (
    "d677128a3a540b67c822c2226085fc209f009d5b52c16f9d63e0ef5979bf0bb9"
)
SANA_BASE_YNORM_BF16_SHA256 = (
...
def _verify_pretrained_ynorm_source() -> dict[str, Any]:
    """Bind the only R6-missing parameter to the published SANA base bytes."""
    import torch

    checkpoint_artifact = _verify_file(
        SANA_BASE_CHECKPOINT, SANA_BASE_CHECKPOINT_SHA256
    )
    checkpoint = torch.load(
        SANA_BASE_CHECKPOINT,
        map_location="cpu",
        weights_only=True,
        mmap=True,
    )
    if (
        ...
        and not any(str(key).startswith("blocks.") for key in checkpoint)
    ):
        checkpoint = checkpoint["state_dict"]
    if not isinstance(checkpoint, dict) or SANA_BASE_YNORM_KEY not in checkpoint:
        raise RuntimeError("SANA base checkpoint omitted attention_y_norm.weight")
    weight = checkpoint[SANA_BASE_YNORM_KEY]
    if (
        tuple(weight.shape) != (2_240,)
        or weight.dtype != torch.float32
        or not bool(torch.isfinite(weight).all().item())
    ):
        raise RuntimeError("SANA base attention_y_norm.weight metadata differs")
    fp32_sha256 = _verify_tensor_sha256(
        weight,
        SANA_BASE_YNORM_FP32_SHA256,
        label="SANA base FP32 attention_y_norm.weight",
    )
    bf16_sha256 = _verify_tensor_sha256(
        weight.to(torch.bfloat16),
        SANA_BASE_YNORM_BF16_SHA256,
        label="SANA base BF16 attention_y_norm.weight",
    )
    return {
        "checkpoint": checkpoint_artifact,
        "parameter": {
            "source_name": SANA_BASE_YNORM_KEY,
            "architecture_name": YNORM_WEIGHT_KEY,
            "shape": [2_240],
            "source_dtype": "torch.float32",
            "source_sha256": fp32_sha256,
            "runtime_dtype": "torch.bfloat16",
            "runtime_expected_sha256": bf16_sha256,
            "digest_contract": (
                "sha256(detach.cpu.contiguous.view(torch.uint8).numpy.tobytes); "
                "no key, shape, dtype, or metadata bytes"
            ),
        },
    }
...
def _verify_source(
    ...
    if Path.cwd().resolve() != ROOT.resolve():
        raise RuntimeError("R7 must run from the repository root")
...
def _verify_config():
    ...
    exact = {
        ...
        "training.init_checkpoint": str(PREDECESSOR_R6_ROOT / "checkpoint_step_4337.safetensors"),
        "training.init_checkpoint_sha256": PREDECESSOR_R6_CHECKPOINT_SHA256,
        "training.init_checkpoint_allow_missing_patterns": list(YNORM_ALLOW_MISSING_PATTERNS),
        "training.formal_predecessor_r6_result_sha256": PREDECESSOR_R6_RESULT_SHA256,
        "training.formal_predecessor_r6_checkpoint_sha256": PREDECESSOR_R6_CHECKPOINT_SHA256,
        "training.scientific_predecessor_r6_formal40_result_sha256": FORMAL40_RESULT_SHA256,
    ...
    for path, expected in exact.items():
        ...
        if observed != expected:
            raise RuntimeError(f"R7 config requires {path}={expected!r}, got {observed!r}")
    candidate = OmegaConf.to_container(cfg, resolve=True)
    baseline = OmegaConf.to_container(
        OmegaConf.load(
            ROOT / "configs/experiments/libero_formal_r6_h8_8gpu_epoch1.yaml"
        ),
        resolve=True,
    )
    for payload in (candidate, baseline):
        model_kwargs = payload["model"]["video_backbone"]["model_kwargs"]
        model_kwargs.pop("y_norm", None)
        model_kwargs.pop("y_norm_scale_factor", None)
        training = payload["training"]
        for key in (
            "output_dir",
            "formal_predecessor_r5_result_sha256",
            "formal_predecessor_r5_checkpoint_sha256",
            "scientific_predecessor_epoch2_formal40_result_sha256",
            "scientific_predecessor_k8_ab_result_sha256",
    ...
    if candidate != baseline:
        raise RuntimeError("R7 config differs from R6 outside the frozen y_norm delta")
    ...
    if contract.get("action_steps") != 20 or contract.get("history_len") != 17:
        raise RuntimeError("R7 deployment parity contract differs")
...
def _verify_predecessors() -> dict[str, Any]:
    result = _verify_file(
        PREDECESSOR_R6_ROOT / "RESULT.json", PREDECESSOR_R6_RESULT_SHA256
    )
    checkpoint = _verify_file(
        PREDECESSOR_R6_ROOT / "checkpoint_step_4337.safetensors",
        PREDECESSOR_R6_CHECKPOINT_SHA256,
    )
    payload = json.loads(
        (PREDECESSOR_R6_ROOT / "RESULT.json").read_text(encoding="utf-8")
    )
    if (
        payload.get("valid_run") is not True
        or payload.get("execution_result") != "PASS"
        or payload.get("verdict")
        != "LIBERO_AR_R6_H8_8GPU_EPOCH1_TRAINING_COMPLETE"
        or payload.get("checkpoint", {}).get("sha256")
        != PREDECESSOR_R6_CHECKPOINT_SHA256
    ):
        raise RuntimeError("R6/H8 predecessor RESULT semantics differ")
    ...
    if (
        ...
        != PREDECESSOR_R6_RESULT_SHA256
    ):
        raise RuntimeError("R6/H8 formal40 predecessor semantics differ from 2/40")
    return {
        "r6_h8_epoch1_result": result,
        "r6_h8_epoch1_checkpoint": checkpoint,
        "scientific_r6_h8_formal40": formal40,
    }


def _verify_materialized_ynorm_warm_start(trainer: Any) -> dict[str, Any]:
    """Prove the R6 warm start omitted only the restored text RMSNorm."""
    import torch
    from safetensors import safe_open

    checkpoint_path = PREDECESSOR_R6_ROOT / "checkpoint_step_4337.safetensors"
    ...
    if missing != [YNORM_WEIGHT_KEY] or unexpected:
        raise RuntimeError(
            "R7 warm-start schema delta must be exactly the text RMSNorm: "
    ...
    if weight is None or weight.numel() != 2_240:
        raise RuntimeError("R7 pretrained text RMSNorm parameter is absent or malformed")
    if weight.requires_grad:
        raise RuntimeError("R7 pretrained text RMSNorm must remain frozen")
    detached = weight.detach()
    if detached.is_meta or not bool(torch.isfinite(detached).all().item()):
        raise RuntimeError("R7 restored pretrained text RMSNorm is non-materialized/non-finite")
    parameter_sha256 = _verify_tensor_sha256(
        detached,
        SANA_BASE_YNORM_BF16_SHA256,
        label="materialized R7 BF16 attention_y_norm.weight",
    )
    return {
        "strict_load": True,
        "allow_missing_patterns": list(YNORM_ALLOW_MISSING_PATTERNS),
        "observed_missing_keys": missing,
        "observed_unexpected_keys": unexpected,
        "pretrained_text_rmsnorm_restored": True,
        "source_checkpoint_sha256": SANA_BASE_CHECKPOINT_SHA256,
        "source_parameter_fp32_sha256": SANA_BASE_YNORM_FP32_SHA256,
        "runtime_expected_sha256": SANA_BASE_YNORM_BF16_SHA256,
        "parameter": {
...
def _verify_saved_ynorm_checkpoint(path: Path, expected_tensor_sha256: str) -> dict[str, Any]:
    ...
    with safe_open(str(path), framework="pt", device="cpu") as checkpoint:
        if YNORM_WEIGHT_KEY not in checkpoint.keys():
            raise RuntimeError("R7 checkpoint omitted the restored text RMSNorm")
        weight = checkpoint.get_tensor(YNORM_WEIGHT_KEY)
    if weight.numel() != 2_240 or not bool(torch.isfinite(weight).all().item()):
        raise RuntimeError("R7 checkpoint text RMSNorm is malformed or non-finite")
    raw = weight.contiguous().view(torch.uint8).numpy().tobytes()
    observed = hashlib.sha256(raw).hexdigest()
    if observed != expected_tensor_sha256:
        raise RuntimeError("R7 checkpoint changed the frozen pretrained text RMSNorm")
    return {
        "name": YNORM_WEIGHT_KEY,
        "present": True,
        "numel": weight.numel(),
...
def _prepare_root_parent() -> None:
    ...
    namespace_paths = (
        ...
        Path("/DATA/share/sana_wam_libero_training/formal_epoch1_r7_h8_ynorm"),
    )
    ...
    if RUN_ROOT.parent != namespace_paths[-1]:
        raise RuntimeError("R7 root parent differs from its declared namespace")
    if os.path.lexists(RUN_ROOT):
        raise FileExistsError(f"R7 root already exists: {RUN_ROOT}")
...
def _freeze_root_contents() -> None:
    ...
    if stat.S_IMODE(os.lstat(RUN_ROOT).st_mode) != 0o700:
        raise RuntimeError("R7 root must remain mode 0700 before terminal publication")
...
def _verify_frozen_contents(*, expected_root_mode: int) -> None:
    ...
    if (
        ...
        raise RuntimeError(
            f"R7 root mode differs from {expected_root_mode:04o} during verification"
        )
    for directory, subdirs, files in os.walk(RUN_ROOT, topdown=False):
        base = Path(directory)
        for name in files:
            ...
            if (
                ...
                raise RuntimeError(f"R7 file is not frozen to 0444: {child}")
        for name in subdirs:
            child = base / name
            metadata = os.lstat(child)
            if (
                ...
                raise RuntimeError(f"R7 directory is not frozen to 0555: {child}")
...
def _seal_root(expected_terminal: str) -> None:
    ...
    if not stat.S_ISDIR(metadata.st_mode) or stat.S_IMODE(metadata.st_mode) != 0o555:
        raise RuntimeError("R7 terminal root did not freeze to mode 0555")
    ...
    if (
        ...
        raise RuntimeError(f"R7 terminal artifact is not frozen: {terminal}")
...
def _publish_success_result(payload: Any) -> str:
    if os.path.lexists(RUN_ROOT / "RESULT.json") or os.path.lexists(
        ...
        raise FileExistsError("R7 terminal artifact already exists")
...
def _discard_uncommitted_result() -> None:
    ...
    if not stat.S_ISREG(metadata.st_mode) or stat.S_ISLNK(metadata.st_mode):
        raise RuntimeError("uncommitted R7 RESULT is unsafe")
...
def _supervisor_signal_guard():
    ...
    def _raise_interrupted(signum, _frame):
        raise InterruptedError(f"R7 supervisor received signal {signum}")
...
def _worker(args: argparse.Namespace) -> None:
    if os.environ.get("SANA_WAM_R7_H8_YNORM_SUPERVISED_WORKER") != "1":
        raise RuntimeError("R7 worker may only be launched by its supervisor")
    world_size = int(os.environ.get("WORLD_SIZE", "0"))
    rank = int(os.environ.get("RANK", "-1"))
    local_rank = int(os.environ.get("LOCAL_RANK", "-1"))
    if world_size != 8 or rank not in range(8) or local_rank not in range(8):
        raise RuntimeError("R7 worker rank environment differs")
    logging.basicConfig(
        level=logging.INFO,
        format=(
    ...
    worker_logger = logging.getLogger("sana_wam.r7_ynorm.worker")
    ...
    try:
        worker_logger.info("NCCL process group initialized; beginning R7 preflight")
        ...
        def _fail_if_any_rank(stage: str, local_error: str | None) -> None:
            ...
            if failures:
                raise RuntimeError(
                    f"R7 {stage} failed on one or more ranks: "
        ...
        if cfg is None:
            raise RuntimeError("R7 config missing after successful collective preflight")
        ...
        ynorm_warm_start = None
        local_error = None
        try:
            if len(trainer.dataset) != EXPECTED_DATASET_WINDOWS:
                raise RuntimeError("R7 dataset window count differs")
            trainables = [
                parameter
            ...
            if (
                ...
                raise RuntimeError("R7 trainable parameter partition differs")
            ynorm_warm_start = _verify_materialized_ynorm_warm_start(trainer)
        except Exception as exc:
            local_error = f"{type(exc).__name__}: {exc}"
        _fail_if_any_rank("materialized trainer preflight", local_error)
        worker_logger.info("R7 preflight complete; entering formal training")
        ...
        try:
            ...
            if ynorm_warm_start is None:
                raise RuntimeError("R7 y_norm warm-start proof is absent")
            post_training_ynorm = _verify_materialized_ynorm_warm_start(trainer)
            if (
                post_training_ynorm["parameter"]["sha256"]
                != ynorm_warm_start["parameter"]["sha256"]
            ):
                raise RuntimeError("R7 frozen pretrained text RMSNorm changed during training")
            summary["warm_start_integrity"] = {
                **ynorm_warm_start,
                "post_training_parameter_sha256": post_training_ynorm["parameter"]["sha256"],
        ...
        if summary is None:
            raise RuntimeError("R7 summary missing after successful collective preflight")
        ...
        if rank == 0:
            try:
                ...
                if len(set(canonical)) != 1:
                    raise RuntimeError("R7 rank summaries differ")
                worker_result_sha256 = _exclusive_json(
                    RUN_ROOT / "WORKER_RESULT.json",
                    {
                        "schema_version": "sana-wam-libero-r7-h8-ynorm-worker-result-v1",
        ...
        dist.barrier()
        worker_logger.info("R7 worker completed with rank-consistent terminal summary")
    finally:
...
def _supervise(args: argparse.Namespace) -> None:
    ...
    pretrained_ynorm_source = _verify_pretrained_ynorm_source()
    root_created = False
    ...
    try:
        with _supervisor_signal_guard():
            ...
            attempt_sha256 = _exclusive_json(
                ...
                {
                    "schema_version": "sana-wam-libero-r7-h8-ynorm-attempt-v1",
                    "state": "RUNNING_NON_RESUMABLE_WARM_START",
                    "created_at_utc": started_at,
                    "run": {"root": str(RUN_ROOT), "nonce": RUN_NONCE},
                    "source": source,
                    "predecessors": predecessors,
                    "pretrained_ynorm_source": pretrained_ynorm_source,
                    ...
                    "training": {
                        ...
                        "warm_start_checkpoint_sha256": PREDECESSOR_R6_CHECKPOINT_SHA256,
                        "init_checkpoint_allow_missing_patterns": list(
                            YNORM_ALLOW_MISSING_PATTERNS
                        ),
                        "pretrained_text_rmsnorm_restored": True,
            ...
            environment.update(
                {
                    ...
                    "PYTHONUNBUFFERED": "1",
                    "SANA_WAM_R7_H8_YNORM_SUPERVISED_WORKER": "1",
            ...
            if (
                warm_start_integrity.get("pretrained_text_rmsnorm_restored") is not True
                ...
                != SANA_BASE_CHECKPOINT_SHA256
                or warm_start_integrity.get("source_parameter_fp32_sha256")
                != SANA_BASE_YNORM_FP32_SHA256
                or warm_start_integrity.get("parameter", {}).get("sha256")
                != SANA_BASE_YNORM_BF16_SHA256
            ):
                raise RuntimeError("R7 worker y_norm warm-start proof differs")
            saved_ynorm = _verify_saved_ynorm_checkpoint(
                checkpoint_path,
                warm_start_integrity.get("parameter", {}).get("sha256", ""),
            )
            result = {
                "schema_version": "sana-wam-libero-r7-h8-ynorm-result-v1",
                "valid_run": True,
                "verdict": "LIBERO_AR_R7_H8_YNORM_8GPU_EPOCH1_TRAINING_COMPLETE",
                "execution_result": "PASS",
                ...
                "training": {
                    **summary,
                    ...
                    "predecessor_r6_h8_optimizer_steps": EXPECTED_FINAL_STEP,
                    "r7_h8_ynorm_uniform_optimizer_steps": EXPECTED_FINAL_STEP,
                ...
                "ynorm_correction": {
                    ...
                    "scale_factor": 0.01,
                    "pretrained_text_rmsnorm_restored": True,
                    "checkpoint_missing_only": [YNORM_WEIGHT_KEY],
                ...
                "scientific_scope": {
                    "establishes": (
                        "causal H8 parity with the pretrained text RMSNorm restored"
                    ),
                    "does_not_establish": (
        ...
        if root_created and not success_committed:
            ...
            if os.path.lexists(RUN_ROOT / "FAILED.json"):
                raise RuntimeError("R7 FAILED artifact already exists") from error
            _publish_failure(
                {
                    "schema_version": "sana-wam-libero-r7-h8-ynorm-failure-v1",
                    "valid_run": False,
