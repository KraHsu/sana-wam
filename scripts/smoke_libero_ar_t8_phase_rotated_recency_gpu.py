#!/usr/bin/env python
"""Run the bounded LIBERO T8 phase-rotated terminal-recency screen."""
...
T8_UPDATE_STEPS = 20
T8_CYCLE_COUNT = 5
T8_UPDATES_PER_SAMPLE = 5
T8_PHASE_ROTATED_LABELS = ("A1", "A2", "A3", "A0")
T8_PHASE_PROBE_STEPS = {
    17: ("A1", "H1"),
    18: ("A2", "H2"),
    19: ("A3", "H3"),
    20: ("A0", "H0"),
}
T8_INITIALIZATION_SEED = 20260806
T8_LOSS_RECIPE_SEED = 20260826
T8_MEDIAN_RATIO = 0.95
T8_MIN_IMPROVED = 3
T8_MIN_OVERWRITE_PENALTY = 1.02
T8_MIN_OVERWRITE_COUNT = 2
T8_MIN_SPEARMAN_RHO = 0.8
T8_CONFIG_SHA256 = "5df45965d6de3a32c5154a8bb4c41d29d20113bbe8908c1a98c8c63a7bff4a45"
T8_CONFIG_RELATIVE_PATH = "configs/benchmarks/libero/train_libero_ar_baseline.yaml"
...
T8_ELIGIBLE_MANIFEST_BYTES = 8406
T8_ELIGIBLE_MANIFEST_SHA256 = (
    "0b6bfe4c51e8b9e2ac3f538bac7853b5e1e871ea96dda5df6976ee27b5b25370"
)
T8_SELECTION_MANIFEST_BYTES = 3351
T8_SELECTION_MANIFEST_SHA256 = (
...
T8_UPDATE_SAMPLE_MANIFEST_SHA256 = (
    "5199c87af876c437ecec35b57da3e118a2924c44a9bb454c9846e7c0ca968aa6"
)
T8_FIXED_RECIPE_SIGNATURE_SHA256 = (
...
T8_RUN_NAMESPACE = Path("/DATA/share/sana_wam_libero_nonformal_screens/t8")
...
def _read_jsonl(path: Path) -> list[dict[str, Any]]:
    if path.is_symlink() or not path.is_file():
        raise RuntimeError(f"T8 metadata must be a regular non-symlink file: {path}")
    rows = []
    for line_number, line in enumerate(
        ...
        if not line:
            raise RuntimeError(
                f"T8 metadata contains a blank row: {path}:{line_number}"
            )
        row = json.loads(line)
        if not isinstance(row, dict):
            raise RuntimeError(
                f"T8 metadata row is not an object: {path}:{line_number}"
            )
        rows.append(row)
    return rows
...
def _build_eligible_probe_manifest(
    ...
    if set(metadata_by_suite) != set(T8_SUITE_ORDER):
        raise RuntimeError("T8 eligible suite set differs")
    expected_raw_counts = {
        SPATIAL_NAME: 432,
        OBJECT_NAME: 454,
        GOAL_NAME: 428,
    ...
    update_by_dataset = {row["dataset"]: row for row in T8_CYCLIC_TRAIN_SAMPLES}
    consumed = {
        (row["dataset"], row["task_index"], row["episode_index"])
        for row in T8_EXCLUDED_SAMPLES
    }
    suites = []
    for dataset in T8_SUITE_ORDER:
        task_rows, episode_rows = metadata_by_suite[dataset]
        if any(set(row) != {"task_index", "task"} for row in task_rows):
            raise RuntimeError(f"T8 tasks metadata schema differs: {dataset}")
        ...
        if (
            ...
            raise RuntimeError(f"T8 task registry differs: {dataset}")
        ...
        for row in episode_rows:
            if set(row) != {"episode_index", "tasks", "length"}:
                raise RuntimeError(f"T8 episodes metadata schema differs: {dataset}")
            ...
            if (
                ...
                raise RuntimeError(f"T8 episodes metadata identity differs: {dataset}")
        ...
        if len(episode_rows) not in {
            ...
            raise RuntimeError(f"T8 source episode count differs: {dataset}")
        tasks = [
            {
        ...
        if (
            ...
            raise RuntimeError(f"T8 eligible episode population differs: {dataset}")
        suites.append(
            {
                "dataset": dataset,
                "episode_count": sum(row["episode_count"] for row in tasks),
                "episodes_jsonl_sha256": T8_SUITE_METADATA_SHA256[dataset][
                    "episodes.jsonl"
                ],
                "task_count": len(tasks),
                "tasks": tasks,
                "tasks_jsonl_sha256": T8_SUITE_METADATA_SHA256[dataset]["tasks.jsonl"],
    ...
    manifest = {
        ...
        "excluded_samples": list(T8_EXCLUDED_SAMPLES),
        "schema_version": (
    ...
    if (
        ...
        or len(_canonical_json_bytes(manifest)) != T8_ELIGIBLE_MANIFEST_BYTES
        or _sha256_json(manifest) != T8_ELIGIBLE_MANIFEST_SHA256
    ):
        raise RuntimeError("T8 eligible same-task probe manifest differs")
    return manifest
...
def _eligible_sample_manifest(dataset_roots: dict[str, Path]) -> dict[str, Any]:
    ...
    for dataset in T8_SUITE_ORDER:
        ...
        if _sha256_file(tasks_path) != T8_SUITE_METADATA_SHA256[dataset]["tasks.jsonl"]:
            raise RuntimeError(f"T8 tasks metadata SHA differs: {dataset}")
        if (
            _sha256_file(episodes_path)
            != T8_SUITE_METADATA_SHA256[dataset]["episodes.jsonl"]
        ):
            raise RuntimeError(f"T8 episodes metadata SHA differs: {dataset}")
...
def _episode_selection_payload_sha256(
    ...
    payload = (
        ...
        f"ELIGIBLE_MANIFEST_SHA256={T8_ELIGIBLE_MANIFEST_SHA256}\n"
        f"DATASET={dataset}\n"
...
def _selection_manifest(eligible_manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    for rank, dataset in enumerate(T8_SUITE_ORDER):
        task_rows = suite_by_dataset[dataset]["tasks"]
        if len(task_rows) != 1:
            raise RuntimeError(f"T8 eligible probe task count differs: {dataset}")
        ...
        expected = T8_FRESH_HELDOUT_SAMPLES[rank]
        sample = {
        ...
        if sample != expected:
            raise RuntimeError(f"T8 mechanically selected sample H{rank} differs")
        samples.append(sample)
    manifest = {
        "eligible_manifest_sha256": T8_ELIGIBLE_MANIFEST_SHA256,
        "samples": samples,
        "schema_version": (
            "sana-wam-libero-t7-four-suite-cyclic-same-task-probe-selection-v1"
        ),
        "selection_rule": T8_SELECTION_RULE,
        "suite_order": list(T8_SUITE_ORDER),
        "t6_predecessor_result_sha256": T6_PREDECESSOR_RESULT_SHA256,
    }
    if (
        len(_canonical_json_bytes(manifest)) != T8_SELECTION_MANIFEST_BYTES
        or _sha256_json(manifest) != T8_SELECTION_MANIFEST_SHA256
    ):
        raise RuntimeError("T8 selected same-task probe manifest differs")
    return manifest


def _update_sample_manifest() -> dict[str, Any]:
    samples = []
    for frozen_sample in T8_CYCLIC_TRAIN_SAMPLES:
    ...
    if _sha256_json(manifest) != T8_UPDATE_SAMPLE_MANIFEST_SHA256:
        raise RuntimeError("T8 update sample manifest differs")
    return manifest


def _select_t8_samples(
    dataset: Any,
    eligible_manifest: dict[str, Any],
    selection_manifest: dict[str, Any],
) -> dict[str, dict[str, Any]]:
    live_episode_rows: dict[str, dict[int, dict[str, Any]]] = {
        name: {} for name in T8_SUITE_ORDER
    }
    live_task_rows: dict[str, dict[int, dict[str, Any]]] = {
        name: {} for name in T8_SUITE_ORDER
    }
    for episode in dataset._episodes:
        if episode.dataset not in T8_SUITE_ORDER:
            continue
        if episode.dataset == GOAL_NAME and episode.episode_index == 82:
            raise RuntimeError("T8 excluded Goal episode 82 entered live registry")
        ...
        if previous_task != task_row or episode.episode_index in suite_episodes:
            raise RuntimeError("T8 live cross-suite registry identity is ambiguous")
    ...
    live_manifest = _build_eligible_probe_manifest(
        {
            ...
            for name in T8_SUITE_ORDER
        }
    )
    if live_manifest != eligible_manifest:
        raise RuntimeError("T8 live eligible cross-suite population differs")

    role_rows = [*T8_CYCLIC_TRAIN_SAMPLES, *selection_manifest["samples"]]
    expected_keys = {
        (
            row["dataset"],
            int(row["task_index"]),
    ...
    for row in role_rows:
        ...
        if len(matches) != 1:
            raise RuntimeError(
                "T8 cross-suite episode/start must occur exactly once: "
    ...
    if set(selected) != set(T8_TRAIN_LABELS) | set(T8_HELDOUT_LABELS):
        raise RuntimeError("T8 selected role label set differs")
    return selected
...
def _paired_four_sample_summary(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("T8 frozen labels differ")
    sample_by_label = {row["label"]: row for row in samples}
    if set(sample_by_label) != expected_labels:
        raise ValueError("T8 sample labels differ from the frozen set")
    rows = []
    for label in labels:
        ...
        if (
            ...
            raise ValueError("T8 losses must be finite with positive pre-loss")
        ratio = after / before
        rows.append(
            {
    ...
    return {
        ...
        "paired_gate": (
            ...
            <= T8_MEDIAN_RATIO
            and sum(row["improved"] for row in rows) >= T8_MIN_IMPROVED
        ),
        "per_sample": rows,
...
def _four_suite_cyclic_summary(
    ...
    cyclic_training_summary = _paired_four_sample_summary(
        update_before,
        update_after,
        samples=T8_CYCLIC_TRAIN_SAMPLES,
    )
    fresh_heldout_summary = _paired_four_sample_summary(
        heldout_before,
        heldout_after,
        samples=T8_FRESH_HELDOUT_SAMPLES,
    )
    update_by_dataset = {
    ...
    if set(update_by_dataset) != set(T8_SUITE_ORDER) or set(heldout_by_dataset) != set(
        T8_SUITE_ORDER
    ):
        raise ValueError("T8 suite coverage differs from the frozen order")
    ...
    gate = (
        ...
        and len(paired_suite_improved) >= T8_MIN_IMPROVED
    )
    return {
        "fresh_same_task_update_heldout": fresh_heldout_summary,
        "gate": gate,
        "minimum_paired_suite_improved": T8_MIN_IMPROVED,
        "paired_suite_improved": paired_suite_improved,
        "paired_suite_improved_count": len(paired_suite_improved),
        "threshold_median_ratio": T8_MEDIAN_RATIO,
        "update_samples": cyclic_training_summary,
    }


def _cyclic_label_for_step(step_index: int) -> str:
    if step_index < 0 or step_index >= T8_UPDATE_STEPS:
        raise ValueError("T8 step index is outside the frozen 20-step schedule")
    return T8_PHASE_ROTATED_LABELS[step_index % len(T8_PHASE_ROTATED_LABELS)]
...
def _spearman_rho(left: list[float], right: list[float]) -> float | None:
    if len(left) != len(right) or len(left) < 2:
        raise ValueError("T8 Spearman inputs must have equal nontrivial length")
...
def _at_least_threshold(value: float, threshold: float) -> bool:
...
def _terminal_recency_summary(
    ...
    *,
    ...
    for index, dataset in enumerate(T8_SUITE_ORDER):
        ...
        t7_pair_ratio = statistics.mean(
            (
                T7_TERMINAL_UPDATE_RATIOS[update_label],
                T7_TERMINAL_HELDOUT_RATIOS[heldout_label],
            )
        )
        overwrite_penalty = terminal_pair_ratio / phase_pair_ratio
        terminal_pair_ratios.append(terminal_pair_ratio)
        terminal_update_ratios.append(terminal_update_ratio)
        terminal_heldout_ratios.append(terminal_heldout_ratio)
        t7_pair_ratios.append(t7_pair_ratio)
        per_suite.append(
            {
                "both_improved": (
                ...
                "overwrite_threshold_included": index != 0,
                "overwrite_threshold_passed": (
                    index != 0
                    and _at_least_threshold(overwrite_penalty, T8_MIN_OVERWRITE_PENALTY)
                ),
                ...
                "t8_minus_t7_heldout_ratio": (
                    terminal_heldout_ratio - T7_TERMINAL_HELDOUT_RATIOS[heldout_label]
                ),
                "t8_minus_t7_update_ratio": (
                    terminal_update_ratio - T7_TERMINAL_UPDATE_RATIOS[update_label]
                ),
                "t7_terminal_pair_ratio": t7_pair_ratio,
                "trailing_updates": T8_TRAILING_UPDATES[update_label],
                "update_label": update_label,
            }
        )

    nonterminal = [row for row in per_suite if row["trailing_updates"] > 0]
    overwrite_count = sum(
        _at_least_threshold(row["overwrite_penalty"], T8_MIN_OVERWRITE_PENALTY)
        for row in nonterminal
    )
    overwrite_supported = overwrite_count >= T8_MIN_OVERWRITE_COUNT
    spatial = per_suite[0]
    spatial_terminal_rescued = (
        spatial["terminal_update_ratio"] < 1.0
        and spatial["terminal_heldout_ratio"] < 1.0
    )
    trailing_updates = [float(row["trailing_updates"]) for row in per_suite]
    rho_recency = _spearman_rho(
        trailing_updates,
        terminal_pair_ratios,
    )
    rho_suite = _spearman_rho(
        t7_pair_ratios,
        terminal_pair_ratios,
    )
    terminal_recency_supported = (
        ...
        and rho_recency is not None
        and _at_least_threshold(rho_recency, T8_MIN_SPEARMAN_RHO)
    )
    ...
    suite_effect_supported = (
        spatial_terminal_double_regression
        and rho_suite is not None
        and _at_least_threshold(rho_suite, T8_MIN_SPEARMAN_RHO)
    )
    if terminal_recency_supported:
        verdict = "T8_TERMINAL_RECENCY_SUPPORTED"
    elif suite_effect_supported:
        verdict = "T8_SUITE_EFFECT_SUPPORTED"
    else:
    ...
    return {
        ...
        "joint_viable_diagnostic": joint_viable,
        "minimum_overwrite_count": T8_MIN_OVERWRITE_COUNT,
        "minimum_overwrite_penalty": T8_MIN_OVERWRITE_PENALTY,
        "minimum_spearman_rho": T8_MIN_SPEARMAN_RHO,
        "nonterminal_overwrite_count": overwrite_count,
        "object_oldest_diagnostic": per_suite[1],
        "overwrite_supported": overwrite_supported,
        "per_suite": per_suite,
        "rank_vectors": {
            "q_t7": _rankdata(t7_pair_ratios),
            "q_terminal": _rankdata(terminal_pair_ratios),
            "trailing_updates": _rankdata(trailing_updates),
        },
        "suite_order": list(T8_SUITE_ORDER),
        "value_vectors": {
            "q_t7": t7_pair_ratios,
            "q_terminal": terminal_pair_ratios,
            "trailing_updates": [int(value) for value in trailing_updates],
        },
        "rho_recency": rho_recency,
        "rho_suite": rho_suite,
        "sorted_terminal_heldout_ratios": sorted(terminal_heldout_ratios),
        "sorted_terminal_update_ratios": sorted(terminal_update_ratios),
        "spatial_terminal_double_regression": spatial_terminal_double_regression,
        "spatial_terminal_rescued": spatial_terminal_rescued,
        "suite_effect_supported": suite_effect_supported,
        "t7_terminal_heldout_ratios": dict(T7_TERMINAL_HELDOUT_RATIOS),
        "t7_terminal_pair_ratios": t7_pair_ratios,
        "t7_terminal_update_ratios": dict(T7_TERMINAL_UPDATE_RATIOS),
        "terminal_heldout_median_ratio": statistics.median(terminal_heldout_ratios),
        "terminal_recency_supported": terminal_recency_supported,
...
def _training_core_projection(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if len(per_step) != T8_UPDATE_STEPS:
        raise ValueError("T8/T3 training-core view requires exactly 20 step rows")
    return {
...
def main() -> int:
    ...
    parser.add_argument("--steps", type=int, default=T8_UPDATE_STEPS)
    parser.add_argument(
        "--initialization-seed", type=int, default=T8_INITIALIZATION_SEED
    )
    parser.add_argument("--loss-recipe-seed", type=int, default=T8_LOSS_RECIPE_SEED)
    ...
    if args.steps != T8_UPDATE_STEPS:
        raise ValueError(f"T8 steps must be exactly {T8_UPDATE_STEPS}")
    if args.initialization_seed != T8_INITIALIZATION_SEED:
        raise ValueError("T8 initialization seed differs")
    if args.loss_recipe_seed != T8_LOSS_RECIPE_SEED:
        raise ValueError("T8 loss recipe seed differs")
    if len(args.nonce) != 32 or any(
        character not in "0123456789abcdef" for character in args.nonce
    ):
        raise ValueError("T8 nonce must be exactly 32 lowercase hex characters")
    expected_root_name = f"libero-t8-phase-rotated-fixed20-{args.nonce}"
    if args.run_root.name != expected_root_name:
        raise ValueError("T8 run-root basename does not bind the nonce")
    if args.run_root.parent.name != args.expected_repo_commit[:12]:
        raise ValueError("T8 run-root parent does not bind the source commit")
    expected_run_root = (
        T8_RUN_NAMESPACE / args.expected_repo_commit[:12] / expected_root_name
    )
    if args.run_root.expanduser() != expected_run_root:
        raise ValueError(
            "T8 run root differs from the frozen non-formal namespace: "
    ...
    if config_path != expected_config_path:
        raise ValueError(
            f"T8 config path differs: expected={expected_config_path} got={config_path}"
        )
    ...
    if actual_config_sha256 != T8_CONFIG_SHA256:
        raise RuntimeError("T8 config content differs from the frozen pin")
    actual_runner_sha256 = _sha256_file(runner_path)
    expected_identity = {
        "config_sha256": T8_CONFIG_SHA256,
    ...
    if cfg.model.architecture.variant != "autoregressive":
        raise RuntimeError("T8 requires the AR architecture")
    if cfg.model.video_backbone.continuous_timestep_conditioning is not True:
        raise RuntimeError("T8 requires continuous FP32 video timesteps")
    if cfg.training.use_gradient_checkpointing is not True:
        raise RuntimeError("T8 requires gradient checkpointing")
    if int(cfg.training.seed) != args.initialization_seed:
        raise RuntimeError("T8 CLI initialization seed must equal training.seed")
    if tuple(cfg.training.trainable_modules) != EXPECTED_TRAINABLE_ROOTS:
        raise RuntimeError("T8 trainable module allowlist differs")
    if (
        tuple(cfg.training.preserve_frozen_input_grad_modules)
        != EXPECTED_PRESERVE_FROZEN_INPUT_GRAD_MODULES
    ):
        raise RuntimeError("T8 frozen input-gradient preservation differs")
    if cfg.training.optimizer_master_weights is not True:
        raise RuntimeError("T8 requires persistent FP32 optimizer masters")
    if (
        float(cfg.training.lambda_video) != 0.0
        or float(cfg.training.lambda_action) != 1.0
    ):
        raise RuntimeError("T8 requires action-only loss weighting")
    if float(cfg.training.weight_decay) != 0.0:
        raise RuntimeError("T8 requires weight_decay=0")
    if (
        float(cfg.training.action_lr) != 1.0e-4
        or float(cfg.training.video_lr) != 1.0e-4
    ):
        raise RuntimeError("T8 requires fixed base learning rates of 1e-4")
    if float(cfg.training.grad_clip) != 1.0:
        raise RuntimeError("T8 requires the fixed gradient clip bound 1.0")
    ...
    expected_datasets = set(T8_SUITE_ORDER)
    if (
        len(dataset_roots) != len(configured_roots)
        or set(dataset_roots) != expected_datasets
    ):
        raise RuntimeError("T8 four-suite dataset root identity differs")
    ...
    for dataset in T8_SUITE_ORDER:
        for filename, expected_sha256 in sorted(
            T8_SUITE_METADATA_SHA256[dataset].items()
        ):
    ...
    for role, samples in (
        ("update", T8_CYCLIC_TRAIN_SAMPLES),
        ("heldout", T8_FRESH_HELDOUT_SAMPLES),
    ):
    ...
    if (
        ...
        raise RuntimeError("T8 predecessor T2 evidence semantics differ")
    ...
    if (
        ...
        or t3_predecessor.get("scope", {}).get("backward_calls") != T8_UPDATE_STEPS
        or t3_predecessor.get("scope", {}).get("optimizer_steps") != T8_UPDATE_STEPS
    ):
        raise RuntimeError("T8 predecessor T3 evidence semantics differ")
    ...
    if (
        ...
        or t4_predecessor.get("scope", {}).get("backward_calls") != T8_UPDATE_STEPS
        or t4_predecessor.get("scope", {}).get("optimizer_steps") != T8_UPDATE_STEPS
    ):
        raise RuntimeError("T8 predecessor T4 evidence semantics differ")
    ...
    if (
        ...
        or t5_predecessor.get("scope", {}).get("backward_calls") != T8_UPDATE_STEPS
        or t5_predecessor.get("scope", {}).get("optimizer_steps") != T8_UPDATE_STEPS
    ):
        raise RuntimeError("T8 predecessor T5 evidence semantics differ")
    ...
    if (
        ...
        or t6_predecessor.get("scope", {}).get("backward_calls") != T8_UPDATE_STEPS
        or t6_predecessor.get("scope", {}).get("optimizer_steps") != T8_UPDATE_STEPS
    ):
        raise RuntimeError("T8 predecessor T6 evidence semantics differ")
    ...
    if (
        ...
        raise RuntimeError("T8 frozen T3/T4/T5/T6 training-core evidence differs")
    ...
    if (
        ...
        != T7_PREDECESSOR_RUNNER_SHA256
        or t7_predecessor.get("identity", {}).get("config_sha256") != T8_CONFIG_SHA256
        ...
        raise RuntimeError("T8 predecessor T7 identity or execution semantics differ")
    ...
    if (
        ...
        raise RuntimeError("T8 predecessor T7 frozen loss evidence differs")
    ...
    actual_identity.update(
        {
            ...
            "eligible_manifest_sha256": T8_ELIGIBLE_MANIFEST_SHA256,
            "selection_manifest_sha256": T8_SELECTION_MANIFEST_SHA256,
            "update_sample_manifest_sha256": T8_UPDATE_SAMPLE_MANIFEST_SHA256,
        }
    )
    ...
    if "H200" not in gpu_before["name"].upper():
        raise RuntimeError(f"T8 selected GPU is not an H200: {gpu_before['name']}")
    ...
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError(
            "T8 requires exactly one visible CUDA GPU; "
    ...
    if any(parameter.requires_grad for parameter in video_backbone.parameters()):
        raise RuntimeError("T8 video-backbone parameters are not frozen")
    named_trainables = [
        (name, parameter)
    ...
    if observed_roots != tuple(sorted(EXPECTED_TRAINABLE_ROOTS)):
        raise RuntimeError(f"T8 trainable parameter roots differ: {observed_roots!r}")
    if len(named_trainables) != EXPECTED_TRAINABLE_TENSOR_COUNT:
        raise RuntimeError("T8 trainable tensor count differs")
    if sum(parameter.numel() for _name, parameter in named_trainables) != (
        EXPECTED_TRAINABLE_PARAMETER_COUNT
    ):
        raise RuntimeError("T8 trainable parameter count differs")
    ...
    role_selections = _select_t8_samples(
        trainer.dataset,
    ...
    if (
        ...
        raise RuntimeError("T8 fixed real LIBERO sample identity differs")
    raw_samples: dict[str, dict[str, Any]] = {}
    sample_seconds: dict[str, float] = {}
    role_assets_after_sample: dict[str, str] = {}
    role_order = (*T8_TRAIN_LABELS, *T8_HELDOUT_LABELS)
    for label in role_order:
        ...
        if (
            ...
            raise RuntimeError(f"T8 LIBERO sample metadata differs: {label}")
        raw_samples[label] = raw_sample
        role = "update" if label in T8_TRAIN_LABELS else "heldout"
        for relative_path, expected_sha256 in sorted(selected["assets"].items()):
            ...
            if external_assets[key] != observed:
                raise RuntimeError(f"T8 role asset changed while loading: {key}")
            role_assets_after_sample[key] = observed
    if _sha256_file(stats_path) != SELECTED_STATS_SHA256:
        raise RuntimeError("T8 selected-row stats changed while loading")
    ...
    if any(module.training for module in video_backbone.modules()):
        raise RuntimeError("T8 preserved video backbone left eval mode")
    ...
    def prepare_one(label: str, raw_sample: dict[str, Any]) -> dict[str, Any]:
        ...
        if prepared.get("use_gradient_checkpointing") is not True:
            raise RuntimeError(f"T8 {label} inputs dropped gradient checkpointing")
        if prepared.get("use_gradient_checkpointing_offload") is not False:
            raise RuntimeError("T8 requires checkpoint offload=false")
        invalid_tensors = [
            key
        ...
        if invalid_tensors:
            raise RuntimeError(
                f"T8 {label} prepared tensors retain autograd state: "
                f"{invalid_tensors!r}"
            )
        return prepared
    ...
    cyclic_training_inputs = {
        label: prepared_by_label[label] for label in T8_TRAIN_LABELS
    }
    fresh_heldout_inputs = {
        label: prepared_by_label[label] for label in T8_HELDOUT_LABELS
    }
    metadata_after_sample = {}
    for dataset_name in T8_SUITE_ORDER:
        for filename, expected_sha256 in sorted(
            T8_SUITE_METADATA_SHA256[dataset_name].items()
        ):
            ...
            if external_assets[key] != observed:
                raise RuntimeError(f"T8 metadata changed while loading: {key}")
    ...
    if prepare_inputs_calls != 8:
        raise RuntimeError(
            f"T8 prepare_inputs call count differs: {prepare_inputs_calls}"
        )
    eligible_manifest = eligible_sample_manifest
    live_manifest = _eligible_sample_manifest(dataset_roots)
    if live_manifest != eligible_manifest:
        raise RuntimeError(
            "T8 live eligible manifest changed during sample preparation"
        )
    ...
    for label, prepared in prepared_by_label.items():
        for key, value in prepared.items():
            ...
            if previous is not None and previous[0] != label:
                raise RuntimeError(
                    "T8 prepared tensors alias across samples: "
    ...
    if (
        ...
        raise RuntimeError("T8 same-task context pairing differs")

    model_groups = trainer._param_groups()
    if len(model_groups) != 2 or any(
        float(group["lr"]) != 1.0e-4 for group in model_groups
    ):
        raise RuntimeError("T8 optimizer group/LR structure differs")
    ...
    if len(parameter_ids) != len(set(parameter_ids)) or set(parameter_ids) != {
        ...
        raise RuntimeError("T8 optimizer parameters differ from trainables")
    optimizer_groups, master_pairs = trainer._optimizer_param_groups(model_groups)
    if len(master_pairs) != len(params):
        raise RuntimeError("T8 FP32 master coverage differs")
    if [id(model) for model, _master in master_pairs] != parameter_ids:
        raise RuntimeError("T8 FP32 master/model ordering differs")
    ...
    if (
        ...
        raise RuntimeError("T8 FP32 master identity/dtype differs")
    if [
        id(parameter) for group in optimizer_groups for parameter in group["params"]
    ] != initial_master_ids:
        raise RuntimeError("T8 optimizer groups do not contain the FP32 masters")
    if [len(group["params"]) for group in optimizer_groups] != [
        ...
        raise RuntimeError("T8 FP32 master optimizer group structure differs")
    named_masters = [
    ...
    def probe_mutation_snapshot() -> dict[str, Any]:
        if any(parameter.grad is not None for _name, parameter in named_trainables):
            raise RuntimeError("T8 update-free probe observed model gradients")
        if any(parameter.grad is not None for _name, parameter in named_frozen):
            raise RuntimeError("T8 update-free probe observed frozen gradients")
        if any(master.grad is not None for _name, master in named_masters):
            raise RuntimeError("T8 update-free probe observed master gradients")
        optimizer_state = []
        ...
        *, backward: bool, forward_inputs: dict[str, Any], recipe_seed: int
        ...
        with _fixed_rng_recipe(
            ...
            if (
                ...
                raise RuntimeError("T8 produced a non-finite scalar loss")
            scalars = {
            ...
            if abs(scalars["loss"] - scalars["loss_action"]) > tolerance:
                raise RuntimeError("T8 total/action loss identity differs")
            signature = _sha256_json(recipe_capture)
            if not recipe_capture["action"] or not recipe_capture["video"]:
                raise RuntimeError("T8 stochastic recipe capture is incomplete")
            if backward:
                loss.backward()
                backward_calls += 1
        ...
        if (
            ...
            raise RuntimeError("T8 fixed recipe did not restore caller RNG state")
    ...
    try:
        ...
        if optimizer.state:
            raise RuntimeError("T8 optimizer state exists before pre probes")
        cyclic_pre_state = probe_mutation_snapshot()
        for label in T8_TRAIN_LABELS:
        ...
        if probe_mutation_snapshot() != cyclic_pre_state or optimizer.state:
            raise RuntimeError("T8 cyclic training pre probes mutated training state")
        fresh_pre_state = probe_mutation_snapshot()
        for label in T8_HELDOUT_LABELS:
            scalars, signature = fixed_forward(
                backward=False,
        ...
        if probe_mutation_snapshot() != fresh_pre_state or optimizer.state:
            raise RuntimeError("T8 fresh held-out pre probes mutated training state")
        ...
        for step_index in range(T8_UPDATE_STEPS):
            ...
            if step_index == 0 and step_scalars != measurement_before[update_label]:
                raise RuntimeError("T8 fixed initial loss recipe is not reproducible")
            ...
            for name, parameter in named_trainables:
                ...
                if not bool(torch.isfinite(gradient).all().item()):
                    raise RuntimeError(
                        f"T8 non-finite gradient at step {step_index + 1}: {name}"
                    )
            ...
            if missing_gradients:
                raise RuntimeError(
                    f"T8 missing gradients at step {step_index + 1}: "
                    f"{missing_gradients[:8]!r}"
                )
            if nonzero_gradient_roots != set(EXPECTED_TRAINABLE_ROOTS):
                raise RuntimeError(
                    f"T8 nonzero-gradient roots differ at step {step_index + 1}: "
                    f"{sorted(nonzero_gradient_roots)!r}"
                )
            if any(parameter.grad is not None for _name, parameter in named_frozen):
                raise RuntimeError("T8 frozen parameters received gradients")

            grad_norm = torch.nn.utils.clip_grad_norm_(
                params, float(cfg.training.grad_clip)
            )
            if not bool(torch.isfinite(grad_norm).item()):
                raise RuntimeError("T8 gradient norm is non-finite")
            ...
            if step_index == 0:
                ...
                if invalid_master_gradients:
                    raise RuntimeError(
                        "T8 first-step masters lack finite nonzero FP32 gradients: "
                ...
                if {row["name"] for row in master_probes} != {
                    ...
                    raise RuntimeError("T8 first-step master probe coverage differs")
                ...
                for root in EXPECTED_TRAINABLE_ROOTS:
                    ...
                    if len(candidates) != 4:
                        raise RuntimeError(f"T8 lacks four update sentinels for {root}")
            ...
            if step_index == 0:
                ...
                if len(changed_master_ids) != len(named_masters):
                    raise RuntimeError("T8 first step did not update every FP32 master")
            ...
            for before, probe in zip(sentinel_before, update_sentinels, strict=True):
                ...
                if not all(
                    ...
                    raise RuntimeError("T8 update sentinel became non-finite")
            ...
            if changed_master_sentinel_roots != set(EXPECTED_TRAINABLE_ROOTS):
                raise RuntimeError(
                    "T8 sampled FP32 master update roots differ at step "
                    f"{step_index + 1}: "
            ...
            if sentinel_adam_steps != {float(step_index + 1)}:
                raise RuntimeError(
                    "T8 sampled AdamW step counters differ at step "
            ...
            if projection_mismatches:
                raise RuntimeError(
                    f"T8 BF16 projection differs: {projection_mismatches[:8]!r}"
                )
            projection_exact_steps.append(step_index + 1)
            if not _optimizer_state_is_finite(optimizer):
                raise RuntimeError("T8 AdamW state is non-finite")
            if step_index == 0:
            ...
            per_step.append(
                {
                    ...
                    "cycle": step_index // len(T8_TRAIN_LABELS) + 1,
                    "cycle_slot": step_index % len(T8_TRAIN_LABELS),
            ...
            phase_labels = T8_PHASE_PROBE_STEPS.get(step_index + 1)
            if phase_labels is not None:
                if phase_labels[0] != update_label:
                    raise RuntimeError("T8 fifth-update phase label differs")
                optimizer.zero_grad(set_to_none=True)
                for parameter in params:
                    parameter.grad = None
                ...
                for phase_label in phase_labels:
                    phase_inputs = (
                        ...
                        if phase_label in T8_TRAIN_LABELS
                ...
                if probe_mutation_snapshot() != phase_probe_state:
                    raise RuntimeError("T8 own-fifth-update phase probes mutated state")
        ...
        if probe_mutation_snapshot() != cyclic_post_state:
            raise RuntimeError("T8 cyclic training post probes mutated training state")
        ...
        if probe_mutation_snapshot() != fresh_post_state:
            raise RuntimeError("T8 fresh held-out post probes mutated training state")
    finally:
    ...
    if architecture_forward_calls != 44:
        raise RuntimeError(
            f"T8 architecture forward count differs: {architecture_forward_calls}"
        )
    if backward_calls != 20:
        raise RuntimeError(f"T8 backward call count differs: {backward_calls}")
    if optimizer_step_calls != 20:
        raise RuntimeError(
            f"T8 optimizer step call count differs: {optimizer_step_calls}"
        )
    if (
        len(update_recipe_signatures) != T8_UPDATE_STEPS
        or len(set(update_recipe_signatures)) != 1
    ):
        raise RuntimeError("T8 update RNG recipe signatures differ across forwards")
    ...
    if invalid_measurement_signatures:
        raise RuntimeError(
            "T8 measurement RNG recipe signatures differ: "
            f"{invalid_measurement_signatures!r}"
        )
    if set(own_fifth_update_measurements) != set(role_order) or set(
        phase_probe_signatures
    ) != set(role_order):
        raise RuntimeError("T8 own-fifth-update phase probe coverage differs")
    if (
        own_fifth_update_measurements["A0"] != measurement_after["A0"]
        or own_fifth_update_measurements["H0"] != measurement_after["H0"]
    ):
        raise RuntimeError("T8 step-20 phase/terminal repeatability differs")
    ...
    if len(all_recipe_signatures) != 44 or set(all_recipe_signatures) != {
        ...
        raise RuntimeError("T8 fixed loss recipe signature differs across forwards")
    expected_exposures = {
        **{label: 5 for label in T8_TRAIN_LABELS},
        **{label: 0 for label in T8_HELDOUT_LABELS},
    }
    if update_exposure_counts != expected_exposures:
        raise RuntimeError(
            f"T8 cyclic update exposure counts differ: {update_exposure_counts!r}"
        )
    if projection_exact_steps != list(range(1, T8_UPDATE_STEPS + 1)):
        raise RuntimeError("T8 per-step full projection evidence differs")
    ...
    if final_optimizer_master_ids != initial_master_ids:
        raise RuntimeError("T8 FP32 master objects changed during the loop")
    if prepared_input_snapshot() != initial_prepared_input_snapshot:
        raise RuntimeError("T8 prepared input identities or versions changed")
    ...
    if (
        ...
        raise RuntimeError("T8 architecture buffers changed during the update loop")
    ...
    if frozen_version_changes:
        raise RuntimeError(
            f"T8 frozen parameters changed: {frozen_version_changes[:8]!r}"
        )
    if any(not bool(torch.isfinite(master).all().item()) for master in masters):
        raise RuntimeError("T8 FP32 masters became non-finite")
    if any(not bool(torch.isfinite(parameter).all().item()) for parameter in params):
        raise RuntimeError("T8 BF16 trainables became non-finite")
    if not _optimizer_state_is_finite(optimizer):
        raise RuntimeError("T8 final AdamW state is non-finite")
    adam_steps: list[float] = []
    for master in masters:
        state = optimizer.state.get(master)
        if not state or "step" not in state:
            raise RuntimeError("T8 AdamW state is missing for an FP32 master")
        step_value = _tensor_scalar(state["step"])
        if not math.isfinite(step_value):
            raise RuntimeError("T8 AdamW step counter is non-finite")
        adam_steps.append(step_value)
    if set(adam_steps) != {float(T8_UPDATE_STEPS)}:
        raise RuntimeError(
            f"T8 AdamW step counters differ: {sorted(set(adam_steps))!r}"
        )
    ...
    if len(cumulative_master_ids) != len(named_masters) or set(
        ...
        raise RuntimeError("T8 cumulative FP32 master update coverage differs")

    four_suite_cyclic = _four_suite_cyclic_summary(
        {label: measurement_before[label]["loss_action"] for label in T8_TRAIN_LABELS},
        {label: measurement_after[label]["loss_action"] for label in T8_TRAIN_LABELS},
        {
            label: measurement_before[label]["loss_action"]
            for label in T8_HELDOUT_LABELS
        },
        {label: measurement_after[label]["loss_action"] for label in T8_HELDOUT_LABELS},
    )
    ...
    terminal_recency = _terminal_recency_summary(
        ...
        joint_viable=four_suite_cyclic["gate"],
    )
    verdict = terminal_recency["typed_verdict"]
    if terminal_recency["terminal_recency_supported"]:
        verdict_reasons = [
            "the frozen Spatial rescue, overwrite, and recency-rank predicates passed"
        ]
    elif terminal_recency["suite_effect_supported"]:
        verdict_reasons = [
            "the frozen suite-rank and double-Spatial-regression predicates passed"
        ]
    else:
        verdict_reasons = ["neither frozen primary classification predicate passed"]
    sample_by_label = {
        row["label"]: row
        for row in (*T8_CYCLIC_TRAIN_SAMPLES, *T8_FRESH_HELDOUT_SAMPLES)
    }
    ...
    four_suite_cyclic_report = {
        **four_suite_cyclic,
        ...
        "schedule": list(T8_PHASE_ROTATED_LABELS) * T8_CYCLE_COUNT,
    ...
    cyclic_schedule_report = {
        "cycle_count": T8_CYCLE_COUNT,
        "labels": list(T8_PHASE_ROTATED_LABELS) * T8_CYCLE_COUNT,
        "phase_probe_steps": {
            str(step): list(labels)
            for step, labels in sorted(T8_PHASE_PROBE_STEPS.items())
        },
        "updates_per_sample": T8_UPDATES_PER_SAMPLE,
    }
    phase_step_by_label = {
        label: step for step, labels in T8_PHASE_PROBE_STEPS.items() for label in labels
    }
    ...
    phase_rotated_recency_report = {
        **terminal_recency,
        "a0_h0_phase_terminal_reproduction": {
            label: {
                "action_equal": (
    ...
    randomness_report = {
        ...
        "eligible_same_task_probe_manifest_sha256": T8_ELIGIBLE_MANIFEST_SHA256,
        ...
        "selected_same_task_probe_manifest_sha256": T8_SELECTION_MANIFEST_SHA256,
    ...
    sample_roles_report = {
        label: {
            ...
            "role": "update" if label in T8_TRAIN_LABELS else "fresh_same_task_probe",
    ...
    scope_report = {
        ...
        "probe_order": [
            ...
            "updates_A1_A2_A3_A0_repeated_four_cycles",
            "cycle5_updates_with_post_update_A1H1_A2H2_A3H3_A0H0_phase_probes",
    ...
    starting_state_reproduction = {
        ...
        "current_schedule_is_t7_cyclic_phase_rotation": True,
        ...
        "predecessor_result_sha256": T7_PREDECESSOR_RESULT_SHA256,
        "reproduced": True,
        "starting_action_losses_within_frozen_t7_tolerance": True,
    }
    report = {
        ...
        "per_step": per_step,
        "phase_rotated_recency": phase_rotated_recency_report,
    ...
    if any(run_root.iterdir()):
        raise RuntimeError("T8 run root was not empty before terminal write")
