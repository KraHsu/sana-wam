#!/usr/bin/env python
"""Run the bounded LIBERO T14 initialization-seed accumulation-8 replication."""
...
T14_MACRO_STEPS = 20
T14_UPDATE_STEPS = T14_MACRO_STEPS
T14_FORWARD_EXPOSURES_PER_SAMPLE = T14_MACRO_STEPS
T14_LOSS_WEIGHT_PER_SAMPLE_PER_MACRO = 0.125
T14_CUMULATIVE_LOSS_WEIGHT_PER_SAMPLE = 2.5
T14_CUMULATIVE_LOSS_WEIGHT_PER_SUITE = 5.0
T14_TRAINING_FORWARDS = T14_MACRO_STEPS * 8
T14_MEASUREMENT_FORWARDS = 32
T14_ARCHITECTURE_FORWARDS = T14_TRAINING_FORWARDS + T14_MEASUREMENT_FORWARDS
T14_EXECUTION_VERDICT = "T14_INITSEED_REPLICATION_ACCUM8_JOINT_ARM_VALID"
T14_REPLICATED_VERDICT = "T14_INITSEED_REPLICATION_ACCUM8_BALANCED_JOINT_REPLICATED"
T14_TRAINING_FIT_FAILURE_VERDICT = (
    "T14_INITSEED_REPLICATION_ACCUM8_JOINT_TRAINING_FIT_FAILURE"
)
T14_HELDOUT_GAP_VERDICT = "T14_INITSEED_REPLICATION_ACCUM8_JOINT_HELDOUT_GAP"
T14_ROOT_SLUG = "libero-t14-initseed-replication-accum8-balanced-joint-fixed20"
T14_INITIALIZATION_SEED = 20260807
T14_DATALOADER_SEED = 20260806
T14_LOSS_RECIPE_SEED = 20260826
T14_CONFIG_SHA256 = "36daaada09409c22ef2e57e3382ca2ef28c6847f9d2e043cad354464c3dde929"
T14_CONFIG_RELATIVE_PATH = (
...
T13_FIXED_RECIPE_SIGNATURE_SHA256 = (
...
def _read_jsonl(path: Path) -> list[dict[str, Any]]:
    if path.is_symlink() or not path.is_file():
        raise RuntimeError(
            f"T14 inherited T13 metadata must be a regular non-symlink file: {path}"
        )
    rows = []
    for line_number, line in enumerate(
        ...
        if not line:
            raise RuntimeError(
                f"T14 inherited T13 metadata contains a blank row: {path}:{line_number}"
            )
        row = json.loads(line)
        if not isinstance(row, dict):
            raise RuntimeError(
                f"T14 inherited T13 metadata row is not an object: {path}:{line_number}"
            )
        rows.append(row)
    return rows
...
def _multi_episode_sample_summary(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("T14 frozen labels differ")
    sample_by_label = {row["label"]: row for row in samples}
    if set(sample_by_label) != expected_labels:
        raise ValueError("T14 sample labels differ from the frozen set")
    rows = []
    for label in labels:
...
def _require_strictly_positive_finite(value: float, *, field: str) -> float:
    ...
    if not math.isfinite(observed) or observed <= 0.0:
        raise ValueError(f"T14 {field} must be finite and strictly positive")
    return observed
...
def _no_intra_macro_mutation_evidence(
    ...
    *,
    ...
    if macro_step < 1 or macro_step > T14_MACRO_STEPS:
        raise ValueError("T14 no-mutation evidence macro step is outside 1..20")
    if set(before) != set(components) or set(after_microbacks) != set(components):
        raise ValueError("T14 no-mutation evidence component schema differs")
    component_unchanged = {
    ...
    if not state_unchanged:
        ...
        raise RuntimeError(
            "T14 training state mutated within macro before optimizer step "
...
def _multi_episode_summary(
    ...
    if set(update_by_dataset) != set(T13_SUITE_ORDER) or set(heldout_by_dataset) != set(
        ...
        raise ValueError("T14 suite coverage differs from the frozen order")
    per_suite = []
    for dataset in T13_SUITE_ORDER:
        ...
        if len(update_rows) != 2 or len(heldout_rows) != 2:
            raise ValueError(
                f"T14 suite must retain two update and two heldout rows: {dataset}"
            )
        ...
        t13_q = float(T13_Q_BY_SUITE[dataset])
        per_suite.append(
            {
                ...
                "q": q,
                "q_minus_t13_q": q - t13_q,
                "q_strictly_below_t13_q": q < t13_q,
                "t13_q": t13_q,
    ...
    if all_update_improved and all_heldout_improved:
        scientific_verdict = T14_REPLICATED_VERDICT
    elif not all_update_improved:
        scientific_verdict = T14_TRAINING_FIT_FAILURE_VERDICT
    else:
        scientific_verdict = T14_HELDOUT_GAP_VERDICT
    return {
            ...
            "per_suite": [
                {
                    ...
                    for key in (
                        ...
                        "q",
                        "q_minus_t13_q",
                        "q_strictly_below_t13_q",
                        "t13_q",
                    )
                }
                for row in per_suite
            ],
            "q_strictly_below_t13_count": sum(
                row["q_strictly_below_t13_q"] for row in per_suite
            ),
        },
...
def _loss_weights_for_macro(step_index: int) -> dict[str, float]:
    if step_index < 0 or step_index >= T14_UPDATE_STEPS:
        raise ValueError("T14 step index is outside the frozen 20-step schedule")
    return {label: T14_LOSS_WEIGHT_PER_SAMPLE_PER_MACRO for label in T13_TRAIN_LABELS}
...
def _training_core_projection(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if len(per_step) != T14_UPDATE_STEPS:
        raise ValueError("T14/T3 training-core view requires exactly 20 step rows")
    return {
    ...
    if (
        ...
        _write_terminal_report(
            marker,
            {
                ...
                "result": "FAIL",
                "schema_version": "sana-wam-libero-t14-initseed-replication-arm-failure-v1",
            },
        )
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=ROOT / T14_CONFIG_RELATIVE_PATH)
    ...
    parser.add_argument("--steps", type=int, default=T14_UPDATE_STEPS)
    parser.add_argument(
        "--initialization-seed", type=int, default=T14_INITIALIZATION_SEED
    )
    parser.add_argument("--loss-recipe-seed", type=int, default=T14_LOSS_RECIPE_SEED)
    ...
    if args.steps != T14_UPDATE_STEPS:
        raise ValueError(f"T14 steps must be exactly {T14_UPDATE_STEPS}")
    if args.initialization_seed != T14_INITIALIZATION_SEED:
        raise ValueError("T14 initialization seed differs")
    if args.loss_recipe_seed != T14_LOSS_RECIPE_SEED:
        raise ValueError("T14 loss recipe seed differs")
    if len(args.nonce) != 32 or any(
        character not in "0123456789abcdef" for character in args.nonce
    ):
        raise ValueError("T14 nonce must be exactly 32 lowercase hex characters")
    expected_root_name = f"{T14_ROOT_SLUG}-{args.nonce}"
    if args.run_root.name != expected_root_name:
        raise ValueError("T14 run-root basename does not bind the nonce")
    if args.run_root.parent.name != args.expected_repo_commit[:12]:
        raise ValueError("T14 run-root parent does not bind the source commit")
    expected_run_root = (
        T14_RUN_NAMESPACE / args.expected_repo_commit[:12] / expected_root_name
    )
    if args.run_root.expanduser() != expected_run_root:
        raise ValueError(
            "T14 run root differs from the frozen non-formal namespace: "
    ...
    expected_config_path = (ROOT / T14_CONFIG_RELATIVE_PATH).resolve()
    if config_path != expected_config_path:
        raise ValueError(
            f"T14 config path differs: expected={expected_config_path} got={config_path}"
        )
    ...
    if actual_config_sha256 != T14_CONFIG_SHA256:
        raise RuntimeError("T14 config content differs from the frozen pin")
    actual_runner_sha256 = _sha256_file(runner_path)
    expected_identity = {
        "config_sha256": T14_CONFIG_SHA256,
    ...
    if cfg.model.architecture.variant != "autoregressive":
        raise RuntimeError("T14 requires the AR architecture")
    if cfg.model.video_backbone.continuous_timestep_conditioning is not True:
        raise RuntimeError("T14 requires continuous FP32 video timesteps")
    if cfg.training.use_gradient_checkpointing is not True:
        raise RuntimeError("T14 requires gradient checkpointing")
    if int(cfg.training.batch_size) != 1:
        raise RuntimeError("T14 requires production singleton batch_size=1")
    if int(cfg.training.gradient_accumulation_steps) != 8:
        raise RuntimeError("T14 requires production gradient_accumulation_steps=8")
    if int(cfg.training.seed) != args.initialization_seed:
        raise RuntimeError("T14 CLI initialization seed must equal training.seed")
    if int(cfg.dataloader.seed) != T14_DATALOADER_SEED:
        raise RuntimeError("T14 dataloader seed must remain the frozen T14 value")
    if tuple(cfg.training.trainable_modules) != EXPECTED_TRAINABLE_ROOTS:
        raise RuntimeError("T14 trainable module allowlist differs")
    if (
        tuple(cfg.training.preserve_frozen_input_grad_modules)
        != EXPECTED_PRESERVE_FROZEN_INPUT_GRAD_MODULES
    ):
        raise RuntimeError("T14 frozen input-gradient preservation differs")
    if cfg.training.optimizer_master_weights is not True:
        raise RuntimeError("T14 requires persistent FP32 optimizer masters")
    if (
        float(cfg.training.lambda_video) != 0.0
        or float(cfg.training.lambda_action) != 1.0
    ):
        raise RuntimeError("T14 requires action-only loss weighting")
    if float(cfg.training.weight_decay) != 0.0:
        raise RuntimeError("T14 requires weight_decay=0")
    if (
        float(cfg.training.action_lr) != 1.0e-4
        or float(cfg.training.video_lr) != 1.0e-4
    ):
        raise RuntimeError("T14 requires fixed base learning rates of 1e-4")
    if float(cfg.training.grad_clip) != 1.0:
        raise RuntimeError("T14 requires the fixed gradient clip bound 1.0")
    ...
    for path in forbidden_checkpoint_fields:
        if OmegaConf.select(cfg, path, default=None) is not None:
            raise RuntimeError(f"T14 forbids checkpoint field {path}")
    if str(cfg.training.action_stats_sha256) != SELECTED_STATS_SHA256:
        raise RuntimeError("T14 selected-row stats SHA pin differs")
    if str(cfg.training.action_stats_population_sha256) != SELECTED_POPULATION_SHA256:
        raise RuntimeError("T14 selected population SHA pin differs")
    stats_path = Path(str(cfg.dataloader.action_stats_path)).expanduser()
    if stats_path.is_symlink() or not stats_path.is_file():
        raise RuntimeError("T14 stats artifact must be a regular non-symlink file")
    if _sha256_file(stats_path) != SELECTED_STATS_SHA256:
        raise RuntimeError("T14 selected-row stats artifact SHA differs")
    ...
    if (
        ...
        raise RuntimeError("T14 four-suite dataset root identity differs")
    ...
    if (
        ...
        raise RuntimeError("T14 predecessor T2 evidence semantics differ")
    ...
    if (
        ...
        or t3_predecessor.get("scope", {}).get("backward_calls") != T14_UPDATE_STEPS
        or t3_predecessor.get("scope", {}).get("optimizer_steps") != T14_UPDATE_STEPS
    ):
        raise RuntimeError("T14 predecessor T3 evidence semantics differ")
    ...
    if (
        ...
        or t4_predecessor.get("scope", {}).get("backward_calls") != T14_UPDATE_STEPS
        or t4_predecessor.get("scope", {}).get("optimizer_steps") != T14_UPDATE_STEPS
    ):
        raise RuntimeError("T14 predecessor T4 evidence semantics differ")
    ...
    if (
        ...
        or t5_predecessor.get("scope", {}).get("backward_calls") != T14_UPDATE_STEPS
        or t5_predecessor.get("scope", {}).get("optimizer_steps") != T14_UPDATE_STEPS
    ):
        raise RuntimeError("T14 predecessor T5 evidence semantics differ")
    ...
    if (
        ...
        or t6_predecessor.get("scope", {}).get("backward_calls") != T14_UPDATE_STEPS
        or t6_predecessor.get("scope", {}).get("optimizer_steps") != T14_UPDATE_STEPS
    ):
        raise RuntimeError("T14 predecessor T6 evidence semantics differ")
    ...
    if (
        ...
        raise RuntimeError("T14 frozen T3/T4/T5/T6 training-core evidence differs")
    ...
    if (
        ...
        raise RuntimeError("T14 predecessor T7 evidence semantics differ")
    ...
    if (
        ...
        raise RuntimeError("T14 direct predecessor T9 aggregate semantics differ")
    t10_arm_results = {
    ...
    for arm, predecessor in t10_arm_results.items():
        if (
            ...
            raise RuntimeError(f"T14 predecessor T10 {arm} arm semantics differ")
    ...
    if (
        ...
        raise RuntimeError("T14 underlying predecessor T10 aggregate semantics differ")
    if (
        ...
        raise RuntimeError("T14 underlying predecessor T11 terminal root differs")
    ...
    if (
        ...
        raise RuntimeError("T14 underlying predecessor T11 semantics differ")
    if (
        ...
        raise RuntimeError("T14 underlying predecessor T12 terminal root differs")
    ...
    if (
        ...
        raise RuntimeError("T14 underlying predecessor T12 semantics differ")
    if (
        ...
        raise RuntimeError("T14 direct predecessor T13 terminal root differs")
    ...
    if (
        ...
        raise RuntimeError("T14 direct predecessor T13 semantics differ")
    ...
    if "H200" not in gpu_before["name"].upper():
        raise RuntimeError(f"T14 selected GPU is not an H200: {gpu_before['name']}")
    ...
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError(
            "T14 requires exactly one visible CUDA GPU; "
    ...
    if any(parameter.requires_grad for parameter in video_backbone.parameters()):
        raise RuntimeError("T14 video-backbone parameters are not frozen")
    named_trainables = [
        (name, parameter)
    ...
    if observed_roots != tuple(sorted(EXPECTED_TRAINABLE_ROOTS)):
        raise RuntimeError(f"T14 trainable parameter roots differ: {observed_roots!r}")
    if len(named_trainables) != EXPECTED_TRAINABLE_TENSOR_COUNT:
        raise RuntimeError("T14 trainable tensor count differs")
    if sum(parameter.numel() for _name, parameter in named_trainables) != (
        EXPECTED_TRAINABLE_PARAMETER_COUNT
    ):
        raise RuntimeError("T14 trainable parameter count differs")
    ...
    for label in role_order:
        ...
        if (
            ...
            raise RuntimeError(f"T14 LIBERO sample metadata differs: {label}")
        ...
        for relative_path, expected_sha256 in sorted(selected["assets"].items()):
            ...
            if external_assets[key] != observed:
                raise RuntimeError(f"T14 role asset changed while loading: {key}")
            role_assets_after_sample[key] = observed
    if _sha256_file(stats_path) != SELECTED_STATS_SHA256:
        raise RuntimeError("T14 selected-row stats changed while loading")
    ...
    if any(module.training for module in video_backbone.modules()):
        raise RuntimeError("T14 preserved video backbone left eval mode")
    ...
    def prepare_one(label: str, raw_sample: dict[str, Any]) -> dict[str, Any]:
        ...
        if prepared.get("use_gradient_checkpointing") is not True:
            raise RuntimeError(f"T14 {label} inputs dropped gradient checkpointing")
        if prepared.get("use_gradient_checkpointing_offload") is not False:
            raise RuntimeError("T14 requires checkpoint offload=false")
        invalid_tensors = [
            key
        ...
        if invalid_tensors:
            raise RuntimeError(
                f"T14 {label} prepared tensors retain autograd state: "
                f"{invalid_tensors!r}"
            )
        return prepared
    ...
    for dataset_name in T13_SUITE_ORDER:
        for filename, expected_sha256 in sorted(
            ...
            if external_assets[key] != observed:
                raise RuntimeError(f"T14 metadata changed while loading: {key}")
    ...
    if prepare_inputs_calls != 16:
        raise RuntimeError(
            f"T14 prepare_inputs call count differs: {prepare_inputs_calls}"
        )
    eligible_manifest = eligible_sample_manifest
    live_manifest = _eligible_sample_manifest(dataset_roots)
    if live_manifest != eligible_manifest:
        raise RuntimeError(
            "T14 live eligible manifest changed during sample preparation"
        )
    ...
    for label, prepared in prepared_by_label.items():
        for key, value in prepared.items():
            ...
            if previous is not None and previous[0] != label:
                raise RuntimeError(
                    "T14 prepared tensors alias across samples: "
    ...
    if (
        ...
        raise RuntimeError("T14 per-task four-way context grouping differs")

    model_groups = trainer._param_groups()
    if len(model_groups) != 2 or any(
        float(group["lr"]) != 1.0e-4 for group in model_groups
    ):
        raise RuntimeError("T14 optimizer group/LR structure differs")
    ...
    if len(parameter_ids) != len(set(parameter_ids)) or set(parameter_ids) != {
        ...
        raise RuntimeError("T14 optimizer parameters differ from trainables")
    optimizer_groups, master_pairs = trainer._optimizer_param_groups(model_groups)
    if len(master_pairs) != len(params):
        raise RuntimeError("T14 FP32 master coverage differs")
    if [id(model) for model, _master in master_pairs] != parameter_ids:
        raise RuntimeError("T14 FP32 master/model ordering differs")
    ...
    if (
        ...
        raise RuntimeError("T14 FP32 master identity/dtype differs")
    if [
        id(parameter) for group in optimizer_groups for parameter in group["params"]
    ] != initial_master_ids:
        raise RuntimeError("T14 optimizer groups do not contain the FP32 masters")
    if [len(group["params"]) for group in optimizer_groups] != [
        ...
        raise RuntimeError("T14 FP32 master optimizer group structure differs")
    ...
    def probe_mutation_snapshot() -> dict[str, Any]:
        if any(parameter.grad is not None for _name, parameter in named_trainables):
            raise RuntimeError("T14 update-free probe observed model gradients")
        if any(parameter.grad is not None for _name, parameter in named_frozen):
            raise RuntimeError("T14 update-free probe observed frozen gradients")
        if any(master.grad is not None for _name, master in named_masters):
            raise RuntimeError("T14 update-free probe observed master gradients")
        optimizer_state = []
        for name, master in named_masters:
            for key, value in sorted(optimizer.state.get(master, {}).items()):
                if isinstance(value, torch.Tensor):
        ...
        *,
        ...
        with _fixed_rng_recipe(
            ...
            if (
                ...
                raise RuntimeError(
                    "T14 produced a scalar loss that is not finite and strictly "
                    "positive"
                )
            scalars = {
            ...
            if abs(scalars["loss"] - scalars["loss_action"]) > tolerance:
                raise RuntimeError("T14 total/action loss identity differs")
            signature = _sha256_json(recipe_capture)
            if not recipe_capture["action"] or not recipe_capture["video"]:
                raise RuntimeError("T14 stochastic recipe capture is incomplete")
            if backward_scale is not None:
                if backward_scale not in {
                    0.0,
                    T14_LOSS_WEIGHT_PER_SAMPLE_PER_MACRO,
                    1.0,
                }:
                    raise RuntimeError("T14 backward scale differs from matched core")
        ...
        if (
            ...
            raise RuntimeError("T14 fixed recipe did not restore caller RNG state")
    ...
    try:
        ...
        if optimizer.state:
            raise RuntimeError("T14 optimizer state exists before pre probes")
        ...
        if probe_mutation_snapshot() != cyclic_pre_state or optimizer.state:
            raise RuntimeError("T14 cyclic training pre probes mutated training state")
        ...
        if probe_mutation_snapshot() != fresh_pre_state or optimizer.state:
            raise RuntimeError("T14 fresh held-out pre probes mutated training state")
        for step_index in range(T14_UPDATE_STEPS):
            ...
            if set(step_loss_weights) != set(T13_TRAIN_LABELS) or not math.isclose(
                ...
                raise RuntimeError(
                    "T14 per-macro loss weights are not exactly unit-sum"
                )
            for update_label in T13_TRAIN_LABELS:
                ...
                if step_index == 0:
                    ...
                    if abs(step_scalars["loss_action"] - expected) > tolerance:
                        raise RuntimeError(
                            "T14 fixed initial loss recipe is not reproducible "
                            f"for {update_label}"
                        )
            ...
            if abs(joint_loss - joint_action_loss) > joint_tolerance:
                raise RuntimeError("T14 joint total/action loss identity differs")
            ...
            for name, parameter in named_trainables:
                ...
                if not bool(torch.isfinite(gradient).all().item()):
                    raise RuntimeError(
                        f"T14 non-finite gradient at step {step_index + 1}: {name}"
                    )
            ...
            if missing_gradients:
                raise RuntimeError(
                    f"T14 missing gradients at step {step_index + 1}: "
                    f"{missing_gradients[:8]!r}"
                )
            if nonzero_gradient_roots != set(EXPECTED_TRAINABLE_ROOTS):
                raise RuntimeError(
                    f"T14 nonzero-gradient roots differ at step {step_index + 1}: "
                    f"{sorted(nonzero_gradient_roots)!r}"
                )
            if any(parameter.grad is not None for _name, parameter in named_frozen):
                raise RuntimeError("T14 frozen parameters received gradients")

            grad_norm = torch.nn.utils.clip_grad_norm_(
                params, float(cfg.training.grad_clip)
            )
            if not bool(torch.isfinite(grad_norm).item()):
                raise RuntimeError("T14 gradient norm is non-finite")
            ...
            if step_index == 0:
                ...
                if invalid_master_gradients:
                    raise RuntimeError(
                        "T14 first-step masters lack finite nonzero FP32 gradients: "
                ...
                if {row["name"] for row in master_probes} != {
                    ...
                    raise RuntimeError("T14 first-step master probe coverage differs")
                ...
                for root in EXPECTED_TRAINABLE_ROOTS:
                    ...
                    if len(candidates) != 4:
                        raise RuntimeError(
                            f"T14 lacks four update sentinels for {root}"
                        )
            ...
            if step_index == 0:
                ...
                if len(changed_master_ids) != len(named_masters):
                    raise RuntimeError(
                        "T14 first step did not update every FP32 master"
                    )
            ...
            for before, probe in zip(sentinel_before, update_sentinels, strict=True):
                ...
                if not all(
                    ...
                    raise RuntimeError("T14 update sentinel became non-finite")
            ...
            if changed_master_sentinel_roots != set(EXPECTED_TRAINABLE_ROOTS):
                raise RuntimeError(
                    "T14 sampled FP32 master update roots differ at step "
                    f"{step_index + 1}: "
            ...
            if sentinel_adam_steps != {float(step_index + 1)}:
                raise RuntimeError(
                    "T14 sampled AdamW step counters differ at step "
            ...
            if projection_mismatches:
                raise RuntimeError(
                    f"T14 BF16 projection differs: {projection_mismatches[:8]!r}"
                )
            projection_exact_steps.append(step_index + 1)
            if not _optimizer_state_is_finite(optimizer):
                raise RuntimeError("T14 AdamW state is non-finite")
            if step_index == 0:
        ...
        if probe_mutation_snapshot() != cyclic_post_state:
            raise RuntimeError("T14 cyclic training post probes mutated training state")
        ...
        if probe_mutation_snapshot() != fresh_post_state:
            raise RuntimeError("T14 fresh held-out post probes mutated training state")
    finally:
    ...
    if architecture_forward_calls != T14_ARCHITECTURE_FORWARDS:
        raise RuntimeError(
            f"T14 architecture forward count differs: {architecture_forward_calls}"
        )
    if backward_calls != T14_TRAINING_FORWARDS:
        raise RuntimeError(f"T14 backward call count differs: {backward_calls}")
    if optimizer_step_calls != 20:
        raise RuntimeError(
            f"T14 optimizer step call count differs: {optimizer_step_calls}"
        )
    all_macro_states_unchanged = (
        len(per_macro_no_mutation_evidence) == T14_MACRO_STEPS
        and [row["macro_step"] for row in per_macro_no_mutation_evidence]
        == list(range(1, T14_MACRO_STEPS + 1))
        and all(
            row["state_unchanged"]
    ...
    if not all_macro_states_unchanged:
        raise RuntimeError("T14 no-intra-macro-mutation evidence coverage differs")
    if (
        len(update_recipe_signatures) != T14_TRAINING_FORWARDS
        or len(set(update_recipe_signatures)) != 1
    ):
        raise RuntimeError("T14 update RNG recipe signatures differ across forwards")
    ...
    if invalid_measurement_signatures:
        raise RuntimeError(
            "T14 measurement RNG recipe signatures differ: "
    ...
    if len(per_macro_recipe_signatures) != T14_MACRO_STEPS or any(
        set(signatures) != set(T13_TRAIN_LABELS)
        for signatures in per_macro_recipe_signatures
    ):
        raise RuntimeError("T14 per-macro recipe coverage differs")
    if per_macro_loss_weights != [
        _loss_weights_for_macro(step_index) for step_index in range(T14_MACRO_STEPS)
    ]:
        raise RuntimeError("T14 frozen matched-core loss-weight schedule differs")
    if len(all_recipe_signatures) != T14_ARCHITECTURE_FORWARDS or set(
        all_recipe_signatures
    ) != {T13_FIXED_RECIPE_SIGNATURE_SHA256}:
        raise RuntimeError("T14 fixed loss recipe signature differs across forwards")
    expected_exposures = {
        **{label: T14_FORWARD_EXPOSURES_PER_SAMPLE for label in T13_TRAIN_LABELS},
        **{label: 0 for label in T13_HELDOUT_LABELS},
    }
    if update_exposure_counts != expected_exposures:
        raise RuntimeError(
            f"T14 joint update exposure counts differ: {update_exposure_counts!r}"
        )
    if projection_exact_steps != list(range(1, T14_MACRO_STEPS + 1)):
        raise RuntimeError("T14 per-macro full projection evidence differs")
    expected_effective_weights = {
        **{label: T14_CUMULATIVE_LOSS_WEIGHT_PER_SAMPLE for label in T13_TRAIN_LABELS},
        **{label: 0.0 for label in T13_HELDOUT_LABELS},
    }
    if effective_weight_counts != expected_effective_weights:
        raise RuntimeError("T14 effective cumulative loss weights differ")
    ...
    if final_optimizer_master_ids != initial_master_ids:
        raise RuntimeError("T14 FP32 master objects changed during the loop")
    if prepared_input_snapshot() != initial_prepared_input_snapshot:
        raise RuntimeError("T14 prepared input identities or versions changed")
    ...
    if frozen_version_changes:
        raise RuntimeError(
            f"T14 frozen parameters changed: {frozen_version_changes[:8]!r}"
        )
    if any(not bool(torch.isfinite(master).all().item()) for master in masters):
        raise RuntimeError("T14 FP32 masters became non-finite")
    if any(not bool(torch.isfinite(parameter).all().item()) for parameter in params):
        raise RuntimeError("T14 BF16 trainables became non-finite")
    if not _optimizer_state_is_finite(optimizer):
        raise RuntimeError("T14 final AdamW state is non-finite")
    adam_steps: list[float] = []
    for master in masters:
        state = optimizer.state.get(master)
        if not state or "step" not in state:
            raise RuntimeError("T14 AdamW state is missing for an FP32 master")
        step_value = _tensor_scalar(state["step"])
        if not math.isfinite(step_value):
            raise RuntimeError("T14 AdamW step counter is non-finite")
        adam_steps.append(step_value)
    if set(adam_steps) != {float(T14_UPDATE_STEPS)}:
        raise RuntimeError(
            f"T14 AdamW step counters differ: {sorted(set(adam_steps))!r}"
        )
    ...
    if len(cumulative_master_ids) != len(named_masters) or set(
        ...
        raise RuntimeError("T14 cumulative FP32 master update coverage differs")
    ...
    matched_core_schedule_report = {
        ...
        "forward_exposures_per_sample": T14_FORWARD_EXPOSURES_PER_SAMPLE,
        "macro_steps": T14_MACRO_STEPS,
    ...
    execution_verdict = T14_EXECUTION_VERDICT
    scientific_verdict = classification_summary["scientific_verdict"]
    verdict_reasons = [
        "the frozen T14 multi-episode accumulation-8 balanced-JOINT arm completed validly",
    ...
    scope_report = {
        ...
        "effective_cumulative_loss_weight_per_suite": (
            T14_CUMULATIVE_LOSS_WEIGHT_PER_SUITE
        ),
        "effective_cumulative_loss_weight_per_update_sample": (
            T14_CUMULATIVE_LOSS_WEIGHT_PER_SAMPLE
        ),
    ...
    report = {
        ...
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
        "experimental_axis": {
            "changed_field": "training.seed",
            "predecessor_value": 20260806,
            "replication_value": T14_INITIALIZATION_SEED,
            "held_fixed": {
                "dataloader.seed": T14_DATALOADER_SEED,
                "loss_recipe_seed": T14_LOSS_RECIPE_SEED,
                "macro_steps": T14_MACRO_STEPS,
                "sample_count": 16,
                "selection_manifest_sha256": T13_SELECTION_MANIFEST_SHA256,
            },
            "only_scientific_axis": "fresh process/model/Trainer initialization seed",
        },
        "gpu": {
            "after": gpu_after,
            "before": gpu_before,
        ...
        "interpretation_limits": {
            ...
            "model_facing_task_definition": (
                "T14 deliberately reuses the exact T13 tasks and sixteen samples; "
        ...
        "schema_version": "sana-wam-libero-t14-initseed-replication-accum8-v1",
        "scope": scope_report,
    ...
    if any(run_root.iterdir()):
        raise RuntimeError("T14 run root was not empty before terminal write")
    ...
    try:
        ...
        if _ACTIVE_RUN_ROOT is not None:
            try:
                ...
                print(
                    "failed to terminalize T14 root: "
