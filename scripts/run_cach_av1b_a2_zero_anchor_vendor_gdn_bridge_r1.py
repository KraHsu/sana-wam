...
EXPECTED_CANONICAL_SSH_ALIAS = "H200"
EXPECTED_OBSERVED_HOSTNAME = "huaxiyun"
...
EXPECTED_DECISION_SHA256 = (
    "db51c2d2fca6b0327a1466595d7297630a39a6091dfc45bc2f9633b10d283c23"
)
EXPECTED_PLAN = (
...
EXPECTED_BRIDGE = "src/sana_wam/model/cach_av1b_a2_zero_anchor_vendor_gdn_bridge_r1.py"
EXPECTED_CONFIG = "configs/experiments/cach_av1b_a2_zero_anchor_vendor_gdn_bridge_r1.yaml"
EXPECTED_RUNNER = "scripts/run_cach_av1b_a2_zero_anchor_vendor_gdn_bridge_r1.py"
EXPECTED_TEST = "tests/test_cach_av1b_a2_zero_anchor_vendor_gdn_bridge_r1.py"
SOURCE_QUARTET = (EXPECTED_BRIDGE, EXPECTED_CONFIG, EXPECTED_RUNNER, EXPECTED_TEST)

EXPECTED_NONCE = "dc17cb7bdbd70955ea4b91212f0d4ac3"
EXPECTED_ROOT = Path(
...
EXPECTED_NAMESPACE_PATHS = (
    EXPECTED_NAMESPACE_BASE / "cach_a2_r1",
    EXPECTED_NAMESPACE_BASE / "cach_a2_r1" / "06f5d09127f8",
)
...
AUTHORITY_ENVELOPE_SCHEMA = "cach.cach_a2_r1.runtime_execution_authority.v1"
CONFIG_SCHEMA = "cach.cach_a2_r1.zero_anchor_vendor_gdn_bridge.config.v1"
...
def _validate_card(card_argument: Path) -> dict[str, Any]:
    ...
    _require(
        card.get("schema") == "cach.architecture_validation.cach_a2_r1_launcher_fix_run_card.v1",
        "AUTH_BLOCKED",
        "CARD_SCHEMA_MISMATCH",
        repr(card.get("schema")),
    )
    _require(
        card.get("card_id") == "cach-a2-r1-launcher-preflight-fix-v1",
        "AUTH_BLOCKED",
        "CARD_ID_MISMATCH",
        repr(card.get("card_id")),
    )
    _require(
        card.get("card_state") == "FROZEN_R1_LAUNCHER_FIX_SOURCE_AND_EXECUTION_AUTHORIZED",
        "AUTH_BLOCKED",
        "CARD_STATE_MISMATCH",
    ...
    _require(
        identity.get("canonical_ssh_alias") == EXPECTED_CANONICAL_SSH_ALIAS
        and identity.get("expected_platform_node") == EXPECTED_OBSERVED_HOSTNAME,
        "AUTH_BLOCKED",
        "CARD_HOST_IDENTITY_MISMATCH",
        repr(
            {
                "canonical_ssh_alias": identity.get("canonical_ssh_alias"),
                "expected_platform_node": identity.get("expected_platform_node"),
            }
        ),
    )
    _require(
...
def _validate_authority(
    ...
    _require(envelope.get("operation") == "EXECUTE_CACH_A2_R1_ONCE", "AUTH_BLOCKED", "AUTHORITY_OPERATION", repr(envelope.get("operation")))
    _require(
...
def _validate_config(
    ...
    _require(
        identity.get("canonical_ssh_alias") == EXPECTED_CANONICAL_SSH_ALIAS,
        "AUTH_BLOCKED",
        "CONFIG_CANONICAL_SSH_ALIAS",
        repr(identity.get("canonical_ssh_alias")),
    )
    _require(
    ...
    _require(
        runtime.get("canonical_ssh_alias") == EXPECTED_CANONICAL_SSH_ALIAS
        and runtime.get("expected_platform_node") == EXPECTED_OBSERVED_HOSTNAME,
        "AUTH_BLOCKED",
        "CONFIG_RUNTIME_HOST_IDENTITY",
        repr(
            {
                "canonical_ssh_alias": runtime.get("canonical_ssh_alias"),
                "expected_platform_node": runtime.get("expected_platform_node"),
            }
        ),
    )
...
def _validate_precreate_environment(
    ...
    expected_platform_node = config["runtime"]["expected_platform_node"]
    _require(
        expected_platform_node == EXPECTED_OBSERVED_HOSTNAME
        and platform.node().split(".")[0] == expected_platform_node,
        "ENV_BLOCKED",
        "HOST_MISMATCH",
        repr(
            {
                "canonical_ssh_alias": EXPECTED_CANONICAL_SSH_ALIAS,
                "expected_platform_node": expected_platform_node,
                "observed_platform_node": platform.node().split(".")[0],
            }
        ),
    )
    heads = {
    ...
    _require(
        torch_distribution == toolchain["torch"],
        "ENV_BLOCKED",
    ...
    return {
        ...
        "canonical_ssh_alias": EXPECTED_CANONICAL_SSH_ALIAS,
        "observed_hostname": platform.node().split(".")[0],
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        from sana_wam.model.cach_av1b_a2_zero_anchor_vendor_gdn_bridge_r1 import (
            run_cach_a2_screen,
        )
