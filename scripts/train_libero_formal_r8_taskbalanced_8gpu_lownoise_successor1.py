#!/usr/bin/env python
"""Fail-closed 8-GPU R8 warm-start task-balanced low-noise successor."""
...
CONFIG_RELATIVE_PATH = (
    "configs/experiments/libero_formal_r8_taskbalanced_8gpu_lownoise_successor1.yaml"
)
BASELINE_CONFIG_RELATIVE_PATH = (
    "configs/experiments/libero_formal_r8_taskbalanced_8gpu_successor1.yaml"
)
CONTROL_RUNNER_RELATIVE_PATH = (
    "scripts/train_libero_formal_r8_taskbalanced_8gpu_successor1.py"
)
RUN_NONCE = "709c702a6bc3bb797915942d0bb26572"
RUN_ROOT = Path(
    "/DATA/share/sana_wam_libero_training/"
    "formal_taskbalanced_r8_h8_ynorm_fp32attn_lownoise/"
    "libero-ar-r8-taskbalanced-lownoise-causal1-warmstart-8gpu-4340-"
    + RUN_NONCE
)
...
CONTROL_REPO_COMMIT = "25aa41f13000c458b36e9d5bf28fbf0b9dd097d1"
CONTROL_AB_RESULT_PATH = Path(
    "/DATA/share/sana_wam_libero_evaluation/"
    "r8_taskbalanced_settle10_same40/"
    "libero-r8-taskbalanced-settle10-same40-c16a4ccdfd0247b0a65c7d2e34220f91/"
    "AB_RESULT.json"
)
CONTROL_AB_RESULT_SHA256 = (
...
WORKER_MARKER = "SANA_WAM_R8_TASKBALANCED_LOWNOISE_SUPERVISED_WORKER"
...
def _load_and_verify_config():
    ...
    exact = {
        ...
        "model.architecture.action_loss_weighting": "low_noise",
    }
    ...
    candidate_weighting = candidate["model"]["architecture"][
        "action_loss_weighting"
    ]
    baseline_weighting = baseline["model"]["architecture"][
        "action_loss_weighting"
    ]
    if candidate_weighting != "low_noise" or baseline_weighting != "none":
        raise RuntimeError(
            "low-noise successor requires the single loss delta none -> low_noise"
        )
    candidate["model"]["architecture"]["action_loss_weighting"] = (
        baseline_weighting
    )
    for payload in (candidate, baseline):
        payload["training"].pop("output_dir")
    if candidate != baseline:
        raise RuntimeError(
            "low-noise config differs from the task-balanced control outside "
            "loss weighting and output root"
        )
...
def _verify_source(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if tracked_status:
        raise RuntimeError(f"repository has tracked changes: {tracked_status}")
...
def _verify_control_authority() -> dict[str, Any]:
    from omegaconf import OmegaConf

    config_path = ROOT / BASELINE_CONFIG_RELATIVE_PATH
    runner_path = ROOT / CONTROL_RUNNER_RELATIVE_PATH
    result_path = CONTROL_ROOT / "RESULT.json"
    checkpoint_path = CONTROL_ROOT / "checkpoint_step_4340.safetensors"
    config = _verify_file(config_path, CONTROL_CONFIG_SHA256)
    runner = _verify_file(runner_path, CONTROL_RUNNER_SHA256)
    result = _verify_file(result_path, CONTROL_RESULT_SHA256)
    checkpoint = _verify_file(checkpoint_path, CONTROL_CHECKPOINT_SHA256)
    same40_ab_result = _verify_file(
        CONTROL_AB_RESULT_PATH, CONTROL_AB_RESULT_SHA256
    )

    cfg = OmegaConf.load(config_path)
    expected_config = {
        "model.architecture.action_loss_weighting": "none",
        "training.output_dir": str(CONTROL_ROOT),
        "training.init_checkpoint": str(
            PREDECESSOR_R8_ROOT / "checkpoint_step_4337.safetensors"
        ),
        "training.init_checkpoint_sha256": PREDECESSOR_R8_CHECKPOINT_SHA256,
    ...
    for path, expected in expected_config.items():
        ...
        if observed != expected:
            raise RuntimeError(
                f"task-balanced control requires {path}={expected!r}, "
                f"got {observed!r}"
            )

    result_payload = json.loads(result_path.read_text(encoding="utf-8"))
    source_payload = result_payload.get("source", {})
    if (
        result_payload.get("valid_run") is not True
        or result_payload.get("execution_result") != "PASS"
        or result_payload.get("verdict")
        != "LIBERO_AR_R8_TASKBALANCED_H8_8GPU_SUCCESSOR1_TRAINING_COMPLETE"
        or result_payload.get("run", {}).get("root") != str(CONTROL_ROOT)
        or result_payload.get("checkpoint", {}).get("sha256")
        != CONTROL_CHECKPOINT_SHA256
        or source_payload.get("repo_commit") != CONTROL_REPO_COMMIT
        or source_payload.get("config", {}).get("sha256")
        != CONTROL_CONFIG_SHA256
        or source_payload.get("runner", {}).get("sha256")
        != CONTROL_RUNNER_SHA256
    ):
        raise RuntimeError("task-balanced control RESULT semantics differ")
    control_training = result_payload.get("training", {})
    if (
        control_training.get("optimizer_steps") != EXPECTED_FINAL_STEP
        or control_training.get("window_draws") != EXPECTED_WINDOW_DRAWS
    ):
        raise RuntimeError("task-balanced control training summary differs")
    _validate_sampler_summary(control_training.get("task_balanced_sampler", {}))

    ab_payload = json.loads(CONTROL_AB_RESULT_PATH.read_text(encoding="utf-8"))
    aggregate = ab_payload.get("aggregate", {})
    ab_training = ab_payload.get("training", {})
    if (
        ab_payload.get("valid_run") is not True
        or ab_payload.get("execution_result") != "PASS"
        or ab_payload.get("verdict")
        != "LIBERO_R8_TASKBALANCED_SETTLE10_SAME40_COMPLETE"
        or aggregate.get("mechanical_complete") is not True
        or aggregate.get("episodes") != 40
        or aggregate.get("successes") != 6
        or ab_training.get("root") != str(CONTROL_ROOT)
        or ab_training.get("result_sha256") != CONTROL_RESULT_SHA256
        or ab_training.get("checkpoint_sha256") != CONTROL_CHECKPOINT_SHA256
    ):
        raise RuntimeError("task-balanced same40 AB_RESULT semantics differ")

    return {
        "training_root": str(CONTROL_ROOT),
        "config": config,
        "runner": runner,
        "result": result,
        "checkpoint": checkpoint,
        "same40_ab_result": same40_ab_result,
        "training_semantics": {
            ...
            "initial_checkpoint_sha256": PREDECESSOR_R8_CHECKPOINT_SHA256,
        },
        "same40_semantics": {"episodes": 40, "successes": 6},
    }
...
def _worker(args: argparse.Namespace) -> None:
    ...
    try:
        rank = dist.get_rank()
        gathered_source: list[Any] = [None] * dist.get_world_size()
        dist.all_gather_object(gathered_source, source)
        canonical_source = _canonical_json_bytes(source)
        if any(
            _canonical_json_bytes(item) != canonical_source
            for item in gathered_source
        ):
            raise RuntimeError("rank source observations differ")
        ...
        if rank == 0:
            _exclusive_json(
                ...
                {
                    "schema_version": "sana-wam-libero-taskbalanced-worker-v1",
                    "source": source,
                    "training": summary,
                },
            )
...
def _supervise(args: argparse.Namespace) -> None:
    ...
    control_authority = _verify_control_authority()
    ...
    attempt = {
        "schema_version": "sana-wam-libero-taskbalanced-lownoise-attempt-v1",
        "run": {"root": str(RUN_ROOT), "nonce": RUN_NONCE},
        "source": source,
        "predecessor": predecessor,
        "control_authority": control_authority,
        "scientific_treatment": {
            "action_loss_weighting_control": "none",
            "action_loss_weighting_treatment": "low_noise",
        },
        "sampler_contract": {
            "contract": "task_balanced_40_v1",
    ...
    try:
        ...
        if _canonical_json_bytes(worker.get("source")) != _canonical_json_bytes(
            source
        ):
            raise RuntimeError("worker source authority differs from supervisor")
        ...
        result = {
            "schema_version": "sana-wam-libero-r8-taskbalanced-lownoise-result-v1",
            "valid_run": True,
            "execution_result": "PASS",
            "verdict": "LIBERO_AR_R8_TASKBALANCED_LOWNOISE_H8_8GPU_SUCCESSOR1_TRAINING_COMPLETE",
            "run": {"root": str(RUN_ROOT), "nonce": RUN_NONCE},
        ...
        if not os.path.lexists(RUN_ROOT / "RESULT.json"):
            _exclusive_json(
                ...
                {
                    "schema_version": "sana-wam-libero-taskbalanced-lownoise-failure-v1",
                    "valid_run": False,
