#!/usr/bin/env python
"""Run one bounded LIBERO T9 Latin-square phase arm (C or D)."""
...
T9_INITIALIZATION_SEED = 20260806
T9_LOSS_RECIPE_SEED = 20260826
T9_MEDIAN_RATIO = 0.95
T9_MIN_IMPROVED = 3
T9_CONFIG_SHA256 = "5df45965d6de3a32c5154a8bb4c41d29d20113bbe8908c1a98c8c63a7bff4a45"
T9_CONFIG_RELATIVE_PATH = "configs/benchmarks/libero/train_libero_ar_baseline.yaml"
...
T9_ELIGIBLE_MANIFEST_BYTES = 8406
T9_ELIGIBLE_MANIFEST_SHA256 = (
    "0b6bfe4c51e8b9e2ac3f538bac7853b5e1e871ea96dda5df6976ee27b5b25370"
)
T9_SELECTION_MANIFEST_BYTES = 3351
T9_SELECTION_MANIFEST_SHA256 = (
...
T9_UPDATE_SAMPLE_MANIFEST_SHA256 = (
    "5199c87af876c437ecec35b57da3e118a2924c44a9bb454c9846e7c0ca968aa6"
)
T9_FIXED_RECIPE_SIGNATURE_SHA256 = (
...
T7_PREDECESSOR_RUNNER_PATH = ROOT / (
    "scripts/smoke_libero_ar_t7_four_suite_cyclic_gpu.py"
)
T8_PREDECESSOR_RESULT_SHA256 = (
    "bfdb852e14a5bd9b1c8e776be9f4ff108899eae65d557cebe42b06f1991b0a18"
)
T8_PREDECESSOR_RESULT_PATH = Path(
...
T8_PREDECESSOR_SOURCE_COMMIT = "9cd1c490d14b3c2437e82225ee8cfdf58646837e"
T8_PREDECESSOR_RUNNER_SHA256 = (
    "9ec74d83c8443d78d2ba765dd766f82a96cccef894ae63d08cf9e2a8dbf97669"
)
T8_PREDECESSOR_RUNNER_PATH = ROOT / (
    "scripts/smoke_libero_ar_t8_phase_rotated_recency_gpu.py"
)
...
def _read_jsonl(path: Path) -> list[dict[str, Any]]:
    if path.is_symlink() or not path.is_file():
        raise RuntimeError(f"T9 metadata must be a regular non-symlink file: {path}")
    rows = []
    for line_number, line in enumerate(
        ...
        if not line:
            raise RuntimeError(
                f"T9 metadata contains a blank row: {path}:{line_number}"
            )
        row = json.loads(line)
        if not isinstance(row, dict):
            raise RuntimeError(
                f"T9 metadata row is not an object: {path}:{line_number}"
            )
        rows.append(row)
    return rows
...
def _build_eligible_probe_manifest(
    ...
    if set(metadata_by_suite) != set(T9_SUITE_ORDER):
        raise RuntimeError("T9 eligible suite set differs")
    expected_raw_counts = {
        SPATIAL_NAME: 432,
        OBJECT_NAME: 454,
        GOAL_NAME: 428,
    ...
    update_by_dataset = {row["dataset"]: row for row in T9_CYCLIC_TRAIN_SAMPLES}
    consumed = {
        (row["dataset"], row["task_index"], row["episode_index"])
        for row in T9_EXCLUDED_SAMPLES
    }
    suites = []
    for dataset in T9_SUITE_ORDER:
        task_rows, episode_rows = metadata_by_suite[dataset]
        if any(set(row) != {"task_index", "task"} for row in task_rows):
            raise RuntimeError(f"T9 tasks metadata schema differs: {dataset}")
        ...
        if (
            ...
            raise RuntimeError(f"T9 task registry differs: {dataset}")
        ...
        for row in episode_rows:
            if set(row) != {"episode_index", "tasks", "length"}:
                raise RuntimeError(f"T9 episodes metadata schema differs: {dataset}")
            ...
            if (
                ...
                raise RuntimeError(f"T9 episodes metadata identity differs: {dataset}")
        ...
        if len(episode_rows) not in {
            ...
            raise RuntimeError(f"T9 source episode count differs: {dataset}")
        tasks = [
            {
        ...
        if (
            ...
            raise RuntimeError(f"T9 eligible episode population differs: {dataset}")
        suites.append(
            {
                "dataset": dataset,
                "episode_count": sum(row["episode_count"] for row in tasks),
                "episodes_jsonl_sha256": T9_SUITE_METADATA_SHA256[dataset][
                    "episodes.jsonl"
                ],
                "task_count": len(tasks),
                "tasks": tasks,
                "tasks_jsonl_sha256": T9_SUITE_METADATA_SHA256[dataset]["tasks.jsonl"],
    ...
    manifest = {
        ...
        "excluded_samples": list(T9_EXCLUDED_SAMPLES),
        "schema_version": (
    ...
    if (
        ...
        or len(_canonical_json_bytes(manifest)) != T9_ELIGIBLE_MANIFEST_BYTES
        or _sha256_json(manifest) != T9_ELIGIBLE_MANIFEST_SHA256
    ):
        raise RuntimeError("T9 eligible same-task probe manifest differs")
    return manifest
...
def _eligible_sample_manifest(dataset_roots: dict[str, Path]) -> dict[str, Any]:
    ...
    for dataset in T9_SUITE_ORDER:
        ...
        if _sha256_file(tasks_path) != T9_SUITE_METADATA_SHA256[dataset]["tasks.jsonl"]:
            raise RuntimeError(f"T9 tasks metadata SHA differs: {dataset}")
        if (
            _sha256_file(episodes_path)
            != T9_SUITE_METADATA_SHA256[dataset]["episodes.jsonl"]
        ):
            raise RuntimeError(f"T9 episodes metadata SHA differs: {dataset}")
...
def _episode_selection_payload_sha256(
    ...
    payload = (
        ...
        f"ELIGIBLE_MANIFEST_SHA256={T9_ELIGIBLE_MANIFEST_SHA256}\n"
        f"DATASET={dataset}\n"
...
def _selection_manifest(eligible_manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    for rank, dataset in enumerate(T9_SUITE_ORDER):
        task_rows = suite_by_dataset[dataset]["tasks"]
        if len(task_rows) != 1:
            raise RuntimeError(f"T9 eligible probe task count differs: {dataset}")
        ...
        expected = T9_FRESH_HELDOUT_SAMPLES[rank]
        sample = {
        ...
        if sample != expected:
            raise RuntimeError(f"T9 mechanically selected sample H{rank} differs")
        samples.append(sample)
    manifest = {
        "eligible_manifest_sha256": T9_ELIGIBLE_MANIFEST_SHA256,
        "samples": samples,
        "schema_version": (
            "sana-wam-libero-t7-four-suite-cyclic-same-task-probe-selection-v1"
        ),
        "selection_rule": T9_SELECTION_RULE,
        "suite_order": list(T9_SUITE_ORDER),
        "t6_predecessor_result_sha256": T6_PREDECESSOR_RESULT_SHA256,
    }
    if (
        len(_canonical_json_bytes(manifest)) != T9_SELECTION_MANIFEST_BYTES
        or _sha256_json(manifest) != T9_SELECTION_MANIFEST_SHA256
    ):
        raise RuntimeError("T9 selected same-task probe manifest differs")
    return manifest


def _update_sample_manifest() -> dict[str, Any]:
    samples = []
    for frozen_sample in T9_CYCLIC_TRAIN_SAMPLES:
    ...
    if _sha256_json(manifest) != T9_UPDATE_SAMPLE_MANIFEST_SHA256:
        raise RuntimeError("T9 update sample manifest differs")
    return manifest


def _select_t9_samples(
    dataset: Any,
    eligible_manifest: dict[str, Any],
    selection_manifest: dict[str, Any],
) -> dict[str, dict[str, Any]]:
    live_episode_rows: dict[str, dict[int, dict[str, Any]]] = {
        name: {} for name in T9_SUITE_ORDER
    }
    live_task_rows: dict[str, dict[int, dict[str, Any]]] = {
        name: {} for name in T9_SUITE_ORDER
    }
    for episode in dataset._episodes:
        if episode.dataset not in T9_SUITE_ORDER:
            continue
        if episode.dataset == GOAL_NAME and episode.episode_index == 82:
            raise RuntimeError("T9 excluded Goal episode 82 entered live registry")
        ...
        if previous_task != task_row or episode.episode_index in suite_episodes:
            raise RuntimeError("T9 live cross-suite registry identity is ambiguous")
    ...
    live_manifest = _build_eligible_probe_manifest(
        {
            ...
            for name in T9_SUITE_ORDER
        }
    )
    if live_manifest != eligible_manifest:
        raise RuntimeError("T9 live eligible cross-suite population differs")

    role_rows = [*T9_CYCLIC_TRAIN_SAMPLES, *selection_manifest["samples"]]
    expected_keys = {
        (
            row["dataset"],
            int(row["task_index"]),
    ...
    for row in role_rows:
        ...
        if len(matches) != 1:
            raise RuntimeError(
                "T9 cross-suite episode/start must occur exactly once: "
    ...
    if set(selected) != set(T9_TRAIN_LABELS) | set(T9_HELDOUT_LABELS):
        raise RuntimeError("T9 selected role label set differs")
    return selected
...
def _paired_four_sample_summary(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("T9 frozen labels differ")
    sample_by_label = {row["label"]: row for row in samples}
    if set(sample_by_label) != expected_labels:
        raise ValueError("T9 sample labels differ from the frozen set")
    rows = []
    for label in labels:
        ...
        if (
            ...
            raise ValueError("T9 losses must be finite with positive pre-loss")
        ratio = after / before
        rows.append(
            {
    ...
    return {
        ...
        "paired_gate": (
            ...
            <= T9_MEDIAN_RATIO
            and sum(row["improved"] for row in rows) >= T9_MIN_IMPROVED
        ),
        "per_sample": rows,
...
def _four_suite_cyclic_summary(
    ...
    cyclic_training_summary = _paired_four_sample_summary(
        update_before,
        update_after,
        samples=T9_CYCLIC_TRAIN_SAMPLES,
    )
    fresh_heldout_summary = _paired_four_sample_summary(
        heldout_before,
        heldout_after,
        samples=T9_FRESH_HELDOUT_SAMPLES,
    )
    update_by_dataset = {
    ...
    if set(update_by_dataset) != set(T9_SUITE_ORDER) or set(heldout_by_dataset) != set(
        T9_SUITE_ORDER
    ):
        raise ValueError("T9 suite coverage differs from the frozen order")
...
def _arm_spec(arm: str) -> dict[str, Any]:
    try:
        return T9_ARM_SPECS[arm]
    except KeyError as exc:
        raise ValueError(f"unknown T9 Latin arm: {arm!r}") from exc


def _cyclic_label_for_step(step_index: int, *, arm: str) -> str:
    if arm not in T9_ARM_SPECS:
        raise ValueError(f"unknown T9 Latin arm: {arm!r}")
    spec = T9_ARM_SPECS[arm]
    if step_index < 0 or step_index >= T9_UPDATE_STEPS:
        raise ValueError("T9 step index is outside the frozen 20-step schedule")
    schedule = spec["schedule"]
    return schedule[step_index % len(schedule)]
...
def _rankdata(values: list[float]) -> list[float]:
    ...
    while cursor < len(ordered):
        ...
        for ordered_index in ordered[cursor:end]:
            ranked[ordered_index] = average_rank
        cursor = end
    return ranked
...
def _spearman_rho(left: list[float], right: list[float]) -> float | None:
    if len(left) != len(right) or len(left) < 2:
        raise ValueError("T9 Spearman inputs must have equal nontrivial length")
...
def _classify_latin_square_results(
    ...
    """Classify a complete four-arm Latin-square consolidation."""
    if set(q_by_suite_by_trailing) != set(T9_SUITE_ORDER):
        raise ValueError("T9 consolidated suite keys differ from the frozen set")
    per_suite = []
    endpoint_count = 0
    strong_count = 0
    for suite in T9_SUITE_ORDER:
        q_by_trailing = q_by_suite_by_trailing[suite]
        if set(q_by_trailing) != {0, 1, 2, 3}:
            raise ValueError(f"T9 consolidated trailing keys differ for suite {suite}")
        q_values = [q_by_trailing[trailing] for trailing in range(4)]
        if any(
            ...
            raise ValueError("T9 consolidated q values must be finite and positive")
        q_values = [float(value) for value in q_values]
        rho = _spearman_rho([0.0, 1.0, 2.0, 3.0], q_values)
        ...
        per_suite.append(
            {
                ...
                "q_average_ranks": _rankdata(q_values),
                "q_by_trailing": {
                    str(trailing): q_values[trailing] for trailing in range(4)
                },
                "rho": rho,
...
def _latin_arm_summary(
    before: dict[str, float],
    own_fifth_update: dict[str, float],
    terminal: dict[str, float],
    *,
    arm: str,
) -> dict[str, Any]:
    spec = _arm_spec(arm)
    labels = (*T9_TRAIN_LABELS, *T9_HELDOUT_LABELS)
    if (
        set(before) != set(labels)
        or set(own_fifth_update) != set(labels)
        or set(terminal) != set(labels)
    ):
        raise ValueError("T9 Latin-arm loss labels differ from the frozen set")
    for mapping in (before, own_fifth_update, terminal):
        if any(not math.isfinite(mapping[label]) for label in labels):
            raise ValueError("T9 Latin-arm losses must be finite")
    if any(before[label] <= 0.0 for label in labels) or any(
        own_fifth_update[label] <= 0.0 or terminal[label] <= 0.0 for label in labels
    ):
        raise ValueError("T9 Latin-arm losses require positive pre/phase/terminal loss")

    per_suite = []
    terminal_update_ratios: list[float] = []
    terminal_heldout_ratios: list[float] = []
    schedule = tuple(spec["schedule"])
    trailing_updates = {
        label: len(schedule) - position - 1 for position, label in enumerate(schedule)
    }
    if trailing_updates != spec["trailing_updates"]:
        raise RuntimeError(f"T9 arm {arm} trailing-update contract is inconsistent")
    for index, dataset in enumerate(T9_SUITE_ORDER):
        update_label = f"A{index}"
        heldout_label = f"H{index}"
        phase_update_ratio = own_fifth_update[update_label] / before[update_label]
        phase_heldout_ratio = own_fifth_update[heldout_label] / before[heldout_label]
        terminal_update_ratio = terminal[update_label] / before[update_label]
        terminal_heldout_ratio = terminal[heldout_label] / before[heldout_label]
        phase_pair_ratio = statistics.mean((phase_update_ratio, phase_heldout_ratio))
        terminal_pair_ratio = statistics.mean(
            (terminal_update_ratio, terminal_heldout_ratio)
        )
        overwrite_penalty = terminal_pair_ratio / phase_pair_ratio
        terminal_update_ratios.append(terminal_update_ratio)
        terminal_heldout_ratios.append(terminal_heldout_ratio)
        per_suite.append(
            {
                "both_improved": (
                    terminal_update_ratio < 1.0 and terminal_heldout_ratio < 1.0
                ),
                "dataset": dataset,
                "heldout_label": heldout_label,
                "own_fifth_heldout_ratio": phase_heldout_ratio,
                "own_fifth_pair_ratio": phase_pair_ratio,
                "own_fifth_update_ratio": phase_update_ratio,
                "overwrite_penalty": overwrite_penalty,
                "phase_overwrite": overwrite_penalty,
                "terminal_cell": trailing_updates[update_label] == 0,
                "terminal_heldout_ratio": terminal_heldout_ratio,
                "terminal_pair_ratio": terminal_pair_ratio,
                "terminal_update_ratio": terminal_update_ratio,
                "q_phase": phase_pair_ratio,
                "q_terminal": terminal_pair_ratio,
                "trailing_updates": trailing_updates[update_label],
                "update_label": update_label,
            }
        )

    terminal_phase_labels = tuple(spec["phase_probe_steps"][20])
    terminal_phase_reproduction = {
        label: own_fifth_update[label] == terminal[label]
        for label in terminal_phase_labels
    }
    if not all(terminal_phase_reproduction.values()):
        raise ValueError(
            f"T9 arm {arm} step-20 phase probes did not reproduce at terminal"
        )
    verdict = spec["verdict"]
    return {
        "arm": arm,
        "arm_id": arm,
        "cycle_labels": list(schedule),
        "joint_improved_suite_count": sum(
            bool(row["both_improved"]) for row in per_suite
        ),
        "phase_probe_steps": {
            str(step): list(probe_labels)
            for step, probe_labels in sorted(spec["phase_probe_steps"].items())
        },
        "per_suite": per_suite,
        "phase_overwrite": {
            row["dataset"]: row["phase_overwrite"] for row in per_suite
        },
        "phase_overwrite_diagnostic": [
            {
                "dataset": row["dataset"],
                "heldout_label": row["heldout_label"],
                "overwrite_penalty": row["overwrite_penalty"],
                "q_phase": row["q_phase"],
                "rA_phase": row["own_fifth_update_ratio"],
                "rH_phase": row["own_fifth_heldout_ratio"],
                "update_label": row["update_label"],
            }
            for row in per_suite
        ],
        "schedule": list(schedule) * T9_CYCLE_COUNT,
        "schedule_unit": list(schedule),
        "suite_order": list(T9_SUITE_ORDER),
        "sorted_terminal_heldout_ratios": sorted(terminal_heldout_ratios),
        "sorted_terminal_update_ratios": sorted(terminal_update_ratios),
        "terminal_phase_labels": list(terminal_phase_labels),
        "terminal_cells": [
            {
                "dataset": row["dataset"],
                "heldout_label": row["heldout_label"],
                "q_terminal": row["q_terminal"],
                "rA_terminal": row["terminal_update_ratio"],
                "rH_terminal": row["terminal_heldout_ratio"],
                "trailing_updates": row["trailing_updates"],
                "update_label": row["update_label"],
            }
            for row in per_suite
        ],
        "terminal_phase_reproduction": terminal_phase_reproduction,
        "terminal_heldout_median_ratio": statistics.median(terminal_heldout_ratios),
        "terminal_update_median_ratio": statistics.median(terminal_update_ratios),
        "trailing_updates_by_update_label": trailing_updates,
        "trailing_updates_by_label": trailing_updates,
        "trailing": trailing_updates,
        "typed_verdict": verdict,
    }
...
def _training_core_projection(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if len(per_step) != T9_UPDATE_STEPS:
        raise ValueError("T9/T3 training-core view requires exactly 20 step rows")
    return {
...
def main() -> int:
    ...
    parser.add_argument("--arm", required=True, choices=tuple(T9_ARM_SPECS))
    ...
    parser.add_argument("--steps", type=int, default=T9_UPDATE_STEPS)
    parser.add_argument(
        "--initialization-seed", type=int, default=T9_INITIALIZATION_SEED
    )
    parser.add_argument("--loss-recipe-seed", type=int, default=T9_LOSS_RECIPE_SEED)
    args = parser.parse_args()
    arm_spec = _arm_spec(args.arm)
    ...
    if args.steps != T9_UPDATE_STEPS:
        raise ValueError(f"T9 steps must be exactly {T9_UPDATE_STEPS}")
    if args.initialization_seed != T9_INITIALIZATION_SEED:
        raise ValueError("T9 initialization seed differs")
    if args.loss_recipe_seed != T9_LOSS_RECIPE_SEED:
        raise ValueError("T9 loss recipe seed differs")
    if len(args.nonce) != 32 or any(
        character not in "0123456789abcdef" for character in args.nonce
    ):
        raise ValueError("T9 nonce must be exactly 32 lowercase hex characters")
    ...
    if args.run_root.expanduser() != expected_run_root:
        raise ValueError(
            "T9 run root differs from the frozen non-formal namespace: "
    ...
    counterpart_root = expected_run_root.parent / (
        f"{T9_ARM_SPECS[counterpart_arm]['root_slug']}-{args.nonce}"
    )
    ...
    if config_path != expected_config_path:
        raise ValueError(
            f"T9 config path differs: expected={expected_config_path} got={config_path}"
        )
    ...
    if actual_config_sha256 != T9_CONFIG_SHA256:
        raise RuntimeError("T9 config content differs from the frozen pin")
    actual_runner_sha256 = _sha256_file(runner_path)
    expected_identity = {
        "config_sha256": T9_CONFIG_SHA256,
    ...
    if cfg.model.architecture.variant != "autoregressive":
        raise RuntimeError("T9 requires the AR architecture")
    if cfg.model.video_backbone.continuous_timestep_conditioning is not True:
        raise RuntimeError("T9 requires continuous FP32 video timesteps")
    if cfg.training.use_gradient_checkpointing is not True:
        raise RuntimeError("T9 requires gradient checkpointing")
    if int(cfg.training.seed) != args.initialization_seed:
        raise RuntimeError("T9 CLI initialization seed must equal training.seed")
    if tuple(cfg.training.trainable_modules) != EXPECTED_TRAINABLE_ROOTS:
        raise RuntimeError("T9 trainable module allowlist differs")
    if (
        tuple(cfg.training.preserve_frozen_input_grad_modules)
        != EXPECTED_PRESERVE_FROZEN_INPUT_GRAD_MODULES
    ):
        raise RuntimeError("T9 frozen input-gradient preservation differs")
    if cfg.training.optimizer_master_weights is not True:
        raise RuntimeError("T9 requires persistent FP32 optimizer masters")
    if (
        float(cfg.training.lambda_video) != 0.0
        or float(cfg.training.lambda_action) != 1.0
    ):
        raise RuntimeError("T9 requires action-only loss weighting")
    if float(cfg.training.weight_decay) != 0.0:
        raise RuntimeError("T9 requires weight_decay=0")
    if (
        float(cfg.training.action_lr) != 1.0e-4
        or float(cfg.training.video_lr) != 1.0e-4
    ):
        raise RuntimeError("T9 requires fixed base learning rates of 1e-4")
    if float(cfg.training.grad_clip) != 1.0:
        raise RuntimeError("T9 requires the fixed gradient clip bound 1.0")
    ...
    expected_datasets = set(T9_SUITE_ORDER)
    if (
        len(dataset_roots) != len(configured_roots)
        or set(dataset_roots) != expected_datasets
    ):
        raise RuntimeError("T9 four-suite dataset root identity differs")
    ...
    external_assets["t8_predecessor/RESULT.json"] = _verify_pinned_asset(
        T8_PREDECESSOR_RESULT_PATH,
        T8_PREDECESSOR_RESULT_SHA256,
    )
    external_assets["t8_predecessor/runner.py"] = _verify_pinned_asset(
        T8_PREDECESSOR_RUNNER_PATH,
        T8_PREDECESSOR_RUNNER_SHA256,
    )
    for dataset in T9_SUITE_ORDER:
        for filename, expected_sha256 in sorted(
            T9_SUITE_METADATA_SHA256[dataset].items()
        ):
    ...
    for role, samples in (
        ("update", T9_CYCLIC_TRAIN_SAMPLES),
        ("heldout", T9_FRESH_HELDOUT_SAMPLES),
    ):
    ...
    if (
        ...
        raise RuntimeError("T9 predecessor T2 evidence semantics differ")
    ...
    if (
        ...
        or t3_predecessor.get("scope", {}).get("backward_calls") != T9_UPDATE_STEPS
        or t3_predecessor.get("scope", {}).get("optimizer_steps") != T9_UPDATE_STEPS
    ):
        raise RuntimeError("T9 predecessor T3 evidence semantics differ")
    ...
    if (
        ...
        or t4_predecessor.get("scope", {}).get("backward_calls") != T9_UPDATE_STEPS
        or t4_predecessor.get("scope", {}).get("optimizer_steps") != T9_UPDATE_STEPS
    ):
        raise RuntimeError("T9 predecessor T4 evidence semantics differ")
    ...
    if (
        ...
        or t5_predecessor.get("scope", {}).get("backward_calls") != T9_UPDATE_STEPS
        or t5_predecessor.get("scope", {}).get("optimizer_steps") != T9_UPDATE_STEPS
    ):
        raise RuntimeError("T9 predecessor T5 evidence semantics differ")
    ...
    if (
        ...
        or t6_predecessor.get("scope", {}).get("backward_calls") != T9_UPDATE_STEPS
        or t6_predecessor.get("scope", {}).get("optimizer_steps") != T9_UPDATE_STEPS
    ):
        raise RuntimeError("T9 predecessor T6 evidence semantics differ")
    ...
    if (
        ...
        raise RuntimeError("T9 frozen T3/T4/T5/T6 training-core evidence differs")
    ...
    if (
        ...
        != T7_PREDECESSOR_RUNNER_SHA256
        or t7_predecessor.get("identity", {}).get("config_sha256") != T9_CONFIG_SHA256
        or t7_predecessor.get("scope", {}).get("prepare_inputs_calls") != 8
        ...
        raise RuntimeError("T9 predecessor T7 identity or execution semantics differ")
    t7_update_ratio_by_label = {
        row["label"]: row["post_to_pre_ratio"]
        for row in t7_predecessor["cyclic_training"]["per_sample"]
    }
    t7_heldout_ratio_by_label = {
        row["label"]: row["post_to_pre_ratio"]
        for row in t7_predecessor["fresh_heldout_transfer"]["per_sample"]
    }
    if (
        t7_update_ratio_by_label != T7_TERMINAL_UPDATE_RATIOS
        or t7_heldout_ratio_by_label != T7_TERMINAL_HELDOUT_RATIOS
        or t7_predecessor.get("starting_state_reproduction", {}).get(
            "observed_action_losses"
        )
        != T9_STARTING_ACTION_LOSSES
    ):
        raise RuntimeError("T9 predecessor T7 frozen loss evidence differs")
    t8_predecessor = json.loads(T8_PREDECESSOR_RESULT_PATH.read_text(encoding="utf-8"))
    if (
        t8_predecessor.get("valid_run") is not True
        or t8_predecessor.get("scientific_verdict")
        != "T8_PHASE_ROTATED_MIXED_INCONCLUSIVE"
        or t8_predecessor.get("identity", {}).get("repo_commit")
        != T8_PREDECESSOR_SOURCE_COMMIT
        or t8_predecessor.get("identity", {}).get("runner_sha256")
        != T8_PREDECESSOR_RUNNER_SHA256
        or t8_predecessor.get("identity", {}).get("config_sha256") != T9_CONFIG_SHA256
        or t8_predecessor.get("scope", {}).get("prepare_inputs_calls") != 8
        or t8_predecessor.get("scope", {}).get("architecture_forwards_total") != 44
        or t8_predecessor.get("scope", {}).get("backward_calls") != 20
        or t8_predecessor.get("scope", {}).get("optimizer_steps") != 20
        or t8_predecessor.get("starting_state_reproduction", {}).get(
            "observed_action_losses"
        )
        != {**T9_STARTING_ACTION_LOSSES, **T9_STARTING_HELDOUT_ACTION_LOSSES}
    ):
        raise RuntimeError("T9 direct predecessor T8 evidence semantics differ")
    external_assets = dict(sorted(external_assets.items()))
    actual_identity.update(
        {
            "arm": args.arm,
            "arm_spec_sha256": _sha256_json(arm_spec),
            ...
            "t8_predecessor_result_sha256": T8_PREDECESSOR_RESULT_SHA256,
            "t8_predecessor_runner_sha256": T8_PREDECESSOR_RUNNER_SHA256,
            "t8_predecessor_source_commit": T8_PREDECESSOR_SOURCE_COMMIT,
            "eligible_manifest_sha256": T9_ELIGIBLE_MANIFEST_SHA256,
            "selection_manifest_sha256": T9_SELECTION_MANIFEST_SHA256,
            "update_sample_manifest_sha256": T9_UPDATE_SAMPLE_MANIFEST_SHA256,
        }
    )
    ...
    if "H200" not in gpu_before["name"].upper():
        raise RuntimeError(f"T9 selected GPU is not an H200: {gpu_before['name']}")
    ...
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError(
            "T9 requires exactly one visible CUDA GPU; "
    ...
    if any(parameter.requires_grad for parameter in video_backbone.parameters()):
        raise RuntimeError("T9 video-backbone parameters are not frozen")
    named_trainables = [
        (name, parameter)
    ...
    if observed_roots != tuple(sorted(EXPECTED_TRAINABLE_ROOTS)):
        raise RuntimeError(f"T9 trainable parameter roots differ: {observed_roots!r}")
    if len(named_trainables) != EXPECTED_TRAINABLE_TENSOR_COUNT:
        raise RuntimeError("T9 trainable tensor count differs")
    if sum(parameter.numel() for _name, parameter in named_trainables) != (
        EXPECTED_TRAINABLE_PARAMETER_COUNT
    ):
        raise RuntimeError("T9 trainable parameter count differs")
    frozen_versions = {name: parameter._version for name, parameter in named_frozen}
    initial_architecture_buffer_versions = {
        name: buffer._version for name, buffer in architecture.named_buffers()
    }
    initial_architecture_buffer_snapshot = [
    ...
    role_selections = _select_t9_samples(
        trainer.dataset,
    ...
    if (
        ...
        raise RuntimeError("T9 fixed real LIBERO sample identity differs")
    raw_samples: dict[str, dict[str, Any]] = {}
    sample_seconds: dict[str, float] = {}
    role_assets_after_sample: dict[str, str] = {}
    role_order = (*T9_TRAIN_LABELS, *T9_HELDOUT_LABELS)
    for label in role_order:
        ...
        if (
            ...
            raise RuntimeError(f"T9 LIBERO sample metadata differs: {label}")
        raw_samples[label] = raw_sample
        role = "update" if label in T9_TRAIN_LABELS else "heldout"
        for relative_path, expected_sha256 in sorted(selected["assets"].items()):
            ...
            if external_assets[key] != observed:
                raise RuntimeError(f"T9 role asset changed while loading: {key}")
            role_assets_after_sample[key] = observed
    if _sha256_file(stats_path) != SELECTED_STATS_SHA256:
        raise RuntimeError("T9 selected-row stats changed while loading")
    ...
    if any(module.training for module in video_backbone.modules()):
        raise RuntimeError("T9 preserved video backbone left eval mode")
    ...
    def prepare_one(label: str, raw_sample: dict[str, Any]) -> dict[str, Any]:
        ...
        if prepared.get("use_gradient_checkpointing") is not True:
            raise RuntimeError(f"T9 {label} inputs dropped gradient checkpointing")
        if prepared.get("use_gradient_checkpointing_offload") is not False:
            raise RuntimeError("T9 requires checkpoint offload=false")
        invalid_tensors = [
            key
        ...
        if invalid_tensors:
            raise RuntimeError(
                f"T9 {label} prepared tensors retain autograd state: "
                f"{invalid_tensors!r}"
            )
        return prepared
    ...
    cyclic_training_inputs = {
        label: prepared_by_label[label] for label in T9_TRAIN_LABELS
    }
    fresh_heldout_inputs = {
        label: prepared_by_label[label] for label in T9_HELDOUT_LABELS
    }
    metadata_after_sample = {}
    for dataset_name in T9_SUITE_ORDER:
        for filename, expected_sha256 in sorted(
            T9_SUITE_METADATA_SHA256[dataset_name].items()
        ):
            ...
            if external_assets[key] != observed:
                raise RuntimeError(f"T9 metadata changed while loading: {key}")
    ...
    if prepare_inputs_calls != 8:
        raise RuntimeError(
            f"T9 prepare_inputs call count differs: {prepare_inputs_calls}"
        )
    eligible_manifest = eligible_sample_manifest
    live_manifest = _eligible_sample_manifest(dataset_roots)
    if live_manifest != eligible_manifest:
        raise RuntimeError(
            "T9 live eligible manifest changed during sample preparation"
        )
    ...
    for label, prepared in prepared_by_label.items():
        for key, value in prepared.items():
            ...
            if previous is not None and previous[0] != label:
                raise RuntimeError(
                    "T9 prepared tensors alias across samples: "
    ...
    if (
        ...
        raise RuntimeError("T9 same-task context pairing differs")

    model_groups = trainer._param_groups()
    if len(model_groups) != 2 or any(
        float(group["lr"]) != 1.0e-4 for group in model_groups
    ):
        raise RuntimeError("T9 optimizer group/LR structure differs")
    ...
    if len(parameter_ids) != len(set(parameter_ids)) or set(parameter_ids) != {
        ...
        raise RuntimeError("T9 optimizer parameters differ from trainables")
    optimizer_groups, master_pairs = trainer._optimizer_param_groups(model_groups)
    if len(master_pairs) != len(params):
        raise RuntimeError("T9 FP32 master coverage differs")
    if [id(model) for model, _master in master_pairs] != parameter_ids:
        raise RuntimeError("T9 FP32 master/model ordering differs")
    ...
    initial_master_projection_mismatches = [
        trainable_name_by_id[id(model)]
        for model, master in master_pairs
        if not torch.equal(master.detach(), model.detach().float())
    ]
    if (
        ...
        or initial_master_projection_mismatches
    ):
        raise RuntimeError("T9 FP32 master identity/dtype differs")
    if [
        id(parameter) for group in optimizer_groups for parameter in group["params"]
    ] != initial_master_ids:
        raise RuntimeError("T9 optimizer groups do not contain the FP32 masters")
    if [len(group["params"]) for group in optimizer_groups] != [
        ...
        raise RuntimeError("T9 FP32 master optimizer group structure differs")
    named_masters = [
    ...
    def probe_mutation_snapshot() -> dict[str, Any]:
        if any(parameter.grad is not None for _name, parameter in named_trainables):
            raise RuntimeError("T9 update-free probe observed model gradients")
        if any(parameter.grad is not None for _name, parameter in named_frozen):
            raise RuntimeError("T9 update-free probe observed frozen gradients")
        if any(master.grad is not None for _name, master in named_masters):
            raise RuntimeError("T9 update-free probe observed master gradients")
        optimizer_state = []
        ...
        *, backward: bool, forward_inputs: dict[str, Any], recipe_seed: int
        ...
        with _fixed_rng_recipe(
            ...
            if (
                ...
                raise RuntimeError("T9 produced a non-finite scalar loss")
            scalars = {
            ...
            if abs(scalars["loss"] - scalars["loss_action"]) > tolerance:
                raise RuntimeError("T9 total/action loss identity differs")
            signature = _sha256_json(recipe_capture)
            if not recipe_capture["action"] or not recipe_capture["video"]:
                raise RuntimeError("T9 stochastic recipe capture is incomplete")
            if backward:
                loss.backward()
                backward_calls += 1
        ...
        if (
            ...
            raise RuntimeError("T9 fixed recipe did not restore caller RNG state")
    ...
    own_fifth_update_measurements: dict[str, dict[str, float]] = {}
    phase_probe_signatures: dict[str, str] = {}
    ...
    try:
        ...
        if optimizer.state:
            raise RuntimeError("T9 optimizer state exists before pre probes")
        cyclic_pre_state = probe_mutation_snapshot()
        for label in T9_TRAIN_LABELS:
        ...
        if probe_mutation_snapshot() != cyclic_pre_state or optimizer.state:
            raise RuntimeError("T9 cyclic training pre probes mutated training state")
        fresh_pre_state = probe_mutation_snapshot()
        for label in T9_HELDOUT_LABELS:
            scalars, signature = fixed_forward(
                backward=False,
        ...
        if probe_mutation_snapshot() != fresh_pre_state or optimizer.state:
            raise RuntimeError("T9 fresh held-out pre probes mutated training state")
        ...
        for label, expected_loss in expected_starting_losses.items():
            ...
            if not passed:
                raise RuntimeError(
                    f"T9 starting action loss differs from frozen T7 for {label}: "
                    f"expected={expected_loss} observed={observed_loss} "
                    f"absolute_error={absolute_error} allowed_error={allowed_error}"
                )

        for step_index in range(T9_UPDATE_STEPS):
            step_started = time.perf_counter()
            update_label = _cyclic_label_for_step(step_index, arm=args.arm)
            ...
            if step_index == 0 and step_scalars != measurement_before[update_label]:
                raise RuntimeError("T9 fixed initial loss recipe is not reproducible")
            ...
            for name, parameter in named_trainables:
                ...
                if not bool(torch.isfinite(gradient).all().item()):
                    raise RuntimeError(
                        f"T9 non-finite gradient at step {step_index + 1}: {name}"
                    )
            ...
            if missing_gradients:
                raise RuntimeError(
                    f"T9 missing gradients at step {step_index + 1}: "
                    f"{missing_gradients[:8]!r}"
                )
            if nonzero_gradient_roots != set(EXPECTED_TRAINABLE_ROOTS):
                raise RuntimeError(
                    f"T9 nonzero-gradient roots differ at step {step_index + 1}: "
                    f"{sorted(nonzero_gradient_roots)!r}"
                )
            if any(parameter.grad is not None for _name, parameter in named_frozen):
                raise RuntimeError("T9 frozen parameters received gradients")

            grad_norm = torch.nn.utils.clip_grad_norm_(
                params, float(cfg.training.grad_clip)
            )
            if not bool(torch.isfinite(grad_norm).item()):
                raise RuntimeError("T9 gradient norm is non-finite")
            ...
            if step_index == 0:
                ...
                if invalid_master_gradients:
                    raise RuntimeError(
                        "T9 first-step masters lack finite nonzero FP32 gradients: "
                ...
                if {row["name"] for row in master_probes} != {
                    ...
                    raise RuntimeError("T9 first-step master probe coverage differs")
                ...
                for root in EXPECTED_TRAINABLE_ROOTS:
                    ...
                    if len(candidates) != 4:
                        raise RuntimeError(f"T9 lacks four update sentinels for {root}")
            ...
            if step_index == 0:
                ...
                if len(changed_master_ids) != len(named_masters):
                    raise RuntimeError("T9 first step did not update every FP32 master")
            ...
            for before, probe in zip(sentinel_before, update_sentinels, strict=True):
                ...
                if not all(
                    ...
                    raise RuntimeError("T9 update sentinel became non-finite")
            ...
            if changed_master_sentinel_roots != set(EXPECTED_TRAINABLE_ROOTS):
                raise RuntimeError(
                    "T9 sampled FP32 master update roots differ at step "
                    f"{step_index + 1}: "
            ...
            if sentinel_adam_steps != {float(step_index + 1)}:
                raise RuntimeError(
                    "T9 sampled AdamW step counters differ at step "
            ...
            if projection_mismatches:
                raise RuntimeError(
                    f"T9 BF16 projection differs: {projection_mismatches[:8]!r}"
                )
            projection_exact_steps.append(step_index + 1)
            if not _optimizer_state_is_finite(optimizer):
                raise RuntimeError("T9 AdamW state is non-finite")
            if step_index == 0:
            ...
            per_step.append(
                {
                    ...
                    "cycle": step_index // len(T9_TRAIN_LABELS) + 1,
                    "cycle_slot": step_index % len(T9_TRAIN_LABELS),
            ...
            phase_labels = arm_spec["phase_probe_steps"].get(step_index + 1)
            if phase_labels is not None:
                if phase_labels[0] != update_label:
                    raise RuntimeError("T9 fifth-update phase label differs")
                optimizer.zero_grad(set_to_none=True)
                for parameter in params:
                    parameter.grad = None
                phase_probe_state = probe_mutation_snapshot()
                for phase_label in phase_labels:
                    phase_inputs = (
                        cyclic_training_inputs[phase_label]
                        if phase_label in T9_TRAIN_LABELS
                        else fresh_heldout_inputs[phase_label]
                    )
                    scalars, signature = fixed_forward(
                        backward=False,
                    ...
                    own_fifth_update_measurements[phase_label] = scalars
                    phase_probe_signatures[phase_label] = signature
                if probe_mutation_snapshot() != phase_probe_state:
                    raise RuntimeError("T9 own-fifth-update phase probes mutated state")
        ...
        if probe_mutation_snapshot() != cyclic_post_state:
            raise RuntimeError("T9 cyclic training post probes mutated training state")
        ...
        if probe_mutation_snapshot() != fresh_post_state:
            raise RuntimeError("T9 fresh held-out post probes mutated training state")
    finally:
    ...
    if architecture_forward_calls != 44:
        raise RuntimeError(
            f"T9 architecture forward count differs: {architecture_forward_calls}"
        )
    if backward_calls != 20:
        raise RuntimeError(f"T9 backward call count differs: {backward_calls}")
    if optimizer_step_calls != 20:
        raise RuntimeError(
            f"T9 optimizer step call count differs: {optimizer_step_calls}"
        )
    if (
        len(update_recipe_signatures) != T9_UPDATE_STEPS
        or len(set(update_recipe_signatures)) != 1
    ):
        raise RuntimeError("T9 update RNG recipe signatures differ across forwards")
    ...
    if invalid_measurement_signatures:
        raise RuntimeError(
            "T9 measurement RNG recipe signatures differ: "
            f"{invalid_measurement_signatures!r}"
        )
    if set(own_fifth_update_measurements) != set(role_order) or set(
        phase_probe_signatures
    ) != set(role_order):
        raise RuntimeError("T9 own-fifth-update phase probe coverage differs")
    terminal_phase_labels = arm_spec["phase_probe_steps"][20]
    if any(
        own_fifth_update_measurements[label] != measurement_after[label]
        for label in terminal_phase_labels
    ):
        raise RuntimeError("T9 step-20 phase/terminal repeatability differs")
    all_recipe_signatures = (
        update_recipe_signatures
        + [
            signature
            for label in role_order
            for signature in measurement_signatures[label]
        ]
        + [phase_probe_signatures[label] for label in role_order]
    )
    if len(all_recipe_signatures) != 44 or set(all_recipe_signatures) != {
        T9_FIXED_RECIPE_SIGNATURE_SHA256
    }:
        raise RuntimeError("T9 fixed loss recipe signature differs across forwards")
    expected_exposures = {
        **{label: 5 for label in T9_TRAIN_LABELS},
        **{label: 0 for label in T9_HELDOUT_LABELS},
    }
    if update_exposure_counts != expected_exposures:
        raise RuntimeError(
            f"T9 cyclic update exposure counts differ: {update_exposure_counts!r}"
        )
    if projection_exact_steps != list(range(1, T9_UPDATE_STEPS + 1)):
        raise RuntimeError("T9 per-step full projection evidence differs")
    ...
    if final_optimizer_master_ids != initial_master_ids:
        raise RuntimeError("T9 FP32 master objects changed during the loop")
    if prepared_input_snapshot() != initial_prepared_input_snapshot:
        raise RuntimeError("T9 prepared input identities or versions changed")
    final_architecture_buffer_versions = {
        name: buffer._version for name, buffer in architecture.named_buffers()
    }
    if (
        final_architecture_buffer_versions != initial_architecture_buffer_versions
        or [
            (name, id(buffer), buffer.data_ptr(), buffer._version)
            for name, buffer in architecture.named_buffers()
        ]
        ...
        raise RuntimeError("T9 architecture buffers changed during the update loop")
    ...
    if frozen_version_changes:
        raise RuntimeError(
            f"T9 frozen parameters changed: {frozen_version_changes[:8]!r}"
        )
    if any(not bool(torch.isfinite(master).all().item()) for master in masters):
        raise RuntimeError("T9 FP32 masters became non-finite")
    if any(not bool(torch.isfinite(parameter).all().item()) for parameter in params):
        raise RuntimeError("T9 BF16 trainables became non-finite")
    if not _optimizer_state_is_finite(optimizer):
        raise RuntimeError("T9 final AdamW state is non-finite")
    adam_steps: list[float] = []
    for master in masters:
        state = optimizer.state.get(master)
        if not state or "step" not in state:
            raise RuntimeError("T9 AdamW state is missing for an FP32 master")
        step_value = _tensor_scalar(state["step"])
        if not math.isfinite(step_value):
            raise RuntimeError("T9 AdamW step counter is non-finite")
        adam_steps.append(step_value)
    if set(adam_steps) != {float(T9_UPDATE_STEPS)}:
        raise RuntimeError(
            f"T9 AdamW step counters differ: {sorted(set(adam_steps))!r}"
        )
    ...
    if len(cumulative_master_ids) != len(named_masters) or set(
        ...
        raise RuntimeError("T9 cumulative FP32 master update coverage differs")

    four_suite_cyclic = _four_suite_cyclic_summary(
        {label: measurement_before[label]["loss_action"] for label in T9_TRAIN_LABELS},
        {label: measurement_after[label]["loss_action"] for label in T9_TRAIN_LABELS},
        {
            label: measurement_before[label]["loss_action"]
            for label in T9_HELDOUT_LABELS
        },
        {label: measurement_after[label]["loss_action"] for label in T9_HELDOUT_LABELS},
    )
    ...
    latin_arm = _latin_arm_summary(
        {label: measurement_before[label]["loss_action"] for label in role_order},
        {
            label: own_fifth_update_measurements[label]["loss_action"]
            for label in role_order
        },
        {label: measurement_after[label]["loss_action"] for label in role_order},
        arm=args.arm,
    )
    latin_arm["joint_viable_diagnostic"] = four_suite_cyclic["gate"]
    verdict = latin_arm["typed_verdict"]
    verdict_reasons = [
        f"the frozen T9 Latin-square arm {args.arm} harness completed validly"
    ]
    sample_by_label = {
        row["label"]: row
        for row in (*T9_CYCLIC_TRAIN_SAMPLES, *T9_FRESH_HELDOUT_SAMPLES)
    }
    ...
    four_suite_cyclic_report = {
        **four_suite_cyclic,
        ...
        "schedule": list(arm_spec["schedule"]) * T9_CYCLE_COUNT,
    ...
    cyclic_schedule_report = {
        "cycle_count": T9_CYCLE_COUNT,
        "arm": args.arm,
        "labels": list(arm_spec["schedule"]) * T9_CYCLE_COUNT,
        "phase_probe_steps": {
            str(step): list(labels)
            for step, labels in sorted(arm_spec["phase_probe_steps"].items())
        },
        "updates_per_sample": T9_UPDATES_PER_SAMPLE,
    }
    phase_step_by_label = {
        label: step
        for step, labels in arm_spec["phase_probe_steps"].items()
        for label in labels
    }
    phase_terminal_measurements = {}
    for label in role_order:
        pre = measurement_before[label]
        phase = own_fifth_update_measurements[label]
        terminal = measurement_after[label]
        phase_action_ratio = phase["loss_action"] / pre["loss_action"]
        terminal_action_ratio = terminal["loss_action"] / pre["loss_action"]
        pre_component_tolerance = 1.0e-6 + 1.0e-6 * abs(pre["loss_action"])
        phase_terminal_measurements[label] = {
            "dataset": sample_by_label[label]["dataset"],
            "expected_pre_action_loss": {
                **T9_STARTING_ACTION_LOSSES,
                **T9_STARTING_HELDOUT_ACTION_LOSSES,
            }[label],
            "expected_pre_action_loss_within_tolerance": (
                starting_state_components[label]["passed"]
            ),
            "improved_at_phase": phase_action_ratio < 1.0,
            "improved_at_terminal": terminal_action_ratio < 1.0,
            "label": label,
            "phase_action_delta": phase["loss_action"] - pre["loss_action"],
            "phase_action_loss": phase["loss_action"],
            "phase_action_relative_drop": 1.0 - phase_action_ratio,
            "phase_action_ratio": phase_action_ratio,
            "phase_probe_after_step": phase_step_by_label[label],
            "phase_recipe_signature_sha256": phase_probe_signatures[label],
            "phase_total_loss": phase["loss"],
            "pre_action_loss": pre["loss_action"],
            "pre_total_action_absolute_error": abs(pre["loss"] - pre["loss_action"]),
            "pre_total_action_tolerance": pre_component_tolerance,
            "pre_total_action_within_tolerance": (
                abs(pre["loss"] - pre["loss_action"]) <= pre_component_tolerance
            ),
            "pre_recipe_signature_sha256": measurement_signatures[label][0],
            "pre_total_loss": pre["loss"],
            "terminal_action_delta": terminal["loss_action"] - pre["loss_action"],
            "terminal_action_loss": terminal["loss_action"],
            "terminal_action_relative_drop": 1.0 - terminal_action_ratio,
            "terminal_action_ratio": terminal_action_ratio,
            "terminal_recipe_signature_sha256": measurement_signatures[label][1],
            "terminal_total_loss": terminal["loss"],
        }
    latin_square_arm_report = {
        **latin_arm,
        "step20_phase_terminal_reproduction": {
            label: {
                "action_equal": (
                    own_fifth_update_measurements[label]["loss_action"]
                    == measurement_after[label]["loss_action"]
                ),
                "total_equal": (
                    own_fifth_update_measurements[label]["loss"]
                    == measurement_after[label]["loss"]
                ),
            }
            for label in terminal_phase_labels
        },
        "measurements": phase_terminal_measurements,
    }
    ...
    architecture_report = {
        ...
        "global_buffer_versions_final": final_architecture_buffer_versions,
        "global_buffer_versions_initial": initial_architecture_buffer_versions,
        "global_buffers_unchanged": True,
    ...
    optimizer_report = {
        ...
        "initial_master_equals_model_float": not initial_master_projection_mismatches,
        "initial_master_projection_mismatches": initial_master_projection_mismatches,
    ...
    randomness_report = {
        ...
        "eligible_same_task_probe_manifest_sha256": T9_ELIGIBLE_MANIFEST_SHA256,
        "initialization_seed": args.initialization_seed,
        "measurement_recipe_signatures_per_label": measurement_signatures,
        "phase_probe_recipe_signatures_per_label": phase_probe_signatures,
        ...
        "selected_same_task_probe_manifest_sha256": T9_SELECTION_MANIFEST_SHA256,
    ...
    sample_roles_report = {
        label: {
            ...
            "role": "update" if label in T9_TRAIN_LABELS else "fresh_same_task_probe",
    ...
    scope_report = {
        ...
        "own_fifth_update_phase_forwards": 8,
        ...
        "probe_order": [
            ...
            f"updates_{'_'.join(arm_spec['schedule'])}_repeated_four_cycles",
            "cycle5_updates_with_dynamic_post_update_AH_phase_probes",
            "terminal_A0_A1_A2_A3_H0_H1_H2_H3",
        ],
    ...
    starting_state_reproduction = {
        ...
        "current_schedule_is_frozen_t9_latin_arm": True,
        "frozen_tolerance_reproduction_expected": True,
        ...
        "per_label_within_tolerance": {
            label: starting_state_components[label]["passed"] for label in role_order
        },
        "predecessor_result_sha256": T8_PREDECESSOR_RESULT_SHA256,
        "reproduced": True,
        "starting_action_losses_within_frozen_t8_tolerance": True,
    }
    report = {
        ...
        "input_integrity": {
            "architecture_global_buffers_unchanged": True,
        ...
        "interpretation_limits": {
            ...
            "combined_latin_square_classification_executed": False,
            "joint_viability_is_diagnostic_only": True,
            "heldout_axis": (
        ...
        "per_step": per_step,
        "latin_square_arm": latin_square_arm_report,
        ...
        "timings_seconds": {
            ...
            "twenty_updates_and_twenty_four_measurements": update_seconds,
        },
        "predecessor_training_core_lineage": predecessor_training_core_lineage,
        "direct_predecessor": {
            "result_path": str(T8_PREDECESSOR_RESULT_PATH),
            "result_sha256": T8_PREDECESSOR_RESULT_SHA256,
            "runner_sha256": T8_PREDECESSOR_RUNNER_SHA256,
            "source_commit": T8_PREDECESSOR_SOURCE_COMMIT,
            "stage": "T8",
        },
        "valid_run": True,
